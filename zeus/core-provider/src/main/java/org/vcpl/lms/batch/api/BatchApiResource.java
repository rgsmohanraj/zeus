 ...
 * under the License.
 */
package org.vcpl.lms.batch.api;
...
import org.vcpl.lms.batch.serialization.BatchRequestJsonHelper;
import org.vcpl.lms.batch.service.BatchApiService;
 ...
 * {@link org.vcpl.lms.batch.service.BatchApiService} and de-serializes the incoming JSON string to a list of
 * {@link org.vcpl.lms.batch.domain .BatchRequest} type. This list is forwarded to BatchApiService which finally
 * returns a list of {@link org.vcpl.lms.batch.domain.BatchResponse} type which is then serialized into JSON
 * response by this Resource class.
 *
 * @author Rishabh Shukla
 *
 * @see org.vcpl.lms.batch.service.BatchApiService
...
@Component
@Scope("singleton")
@Tag(name = "Batch API", description = "The Zeus  Batch API enables a consumer to access significant amounts of data in a single call or to make changes to several objects at once. Batching allows a consumer to pass instructions for several operations in a single HTTP request. A consumer can also specify dependencies between related operations. Once all operations have been completed, a consolidated response will be passed back and the HTTP connection will be closed.\n"
        + "\n"
...
public class BatchApiResource {
    ...
    @Operation(summary = "Batch requests in a single transaction", description = "The Zeus Batch API is also capable of executing all the requests in a single transaction, by setting a Query Parameter, \"enclosingTransaction=true\". So, if one or more of the requests in a batch returns an erroneous response all of the Data base transactions made by other successful requests will be rolled back.\n"
            + "\n"
