...
public class LoanProductWritePlatformServiceJpaRepositoryImpl implements LoanProductWritePlatformService {
    ...
    public LoanProductWritePlatformServiceJpaRepositoryImpl(final PlatformSecurityContext context,
                                                            ...
                                                            final AprCalculator aprCalculator, final FundRepository fundRepository,final LoanProductClassRepository loanProductClassRepository,final LoanProductTypeRepository loanProductTypeRepository,
    ...
    public CommandProcessingResult createLoanProduct(final JsonCommand command) {
        ...
        try {
            ...
            final LoanProductClass loanProductClass = findClassByIdIfProvided(command.longValueOfParameterNamed("classId"));
            final LoanProductType loanProductType = findTypeByIdIfProvided(command.longValueOfParameterNamed("typeId"));
            ...
            final LoanProduct loanproduct = LoanProduct.assembleFromJson(fund, loanProductClass,loanProductType,loanTransactionProcessingStrategy, charges, command,
    ...
    private LoanProductClass findClassByIdIfProvided(final Long classId) {
        LoanProductClass loanProductClass = null;
        if (classId != null) {
            loanProductClass = this.loanProductClassRepository.findById(classId).orElseThrow(() -> new LoanClassNotFoundException(classId));
        }
        return loanProductClass;
    }

    private LoanProductType findTypeByIdIfProvided(final Long typeId) {
        LoanProductType loanProductType = null;
        if (typeId != null) {
            loanProductType = this.loanProductTypeRepository.findById(typeId).orElseThrow(() -> new LoanTypeNotFoundException(typeId));
        }
