...
import org.vcpl.lms.infrastructure.dataqueries.data.ReportParameterData;
import org.vcpl.lms.infrastructure.dataqueries.data.ReportParameterJoinData;
import org.vcpl.lms.infrastructure.dataqueries.data.ResultsetColumnHeaderData;
import org.vcpl.lms.infrastructure.dataqueries.data.ResultsetRowData;
import org.vcpl.lms.infrastructure.dataqueries.exception.ReportNotFoundException;
import org.vcpl.lms.infrastructure.documentmanagement.contentrepository.FileSystemContentRepository;
import org.vcpl.lms.infrastructure.security.service.PlatformSecurityContext;
import org.vcpl.lms.infrastructure.security.service.SqlInjectionPreventerService;
...
//        sql = this.genericDataService.wrapSQL(sql);
    ...
    private Collection<ReportData> retrieveReports(final Long id) {
        ...
        List<String> permissions = context.authenticatedUser().getRoles().stream()
                .flatMap(role -> role.getPermissions().stream())
                .filter(permission -> permission.getGrouping().equals("report"))
                .map(permission -> permission.getCode().substring(5))
                .collect(Collectors.toList());

        final ReportParameterJoinMapper rm = new ReportParameterJoinMapper();

        final String sql = rm.schema(id,permissions);
        ...
        String jasperReportTemplate = null;
            ...
            if (rpJoin.getReportId().equals(prevReportId)) {
                ...
                if (firstReport) {
                    ...
                    reportList.add(new ReportData(reportId, reportName, reportType, reportSubType, reportCategory, description, reportSql,
                            coreReport, useReport,jasperReportTemplate,reportParameters));
                }
                ...
                jasperReportTemplate = rpJoin.getJasperReportTemplate();
        ...
        reportList.add(new ReportData(reportId, reportName, reportType, reportSubType, reportCategory, description, reportSql, coreReport,
                useReport, jasperReportTemplate,reportParameters));
    ...
    private static final class ReportParameterJoinMapper implements RowMapper<ReportParameterJoinData> {
        ...
        public String schema(final Long reportId,List<String> permissions) {

            String sql = "select r.id as reportId, r.report_name as reportName, r.report_type as reportType, "
                    + " r.report_subtype as reportSubType, r.report_category as reportCategory, r.description, r.core_report as coreReport, r.use_report as useReport,r.jasper_report_template_path as jasperReportTemplate,"
            ...
            if (reportId != null) {
                ...
                sql += " where r.use_report = 1 ";
                if(!permissions.isEmpty() && !permissions.contains("ALL_REPORTS")) {
                    sql += " and r.report_name in (" + permissions.stream().map(s -> String.format("'%s'",s))
        ...
        public ReportParameterJoinData mapRow(final ResultSet rs, final int rowNum) throws SQLException {
            ...
            final String jasperReportTemplate = rs.getString("jasperReportTemplate");

            return new ReportParameterJoinData(reportId, reportName, reportType, reportSubType, reportCategory, description, reportSql,
                    coreReport, useReport, reportParameterId, parameterId, reportParameterName, parameterName,jasperReportTemplate);
        }
    }
    ...
    public String jasperTemplateParameters(String reportName) {
        jasperParameterRecord parameterRowMapper = new jasperParameterRecord();
        String sql = """
                SELECT parameter_variable as parameterVariable FROM  stretchy_parameter where id in (select parameter_id from stretchy_report_parameter where report_id = (select id from stretchy_report where report_name = ?))""";
        return this.jdbcTemplate.queryForObject(sql,parameterRowMapper,new Object[]{reportName});
    }
    private static final class  jasperParameterRecord implements RowMapper<String>{
        @Override
        public String mapRow(ResultSet rs, int rowNum) throws SQLException {
            String parameterVariable = rs.getString("parameterVariable");
            return parameterVariable;
        }
    }

    public  String jasperReportTemplateName(String reportname) {
        soaRecord reportRowMapper = new soaRecord();
        String sql = """
        select jasper_report_template_path as jasperReportTemplate
          from stretchy_report where report_name = ? 
          """;
        return this.jdbcTemplate.queryForObject(sql,reportRowMapper,new Object[]{reportname});
    }
    private static final class  soaRecord implements RowMapper<String>{
        @Override
        public String mapRow(ResultSet rs, int rowNum) throws SQLException {
            String jasperReportTemplate = rs.getString("jasperReportTemplate");
