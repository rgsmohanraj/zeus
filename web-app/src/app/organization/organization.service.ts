...
export class OrganizationService {
  ...
  getProvisioningCriterias(): Observable<any> {
   ...
   getOfficesTemplate(): Observable<any> {
        return this.http.get('/offices/template');//office resolver
      }
...
//   getBulkLoans(): Observable<any> {
//       return this.http.disableApiPrefix().get('/assets/mock/bulk_loan.json');
//     }

 /**
        * @returns {Observable<any>} Get product Partner Template.
        */
       getBulkLoans(): Observable<any> {
   ...
   */
  getProduct(partnerId: any): Observable<any> {
    const httpParams = new HttpParams()
      .set('partnerId', partnerId.toString());
    return this.http.get('/product', { params: httpParams });
  }
   ...
   */
  getImportTemplate(urlSuffix: string, partnerId: any, productId: any, staffId: any, legalFormType: string): Observable<any> {
    let httpParams = new HttpParams();
      /* .set('tenantIdentifier', 'default')
      .set('locale', this.settingsService.language.code)
      .set('dateFormat', this.settingsService.dateFormat); */
    if (partnerId) {
      httpParams = httpParams.set('partnerId', partnerId.toString());
    }
    if (productId) {
    ...
    return this.http.get(`${urlSuffix}/uploadtemplate`, { params: httpParams, responseType: 'arraybuffer', observe: 'response' });
  }

  /**
       * Gets bulk Collection downloadable template from API.
       */
  getCollectionImportTemplate(urlSuffix: string,productId: any): Observable<any> {
      let httpParams = new HttpParams();
 if (productId) {
          httpParams = httpParams.set('productId', productId.toString());
        }
...
//         .set('dateFormat', this.settingsService.dateFormat);
    ...
    getChargeImportTemplate( partnerId: any, productId: any): Observable<any> {
        let httpParams = new HttpParams();
        if (partnerId) {
          httpParams = httpParams.set('partnerId', partnerId.toString());
        }
        ...
        return this.http.get(`/charges/collection/template`, { params: httpParams, responseType: 'arraybuffer', observe: 'response' });
      }


         /**
        * @returns {Observable<any>} Get Bulk Reports Data.
        */
            getReportsData(urlSuffix: string,fromDate,toDate,typeId: any) :Observable<any>
            {
             let httpParams = new HttpParams() .set('fromDate', fromDate)
                                                   .set('toDate', toDate)
                                                   .set('type', typeId);
            return this.http.get(`${urlSuffix}/bulkReportsFilter`, { params: httpParams });
            }
  ...
  getImportDocument(id: any): Observable<any> {
    const httpParams = new HttpParams()
      ...
      .set('tenantIdentifier', environment.zeusPlatformTenantId);
  ...
  uploadImportDocument(file: File, urlSuffix: string,productId: any, legalFormType: string): Observable<any> {
    let httpParams = new HttpParams();
     if (productId) {
          httpParams = httpParams.set('productId', productId.toString());
        }
...
//     formData.append('dateFormat', this.settingsService.dateFormat);
     return this.http.post(`${urlSuffix}/upload`, formData, { params: httpParams });
//     return this.http.post(`${urlSuffix}/uploadtemplate`, formData, { params: httpParams });
  ...
  /**
    ...
    uploadCollectionImportDocument(file: File, urlSuffix: string): Observable<any> {
      let httpParams = new HttpParams();
      const formData = new FormData();
      formData.append('file', file);
      formData.append('name', file.name);
  ...
  //     return this.http.post(`${urlSuffix}/upload`, formData, { params: httpParams });
    ...
    uploadChargeDocument(file: File,partnerId:any ,productId: any): Observable<any> {
    let httpParams = new HttpParams();
     if (partnerId) {
              httpParams = httpParams.set('partnerId', partnerId.toString());
            }
    ...
    formData.append('file', file);
    formData.append('name', file.name);
     return this.http.post(`/charges/upload`,formData, { params: httpParams });
  }
