 ...
 * under the License.
 */
package org.vcpl.lms.portfolio.calendar.service;
...
public final class CalendarUtils {
    ...
    private static LocalDate getNextRecurringDate(final Recur recur, final LocalDate seedDate, final LocalDate startDate) {
        DateTime periodStart = new DateTime(java.util.Date.from(startDate.atStartOfDay(ZoneId.systemDefault()).toInstant()));
    ...
    public static LocalDate generateNextDueDateBasedOnDueDate(String reccuringRule, LocalDate seedDate, LocalDate repaymentDate, Integer loanRepaymentInterval,
                                                              String frequency, boolean skipRepaymentOnFirstDayofMonth, Integer numberOfdays) {


        final Recur recur = CalendarUtils.getICalRecur(reccuringRule);
        if (recur == null) {
            return null;
        }
    ...
    public static LocalDate adjustDateForDueDate(LocalDate date, LocalDate seedDate, PeriodFrequencyType frequencyType,Integer nthDay) {
        LocalDate adjustedVal = date;
        if (frequencyType.isMonthly() && nthDay  > 0) {
            switch (date.getMonthValue()) {
                case 2:
                    if(date.isLeapYear() && nthDay >= 29 ){
                        adjustedVal = date.withDayOfMonth(29);
                    } else if (nthDay  > 28) {
                        adjustedVal = date.withDayOfMonth(28);
                    }else{
                        adjustedVal = date.withDayOfMonth(nthDay);
                    }
                    break;
                case 4, 6, 9, 11:
                    adjustedVal = nthDay> 30 ? date.withDayOfMonth(30) : date.withDayOfMonth(nthDay);
                    break;
                case 1:
                case 3:
                case 5:
    ...
    public static void validateNthDayOfMonthFrequency(DataValidatorBuilder baseDataValidator, final String repeatsOnNthDayOfMonthParamName,
        ...
        baseDataValidator.reset().parameter("Due Day").value(repeatsOnNthDayOfMonth).ignoreIfNull()
                .isOneOfTheseValues(NthDayType.ONE.getValue(), NthDayType.TWO.getValue(), NthDayType.THREE.getValue(),
                        NthDayType.FOUR.getValue(),NthDayType.FIVE.getValue(), NthDayType.SIX.getValue(), NthDayType.SEVEN.getValue(), NthDayType.EIGHT.getValue(), NthDayType.NINE.getValue(),
                        NthDayType.TEN.getValue(), NthDayType.ELEVEN.getValue(), NthDayType.TWELVE.getValue(), NthDayType.THIRTEEN.getValue(), NthDayType.FOURTEEN.getValue(),NthDayType.FIFTEEN.getValue(), NthDayType.SIXTEEN.getValue(),
                        NthDayType.SEVENTEEN.getValue(), NthDayType.EIGHTEEN.getValue(), NthDayType.NINETEEN.getValue(), NthDayType.TWENTY.getValue(),NthDayType.TWENTYONE.getValue(), NthDayType.TWENTYTWO.getValue(),
                        NthDayType.TWENTYTHREE.getValue(),NthDayType.TWENTYFOUR.getValue(), NthDayType.TWENTYFIVE.getValue(), NthDayType.TWENTYSIX.getValue(),NthDayType.TWENTYSEVEN.getValue(), NthDayType.TWENTYEIGHT.getValue(),
                        NthDayType.TWENTYNINE.getValue(), NthDayType.THIRTY.getValue(),NthDayType.THIRTYONE.getValue());
...
//                .inMinMaxRange(CalendarWeekDaysType.getMinValue(), CalendarWeekDaysType.getMaxValue());
        ...
        if (nthDayType != null && nthDayType != NthDayType.INVALID) {
            if (nthDayType == NthDayType.ONE || nthDayType == NthDayType.TWO || nthDayType == NthDayType.THREE
                    || nthDayType == NthDayType.FOUR|| nthDayType == NthDayType.FIVE || nthDayType == NthDayType.SIX ||
                    nthDayType == NthDayType.SEVEN  || nthDayType == NthDayType.EIGHT || nthDayType == NthDayType.NINE ||
                    nthDayType == NthDayType.TEN || nthDayType == NthDayType.ELEVEN || nthDayType == NthDayType.TWELVE ||
                    nthDayType == NthDayType.THIRTEEN || nthDayType == NthDayType.FOURTEEN || nthDayType == NthDayType.FIFTEEN ||
                    nthDayType == NthDayType.SIXTEEN || nthDayType == NthDayType.SEVENTEEN || nthDayType == NthDayType.EIGHTEEN ||
                    nthDayType == NthDayType.NINETEEN || nthDayType == NthDayType.TWENTY || nthDayType == NthDayType.TWENTYONE ||
                    nthDayType == NthDayType.TWENTYTWO || nthDayType == NthDayType.TWENTYTHREE || nthDayType == NthDayType.TWENTYFOUR ||
                    nthDayType == NthDayType.TWENTYFIVE || nthDayType == NthDayType.TWENTYSIX || nthDayType == NthDayType.TWENTYSEVEN ||
                    nthDayType == NthDayType.TWENTYEIGHT || nthDayType == NthDayType.TWENTYNINE || nthDayType == NthDayType.THIRTY || nthDayType == NthDayType.THIRTYONE) {
