 ...
 * under the License.
 */
package org.vcpl.lms.organisation.office.data;
...
public class OfficeData implements Serializable {
    ...
    public static OfficeData template(final List<OfficeData> parentLookups, final LocalDate defaultOpeningDate, final List<CodeValueData> stateOptions) {
        return new OfficeData(null, null, null, null, defaultOpeningDate, null, null, null, parentLookups,
                stateOptions, null);
    }

    public static OfficeData appendedTemplate(final OfficeData office, final Collection<OfficeData> allowedParents, final OfficeData templateData) {
        return new OfficeData(office.id, office.name, office.nameDecorated, office.externalId, office.openingDate, office.hierarchy,
                office.parentId, office.parentName, allowedParents, templateData.stateOptions, office.officeGsts);
    }

    public static OfficeData appendedTemplate(final OfficeData office, final Collection<OfficeData> allowedParents) {
        return new OfficeData(office.id, office.name, office.nameDecorated, office.externalId, office.openingDate, office.hierarchy,
                office.parentId, office.parentName, allowedParents, office.stateOptions, office.officeGsts);
    }
    ...
    public OfficeData(final Long id, final String name, final String nameDecorated, final String externalId, final LocalDate openingDate,
                      ...
                      final Collection<OfficeGstData> officeGsts) {
        this.id = id;
        this.name = name;
