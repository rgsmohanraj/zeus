...
import org.vcpl.lms.accounting.glaccount.domain.TrialBalance;
import org.vcpl.lms.accounting.glaccount.domain.TrialBalanceRepositoryWrapper;
...
import org.vcpl.lms.infrastructure.core.service.RoutingDataSourceServiceFactory;
...
import org.vcpl.lms.infrastructure.core.service.database.DatabaseTypeResolver;
...
import org.vcpl.lms.infrastructure.jobs.domain.ScheduledJobDetailRepository;
...
import org.vcpl.lms.infrastructure.jobs.service.JobRegisterService;
import org.vcpl.lms.portfolio.savings.DepositAccountType;
import org.vcpl.lms.portfolio.savings.DepositAccountUtils;
import org.vcpl.lms.portfolio.savings.data.DepositAccountData;
import org.vcpl.lms.portfolio.savings.data.SavingsAccountAnnualFeeData;
import org.vcpl.lms.portfolio.savings.service.DepositAccountReadPlatformService;
import org.vcpl.lms.portfolio.savings.service.DepositAccountWritePlatformService;
import org.vcpl.lms.portfolio.savings.service.SavingsAccountChargeReadPlatformService;
...
import org.vcpl.lms.portfolio.shareaccounts.service.ShareAccountSchedularService;
...
public class ScheduledJobRunnerServiceImpl implements ScheduledJobRunnerService {
    ...
    public ScheduledJobRunnerServiceImpl(final RoutingDataSourceServiceFactory dataSourceServiceFactory,
            ...
            final ScheduledJobDetailRepository scheduledJobDetailsRepository, final LMSProperties lmsProperties,
    ...
    public void updateLoanSummaryDetails() {
        ...
        final String selectPart = new StringBuilder().append(" (SELECT ml.id AS loan_id, ")
                ...
                .append("COALESCE((select SUM(lc.amount + lc.total_gst) from m_loan_charge lc where lc.loan_id = ml.id and lc.is_active = true")
                .append(" and lc.charge_time_enum=1),0) as fee_charges_charged_derived, ")
                .append("SUM(COALESCE(mr.fee_charges_completed_derived,0)) + ")
                .append("COALESCE((select SUM(lc.amount_paid_derived + lc.total_gst) from m_loan_charge lc where lc.loan_id = ml.id and lc.is_active = true")
    ...
    public void executeMissMatchedJobs() throws JobExecutionException {
        ...
        for (ScheduledJobDetail scheduledJobDetail : jobDetails) {
            if (scheduledJobDetail.getNodeId().toString().equals(lmsProperties.getNodeId())) {
