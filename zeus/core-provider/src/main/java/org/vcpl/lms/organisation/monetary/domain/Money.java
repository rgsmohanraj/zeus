 ...
 * under the License.
 */
package org.vcpl.lms.organisation.monetary.domain;
...
import org.vcpl.lms.portfolio.loanaccount.loanschedule.domain.RoundingLogicCalculation;
...
public class Money implements Comparable<Money> {
    ...
    public Money( MonetaryCurrency currency , BigDecimal totalPmtForThisInstallment,Integer decimalplaces,  RoundingMode roundingMode, Integer decimalPlacesHundreds) {

        this.currencyCode = currency.getCode();
        this.currencyDigitsAfterDecimal = currency.getDigitsAfterDecimal();
        this.inMultiplesOf = currency.getCurrencyInMultiplesOf();

        final BigDecimal amountZeroed = defaultToZeroIfNull(totalPmtForThisInstallment);
        ...
        this.amount =( Objects.nonNull(decimalPlacesHundreds) && decimalPlacesHundreds > 0)? RoundingLogicCalculation.roundCalculation(decimalPlacesHundreds,amountScaled,decimalplaces,roundingMode) : amountScaled.setScale(decimalplaces, roundingMode);
    }
    ...
    private Money(final String currencyCode, final int digitsAfterDecimal, final BigDecimal amount, final Integer inMultiplesOf, RoundingMode roundingMode) {
        this.currencyCode = currencyCode;
        this.currencyDigitsAfterDecimal = digitsAfterDecimal;
        this.inMultiplesOf = inMultiplesOf;
        ...
        this.amount = amountScaled.setScale(Integer.valueOf(0), roundingMode);
    }
    ...
    public static Money roundingOff(MonetaryCurrency currency, BigDecimal newAmount, RoundingMode rc) {

            return new Money(currency.getCode(), currency.getDigitsAfterDecimal(), defaultToZeroIfNull(newAmount),
                    currency.getCurrencyInMultiplesOf(),rc);
        }

    public static Money emiRound(MonetaryCurrency currency,BigDecimal totalPmtForThisInstallment,Integer decimalplaces,  RoundingMode roundingMode,Integer multiplesOf) {
        return  new Money(currency,totalPmtForThisInstallment,decimalplaces,roundingMode, multiplesOf );
    }
    ...
    public Money multiplyRetainScale(final BigDecimal valueToMultiplyBy, final RoundingMode roundingMode,final LoanProduct loanProduct) {
        if (valueToMultiplyBy.compareTo(BigDecimal.ONE) == 0) {
            return this;
        }
      //  BigDecimal newAmount = this.amount.multiply(valueToMultiplyBy);

        // Added to Avoid Any Round Off Issues
        BigDecimal newAmount = BigDecimal.valueOf(this.amount.doubleValue() * valueToMultiplyBy.doubleValue());
        BigDecimal value = BigDecimal.valueOf( Objects.nonNull(loanProduct.getInterestDecimalRegex()) ? RoundingLogicCalculation.decimalRoundingRegex(newAmount.doubleValue(),loanProduct.getInterestDecimalRegex()):newAmount.doubleValue());

        return Money.emiRound(monetaryCurrency(), value,loanProduct.getInterestDecimal(),loanProduct.getInterestRoundingMode(),null);
    }

    public Money multiplyRetainScale(final double valueToMultiplyBy, final RoundingMode roundingMode) {

        return this.multiplyRetainScale(BigDecimal.valueOf(valueToMultiplyBy), roundingMode,null);
    }
