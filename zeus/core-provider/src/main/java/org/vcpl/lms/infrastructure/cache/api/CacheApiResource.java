 ...
 * under the License.
 */
package org.vcpl.lms.infrastructure.cache.api;
...
import org.vcpl.lms.infrastructure.cache.data.CacheData;
import org.vcpl.lms.infrastructure.cache.service.CacheWritePlatformService;
import org.vcpl.lms.infrastructure.cache.service.RuntimeDelegatingCacheManager;
...
public class CacheApiResource {
    ...
    public CacheApiResource(final PlatformSecurityContext context,
                            ...
                            final PortfolioCommandSourceWritePlatformService commandsSourceWritePlatformService, CacheWritePlatformService cacheServiceIns, ApplicationContext applicationContext) {
    ...
    //This is used to remove the temporal storage in the bean class
    @GET
    @Path("clearbeancache")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "OK", content = @Content(array = @ArraySchema(schema = @Schema(implementation = CacheApiResourceSwagger.GetCachesResponse.class)))) })
    public ResponseEntity<String> clearAllBeanCache() {
        this.cacheServiceIns.clearBeanCache();
        return ResponseEntity.ok("Bean cache cleared successfully");
    }
