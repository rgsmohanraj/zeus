...
public class JdbcTenantDetailsService implements TenantDetailsService {
    ...
    // private static final class TenantMapper implements RowMapper<ZeusPlatformTenant> {
    ...
    //     @Override
    //     public ZeusPlatformTenant mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
    //         final Long id = rs.getLong("id");
    //         final String tenantIdentifier = rs.getString("identifier");
    //         final String name = rs.getString("name");
    //         final String timezoneId = rs.getString("timezoneId");
    //         final ZeusPlatformTenantConnection connection = getDBConnection(rs);

    //         return new ZeusPlatformTenant(id, tenantIdentifier, name, timezoneId, connection);
    //     }

    //     // gets the DB connection
    //     private ZeusPlatformTenantConnection getDBConnection(ResultSet rs) throws SQLException {
    ...
    //         return new ZeusPlatformTenantConnection(connectionId, schemaName, schemaServer, schemaServerPort,
    ...
    public ZeusPlatformTenant loadTenantById(final String tenantIdentifier) {
        try {
            return Optional.ofNullable(tenantPropertiesHandlerService.getInstance().getTenantPlatformDetails().get(tenantIdentifier)).orElseThrow();
    ...
    @Override
    public List<ZeusPlatformTenant> findAllTenants() {
        return Optional.of(tenantPropertiesHandlerService.getInstance().getTenantPlatformDetails().values()).map(ArrayList::new).orElse(new ArrayList<>());
    }
}
