...
import org.vcpl.lms.commands.domain.CommandSource;
import org.vcpl.lms.commands.domain.CommandSourceRepository;
import org.vcpl.lms.commands.domain.CommandWrapper;
import org.vcpl.lms.commands.exception.CommandNotAwaitingApprovalException;
import org.vcpl.lms.commands.exception.CommandNotFoundException;
import org.vcpl.lms.commands.exception.RollbackTransactionAsCommandIsNotApprovedByCheckerException;
...
import org.vcpl.lms.infrastructure.jobs.service.SchedulerJobRunnerReadService;
...
public class PortfolioCommandSourceWritePlatformServiceImpl implements PortfolioCommandSourceWritePlatformService {
    ...
    @Override
    public CommandProcessingResult logCommandSourceWithSystemUser(CommandWrapper wrapper) {
        final String json = wrapper.getJson();
        CommandProcessingResult result = null;
        JsonCommand command = null;
        int numberOfRetries = 0;
        ...
        while (numberOfRetries <= maxNumberOfRetries) {
            try {
                result = this.processAndLogCommandService.processAndLogCommand(wrapper, command, true);
                numberOfRetries = maxNumberOfRetries + 1;
            } catch (CannotAcquireLockException | ObjectOptimisticLockingFailureException exception) {
                log.info("The following command {} has been retried  {} time(s)", command.json(), numberOfRetries);
                /***
