...
export class AuthenticationService {
  ...
  login(loginContext: LoginContext) {
    ...
    const Eusername  = CryptoJS.AES.encrypt(loginContext.username,key, { iv: iv }).toString();
    const Epassword=CryptoJS.AES.encrypt(loginContext.password, key, { iv: iv }).toString();
    ...
    if (environment.oauth.enabled) {
      ...
      return this.http.post(`${environment.oauth.serverUrl}/v1/jwt/token`, {username: Eusername, password: Epassword})
        .pipe(
      ...
      return this.http.post('/authentication', { username: Eusername, password: Epassword })
        .pipe(
  ...
  private refreshOAuthAccessToken() {
    ...
    this.http.disableApiPrefix().post(`${environment.oauth.serverUrl}/v1/jwt/token/refresh`,  {refreshToken:oAuthRefreshToken })
  ...
  keycloaklogout() : Observable<boolean>
  {
     const oAuthRefreshToken = JSON.parse(this.storage.getItem(this.oAuthTokenDetailsStorageKey)).refresh_token;
    let httpParams = new HttpParams();
    httpParams = httpParams.set('refresh_token', oAuthRefreshToken);
    //this.http.disableApiPrefix().post(`${environment.oauth.serverUrl}/v1/jwt/token/refresh`,  { username: 'admin', password: 'password' })
    this.http.disableApiPrefix().post(`${environment.oauth.serverUrl}/v1/jwt/token/logout`,  {refreshToken:oAuthRefreshToken })
      .subscribe((tokenResponse: any) => {
        this.authenticationInterceptor.removeAuthorization();
        this.setCredentials();
  ...
  checkAccessPermission(action: string) {
    this.cacheMaintainer = JSON.stringify(this?.getCredentials()?.permissions ?? []);
    return /\"ALL_FUNCTIONS\"/.test(this.cacheMaintainer)
    || (this.cacheMaintainer.match('"'+action+'"')?.length ?? 0)> 0 ;
  }
  ...
  if(environment.oauth.enabled){
    ...
    this.notificationService.showError('Please login again','Access Denied')
