...
import org.vcpl.lms.commands.service.CommandProcessingService;
...
import org.vcpl.lms.portfolio.address.service.AddressWritePlatformService;
import org.vcpl.lms.portfolio.client.api.ClientApiConstants;
import org.vcpl.lms.portfolio.client.data.ClientDataValidator;
import org.vcpl.lms.portfolio.client.domain.AccountNumberGenerator;
import org.vcpl.lms.portfolio.client.domain.Client;
import org.vcpl.lms.portfolio.client.domain.ClientNonPerson;
import org.vcpl.lms.portfolio.client.domain.ClientNonPersonRepositoryWrapper;
...
import org.vcpl.lms.portfolio.client.exception.ClientActiveForUpdateException;
import org.vcpl.lms.portfolio.client.exception.ClientHasNoStaffException;
import org.vcpl.lms.portfolio.client.exception.ClientMustBePendingToBeDeletedException;
import org.vcpl.lms.portfolio.client.exception.InvalidClientSavingProductException;
import org.vcpl.lms.portfolio.client.exception.InvalidClientStateTransitionException;
...
import org.vcpl.lms.portfolio.group.exception.GroupMemberCountNotInPermissibleRangeException;
...
import org.vcpl.lms.portfolio.savings.data.SavingsAccountDataDTO;
import org.vcpl.lms.portfolio.savings.domain.SavingsAccount;
import org.vcpl.lms.portfolio.savings.domain.SavingsAccountRepositoryWrapper;
import org.vcpl.lms.portfolio.savings.domain.SavingsProductRepository;
import org.vcpl.lms.portfolio.savings.exception.SavingsProductNotFoundException;
import org.vcpl.lms.portfolio.savings.service.SavingsApplicationProcessWritePlatformService;
...
public class ClientWritePlatformServiceJpaRepositoryImpl implements ClientWritePlatformService {
    ...
    public CommandProcessingResult createClient(final JsonCommand command) {
        ...
        try {
            ...
            if (stateId != null) {
                state = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.STATE, stateId);
            }


//            CodeValue repaymentMode = null;
//            final Long repaymentModeId = command.longValueOfParameterNamed(ClientApiConstants.repaymentModeIdParamName);
//            if (repaymentModeId != null) {
//                repaymentMode = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.REPAYMENTMODE, repaymentModeId);
//            }
//
            ...
            final Long accountTypeId = command.longValueOfParameterNamed(ClientApiConstants.accountTypeIdParamName);
            if (accountTypeId != null) {
                accountType = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.ACCOUNTTYPE, accountTypeId);
            }

            final Client newClient = Client.createNew(currentUser, clientOffice, clientParentGroup, staff, savingsProductId, gender,state,
                    clientType, clientClassification, legalFormValue, command,accountType);
    ...
    public CommandProcessingResult updateClient(final Long clientId, final JsonCommand command) {
        ...
        try {
            ...
            if(changes.containsKey(ClientApiConstants.aadhaarParamName)){

                    throw new DataIntegrityViolationException("Aadhaar Modification is not allowed");
            ...
            if (changes.containsKey(ClientApiConstants.stateIdParamName)) {

                final Long newValue = command.longValueOfParameterNamed(ClientApiConstants.stateIdParamName);
                CodeValue state = null;
                if (newValue != null) {
                    state = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.STATE, newValue);
                }
                clientForUpdate.updateState(state);
            }

//            if (changes.containsKey(ClientApiConstants.repaymentModeIdParamName)) {
//
//                final Long newValue = command.longValueOfParameterNamed(ClientApiConstants.repaymentModeIdParamName);
//                CodeValue repaymentMode = null;
//                if (newValue != null) {
//                    repaymentMode = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.REPAYMENTMODE, newValue);
//                }
//                clientForUpdate.updateRepaymentMode(repaymentMode);
//            }
            ...
            if (changes.containsKey(ClientApiConstants.stateIdParamName)) {
                ...
                CodeValue newCodeVal = null;
                if (newValue != null) {
                    newCodeVal = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.STATE, newValue);
                }
                clientForUpdate.updateState(newCodeVal);
...
//                CodeValue newCodeVal = null;
//                if (newValue != null) {
//                    newCodeVal = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.REPAYMENTMODE, newValue);
//                }
//                clientForUpdate.updateRepaymentMode(newCodeVal);
//            }
            ...
            if (changes.containsKey(ClientApiConstants.accountTypeIdParamName)) {

                final Long newValue = command.longValueOfParameterNamed(ClientApiConstants.accountTypeIdParamName);
                CodeValue newCodeVal = null;
                if (newValue != null) {
                    newCodeVal = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.ACCOUNTTYPE, newValue);
                }
                clientForUpdate.updateAccountType(newCodeVal);
            }
            ...
            if (changes.containsKey(ClientApiConstants.accountTypeIdParamName)) {
                ...
                CodeValue accountType = null;
                if (newValue != null) {
                    accountType = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.ACCOUNTTYPE, newValue);
                }
                clientForUpdate.updateAccountType(accountType);
            }
    ...
    @Override
    public int updateAadhaar(String encryptedAadhaar, Long id) {
        String sql = "UPDATE m_client SET aadhaar=? WHERE id=?";
        int update=jdbcTemplate.update(sql, encryptedAadhaar, id);
        if(update>0){
            LOG.info("Migrated Aadhaar for id:{}", id);
        }
        return update;
    }
    @Transactional
    @Override
    public CommandProcessingResult updateClientAadhaar(Long clientId, JsonCommand command)  {

        this.context.authenticatedUser().validateHasPermissionTo("UPDATE_AADHAAR");
        boolean is_aadhaar_update = false;
        Client client=clientRepository.findOneWithNotFoundDetection(clientId);
        String AADHAAR = "aadhaar";
        fromApiJsonDeserializer.validateForAadhaar(command.stringValueOfParameterNamed(AADHAAR));
        if(!command.stringValueOfParameterNamed(AADHAAR).isEmpty()){
            is_aadhaar_update=true;
            String aadhaar=command.stringValueOfParameterNamed(AADHAAR).toUpperCase();
            client.setAadhaar(aadhaar);
        }
        if (is_aadhaar_update)
        {    this.clientRepository.save(client);    }
        LOG.info("Update aadhaar for client - {}", client.getId());
        assert client.getId() != null;
        return new CommandProcessingResultBuilder().withCommandId(command.commandId())
                .withClientId(clientId).withEntityId(client.getId()).with(Map.of("updated",client.getId())).build();
    }
}
