...
public class LoanChargeReadPlatformServiceImpl implements LoanChargeReadPlatformService {
    ...
    private static final class LoanChargeMapper implements RowMapper<LoanChargeData> {
        ...
        public String schema() {
            return "lc.id as id, lc.external_id as externalId, c.id as chargeId, c.name as name, " + "lc.amount as amountDue,lc.self_share_amount_derived as selfShareAmount,lc.partner_share_amount_derived as partnerShareAmount,lc.self_share_percentage as selfSharePercentage,lc.partner_share_percentage as partnerSharePercentage, "
                    ...
                    + "lc.charge_time_enum as chargeTime, " + "lc.is_penalty as penalty,lc.gst_enabled as gstEnabled,lc.gst_enum as gstEnum,lc.fees_charge_type_cv_id as feesChargeTypes,lc.cgst_amount as cgstAmount,lc.sgst_amount as sgstAmount, lc.igst_amount as igstAmount,"
                    + "lc.due_for_collection_as_of_date as dueAsOfDate, " + "lc.charge_calculation_enum as chargeCalculation,lc.self_gst_derived as selfGst,lc.partner_gst_derived as partnerGst,lc.total_gst as totalGst, "
        ...
        public LoanChargeData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            final BigDecimal amount = rs.getBigDecimal("amountDue");
             BigDecimal partnerShareAmount = rs.getBigDecimal("partnerShareAmount");
            if(partnerShareAmount == null)
            {
                partnerShareAmount=BigDecimal.valueOf(0);
            }
            ...
            final Boolean enableGst=rs.getBoolean("gstEnabled");
            final Integer gstEnum =rs.getInt("gstEnum");
            final String feesChargeTypes =rs.getString("feesChargeTypes");
            final BigDecimal selfSharePercentage =rs.getBigDecimal("selfSharePercentage");
            final BigDecimal partnersharePercentage = rs.getBigDecimal("partnerSharePercentage");
            final BigDecimal selfGst= rs.getBigDecimal("selfGst");
            final BigDecimal partnerGst= rs.getBigDecimal("partnerGst");
            final BigDecimal totalGst= rs.getBigDecimal("totalGst");
            BigDecimal selfShareAmount = rs.getBigDecimal("selfShareAmount");
            if(selfShareAmount == null)
            {
                selfShareAmount=BigDecimal.valueOf(0);
            }


             BigDecimal cgstAmount = rs.getBigDecimal("cgstAmount");
            if (cgstAmount == null) {
                cgstAmount=BigDecimal.valueOf(0);

            }
             BigDecimal sgstAmount = rs.getBigDecimal("sgstAmount");
            if(sgstAmount == null){
                sgstAmount=BigDecimal.valueOf(0);
            }

            BigDecimal igstAmount = rs.getBigDecimal("igstAmount");
            if(igstAmount == null){
            ...
            //final BigDecimal overAllGst=cgstAmount.add(sgstAmount).add(selfShareAmount);
            ...
            return new LoanChargeData(id, chargeId, name, currency, amount, amountPaid, amountWaived, amountWrittenOff, amountOutstanding,
                    ...
                    waived, null, minCap, maxCap, amountOrPercentage, null, externalId,enableGst,gstEnum,selfShareAmount,partnerShareAmount
                    ,cgstAmount,sgstAmount,igstAmount,feesChargeTypes,selfSharePercentage,partnersharePercentage,selfGst,partnerGst,totalGst,false,BigDecimal.ZERO,BigDecimal.ZERO,BigDecimal.ZERO,BigDecimal.ZERO,BigDecimal.ZERO,BigDecimal.ZERO,BigDecimal.ZERO);

        }
    }

    private static final class LoanChargeMapperForLoan implements RowMapper<LoanChargeData> {
        ...
        public String schema() {
            return "lc.id as id, lc.external_id as externalId, c.id as chargeId, c.name as name, " + "lc.amount as amountDue,lc.self_share_amount_derived as selfShareAmount,lc.partner_share_amount_derived as partnerShareAmount,lc.self_share_percentage as selfSharePercentage,lc.partner_share_percentage as partnerSharePercentage, "
                    ...
                    + "lc.charge_payment_mode_enum as chargePaymentMode, " + "lc.is_paid_derived as paid, " + "lc.waived as waied, "
                    + "lc.min_cap as minCap, lc.max_cap as maxCap, " + "lc.charge_amount_or_percentage as amountOrPercentage,lc.total_gst as totalGst,lc.self_gst_derived as selfGst , lc.partner_gst_derived as partnerGst,lc.gst_paid_derived as gstPaidDerived,lc.gst_outstanding_derived as gstOutstandingDerived,lc.gst_waivedoff_derived as gstWaivedOffDerived,"
                    +"lc.gst_paid_derived as gstPaidDerived,lc.self_gst_paid as selfGstPaid,lc.partner_gst_paid as partnerGstPaid,lc.self_share_amount_repaid as selfShareAmountRepaid,lc.partner_share_amount_repaid as partnerShareAmountRepaid,"
                    + "c.currency_code as currencyCode, oc.name as currencyName, "
                    + "date(coalesce(dd.disbursedon_date,dd.expected_disburse_date)) as disbursementDate, "
                    + "oc.decimal_places as currencyDecimalPlaces, oc.currency_multiplesof as inMultiplesOf, oc.display_symbol as currencyDisplaySymbol, "
                    + "oc.internationalized_name_code as currencyNameCode, c.charge_time_enum as chargeTimeEnum from m_charge c "
                    + "join m_organisation_currency oc on c.currency_code = oc.code " + "join m_loan_charge lc on lc.charge_id = c.id "
                    + "left join m_loan_tranche_disbursement_charge dc on dc.loan_charge_id=lc.id left join m_loan_disbursement_detail dd on dd.id=dc.disbursement_detail_id ";
        }
        ...
        public LoanChargeData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            final Long chargeId = rs.getLong("chargeId");
            final String name = rs.getString("name");
             BigDecimal amount = rs.getBigDecimal("amountDue");
             BigDecimal totalGst = rs.getBigDecimal("totalGst");
             if(totalGst == null){
            ...
            final int chargeCalculation = rs.getInt("chargeCalculation");
            final EnumOptionData chargeCalculationType = ChargeEnumerations.chargeCalculationType(chargeCalculation);
//            if(chargeCalculationType.getValue().equalsIgnoreCase(ChargeCalculationType.FLAT.toString())){
//                amount = amount.add(totalGst);
//            }
            amount = amount.add(totalGst);
            ...
            GstEnum gstValue = GstEnum.fromInt(gstEnum);
//            if(gstValue.toString().equals(LoanConstants.INCLUSIVE) && chargeCalculationType.getValue().equalsIgnoreCase(ChargeCalculationType.FLAT.toString()) ){
//                amount = amount.add(totalGst);
//            }
            ...
            final BigDecimal total = rs.getBigDecimal("totalGst");
            final BigDecimal selfGst = rs.getBigDecimal("selfGst");
            final BigDecimal partnerGst = rs.getBigDecimal("partnerGst");
            final  int chargeTimeEnum=rs.getInt("chargeTimeEnum");
            final boolean isBounce= ChargeTimeType.fromInt(chargeTimeEnum).isBounceCharge();

            final BigDecimal gstPaidDerived=rs.getBigDecimal("gstPaidDerived");
             BigDecimal totalPaid=BigDecimal.ZERO;
            if(isBounce || penalty)
                totalPaid= amountPaid.add(Objects.nonNull(gstPaidDerived) ? gstPaidDerived : BigDecimal.ZERO);
            else
                 totalPaid=amountPaid.add(totalGst);
            BigDecimal gstOutStanding=rs.getBigDecimal("gstOutstandingDerived");
            BigDecimal totalOutstanding=amountOutstanding.add(Objects.nonNull(gstOutStanding)?gstOutStanding:BigDecimal.ZERO);

            BigDecimal gstWaivedOffDerived=rs.getBigDecimal("gstWaivedOffDerived");
            BigDecimal totalWaivedOff=amountWaived.add(Objects.nonNull(gstWaivedOffDerived)?gstWaivedOffDerived:BigDecimal.ZERO);


            BigDecimal selfGstPaid=rs.getBigDecimal("selfGstPaid");
            BigDecimal partnerGstPaid=rs.getBigDecimal("partnerGstPaid");
            BigDecimal selfShareAmountRepaid=rs.getBigDecimal("selfShareAmountRepaid");
            BigDecimal partnerShareAmountRepaid=rs.getBigDecimal("partnerShareAmountRepaid");
            ...
            return new LoanChargeData(id, chargeId, name, currency, amount, amountPaid, amountWaived, amountWrittenOff, amountOutstanding,
                    ...
                    waived, null, minCap, maxCap, amountOrPercentage, null, externalId,enableGst,gstEnum,selfShareAmount,partnerShareAmount
                    ,cgstAmount,sgstAmount,igstAmount,feesChargeTypes,selfSharePercentage,partnersharePercentage,selfGst,partnerGst,totalGst,isBounce,totalPaid,totalOutstanding,totalWaivedOff,selfGstPaid,partnerGstPaid,selfShareAmountRepaid,partnerShareAmountRepaid);
    ...
    public ChargeData retrieveLoanChargeTemplate() {
        ...
        final Boolean enableGstCharge=false;
        ...
        return ChargeData.template(null, allowedChargeCalculationTypeOptions, null, allowedChargeTimeOptions, null,
                ...
                accountMappingForChargeConfig, expenseAccountOptions, assetAccountOptions,typeOption,enableGstCharge,gstOption,feesOption, null,
                isGstSlabEnabled,gstSlabLimitApplyForOption,gstSlabLimitOperatorOption,null,null);
    }
    ...
    @Override
    public Collection<LoanChargeData> retrieveLoanChargesForSpecificLoan(Long loanId) {

        this.context.authenticatedUser();

        final LoanChargeMapperForLoan rm = new LoanChargeMapperForLoan();

        final String sql = "select " + rm.schema() + " where lc.loan_id=? AND lc.is_active = true and lc.is_penalty = false And lc.amount_paid_derived IS NOT NULL AND lc.amount_paid_derived !=0 AND lc.cooling_off_reversed = false"
                + " order by coalesce(lc.due_for_collection_as_of_date,date(coalesce(dd.disbursedon_date,dd.expected_disburse_date))),lc.charge_time_enum ASC, " +
                " lc.due_for_collection_as_of_date ASC";
        return this.jdbcTemplate.query(sql, rm, new Object[] { loanId }); // NOSONAR
    ...
    @Override
    public Collection<DisbursementSummary> retrieveDisburesemntSummary(Collection<LoanChargeData> loanChargeData) {
        final Collection<DisbursementSummary> disbursementSummaries =new ArrayList<>();
        BigDecimal total= BigDecimal.valueOf(0);
        BigDecimal overAllTotal= BigDecimal.valueOf(0);
        for(LoanChargeData loanChargeDatas : loanChargeData )
        {
            final String chargeName =loanChargeDatas.getName();
            final BigDecimal ProcessingFee=loanChargeDatas.getAmount();
            final BigDecimal selfProcessingFee=loanChargeDatas.getSelfShareAmount();
            final BigDecimal PartnerProcessingFee=loanChargeDatas.getPartnerShareAmount();
            final BigDecimal cgstAmount =loanChargeDatas.GetCgstAmount();
            final BigDecimal sgstAmount=loanChargeDatas.getSgstAmount();
            final BigDecimal igstAmount=loanChargeDatas.getIgstAmount();
            final BigDecimal selfGst=loanChargeDatas.getSelfGst();
            final BigDecimal partnerGst =loanChargeDatas.getPartnerGst();
            final BigDecimal totalGst = loanChargeDatas.getTotalGst();
           total=ProcessingFee.add(cgstAmount).add(sgstAmount).add(igstAmount);
           // total = BigDecimal.valueOf(ProcessingFee.doubleValue() + cgstAmount.doubleValue() + sgstAmount.doubleValue() + igstAmount.doubleValue()).setScale(2, RoundingMode.UP);
            overAllTotal=overAllTotal.add(total);
            disbursementSummaries.add(new DisbursementSummary(chargeName,ProcessingFee,selfProcessingFee,PartnerProcessingFee,cgstAmount,sgstAmount,igstAmount,total,overAllTotal,selfGst,partnerGst,totalGst));
        }

        return disbursementSummaries;
    ...
    @Override
    public Collection<LoanChargeData> retrieveDisbursementSummaryCharge(Long loanId) {

        this.context.authenticatedUser();

        final LoanChargeMapper rm = new LoanChargeMapper();

        final String sql = "select " + rm.schema() + " where lc.loan_id= ? AND lc.is_active = true AND lc.cooling_off_reversed = false AND lc.charge_time_enum = 1"
                + " order by coalesce(lc.due_for_collection_as_of_date,date(coalesce(dd.disbursedon_date,dd.expected_disburse_date))),lc.charge_time_enum ASC, lc.due_for_collection_as_of_date ASC, lc.is_penalty ASC";
