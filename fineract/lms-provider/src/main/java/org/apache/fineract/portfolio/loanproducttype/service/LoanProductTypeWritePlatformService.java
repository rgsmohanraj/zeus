...
public interface LoanProductTypeWritePlatformService {
    @Transactional
    ...
    CommandProcessingResult createType(JsonCommand command);

    @Transactional
    @CacheEvict(value = "types", key = "T(org.apache.fineract.infrastructure.core.service.ThreadLocalContextUtil).getTenant().getTenantIdentifier().concat('ty')")
    CommandProcessingResult updateType(Long typeId, JsonCommand command);
}
