...
import org.vcpl.lms.infrastructure.core.exception.PlatformInternalServerException;
import org.vcpl.lms.infrastructure.core.service.ThreadLocalContextUtil;
import org.vcpl.lms.infrastructure.jobs.annotation.CronMethodParser;
import org.vcpl.lms.infrastructure.jobs.annotation.CronMethodParser.ClassMethodNamesPair;
import org.vcpl.lms.infrastructure.jobs.domain.JobParameter;
import org.vcpl.lms.infrastructure.jobs.domain.JobParameterRepository;
import org.vcpl.lms.infrastructure.jobs.domain.ScheduledJobDetail;
import org.vcpl.lms.infrastructure.jobs.domain.SchedulerDetail;
import org.vcpl.lms.infrastructure.jobs.exception.JobNodeIdMismatchingException;
import org.vcpl.lms.infrastructure.jobs.exception.JobNotFoundException;
...
//    @PostConstruct
    public void loadAllJobs() {
        final List<ZeusPlatformTenant> allTenants = this.tenantDetailsService.findAllTenants();
        for (final ZeusPlatformTenant tenant : allTenants) {
            if(Objects.isNull(tenant))
                continue;
            ThreadLocalContextUtil.setTenant(tenant);
            final List<ScheduledJobDetail> scheduledJobDetails = this.schedularWritePlatformService
                    .retrieveAllJobs(lmsProperties.getNodeId());
    ...
    @PostConstruct
    public void loadJobsForChaining() throws Exception {
        final List<ZeusPlatformTenant> allTenants = this.tenantDetailsService.findAllTenants();
        for (final ZeusPlatformTenant tenant : allTenants) {
            if(Objects.isNull(tenant))
                continue;
            ...
            List<ScheduledJobDetail> scheduledJobDetails = this.schedularWritePlatformService
                    .retrieveAllJobs(lmsProperties.getNodeId());
            Map<Short, List<ScheduledJobDetail>> map = scheduledJobDetails.stream()
                    .collect(Collectors.groupingBy(ScheduledJobDetail::getSchedulerGroup));
            for (Short key : map.keySet()) {
                List<ScheduledJobDetail>scheduledJobDetailList=map.get(key);
                if (scheduledJobDetailList.size()==1) {
                    //Individual Jobs scheduled here.
                    scheduledJobDetailList.forEach(this::scheduleJob);
                    scheduledJobDetailList.get(0).updateTriggerMisfired(false);
                    scheduledJobDetailList.get(0).updateCurrentlyRunningStatus(false);
                    this.schedularWritePlatformService.saveOrUpdate( scheduledJobDetailList.get(0));
                } else if(map.get(key).size()>=2) {
                        //Chaining Jobs scheduled here according to their priority.
                        Comparator<ScheduledJobDetail> priorityComparator = Comparator.comparing(ScheduledJobDetail::getTaskPriority);
                        map.get(key).sort(priorityComparator);
                        scheduledJobDetails = map.get(key);
                        JobChainingJobListener jobListener = new JobChainingJobListener(key + " ChainListener");
                        Scheduler scheduler = this.createScheduler(key + "  Scheduler", 1, schedulerJobListener);
                        final JobDataMap jobDataMap = new JobDataMap();
                        jobDataMap.put(SchedulerServiceConstants.TENANT_IDENTIFIER, tenant.getTenantIdentifier());
                        //Setting trigger for first job in chaining progress.
                        Trigger jobTrigger = TriggerBuilder.newTrigger()
                                .withIdentity(key + " MyTrigger", key + "JobGroup")
                                .withSchedule(CronScheduleBuilder.cronSchedule(scheduledJobDetails.get(0).getCronExpression())).usingJobData(jobDataMap).build();
                        scheduler.scheduleJob(createJobDetail(scheduledJobDetails.get(0)), jobTrigger);
                        scheduledJobDetails.get(0).updateNextRunTime(jobTrigger.getNextFireTime());
                        scheduledJobDetails.get(0).updateTriggerMisfired(false);
                        scheduledJobDetails.get(0).updateCurrentlyRunningStatus(false);
                        this.schedularWritePlatformService.saveOrUpdate(scheduledJobDetails.get(0));
                        // chaining job here
                        for (int i = 1; i < scheduledJobDetails.size(); i++) {
                            JobDetail prevJobdetail = getJobDetail(scheduledJobDetails.get(i - 1));
                            JobDetail currentJobDetail = getJobDetail(scheduledJobDetails.get(i));
                            jobListener = jobChaining(jobListener, prevJobdetail, currentJobDetail);
                            scheduler = addJobScheduler(scheduler, createJobDetail(scheduledJobDetails.get(i)));
                            scheduledJobDetails.get(i).updateNextRunTime(jobTrigger.getNextFireTime());
                            scheduledJobDetails.get(i).updateTriggerMisfired(false);
                            scheduledJobDetails.get(i).updateCurrentlyRunningStatus(false);
                            this.schedularWritePlatformService.saveOrUpdate(scheduledJobDetails.get(i));
                        }
    ...
    public void rescheduleJob(final Long jobId) {
        ...
        if (nodeIdStored.equals(lmsProperties.getNodeId()) || nodeIdStored.equals("0")) {
            rescheduleJob(scheduledJobDetail);
        } else {
            scheduledJobDetail.setIsMismatchedJob(true);
            this.schedularWritePlatformService.saveOrUpdate(scheduledJobDetail);
            throw new JobNodeIdMismatchingException(nodeIdStored, lmsProperties.getNodeId());
        }
    }
    ...
    private JobDetail getJobDetail(final ScheduledJobDetail scheduledJobDetails) throws Exception {
        final JobDetail jobDetail = createJobDetail(scheduledJobDetails);
        scheduledJobDetails.updateCurrentlyRunningStatus(false);
        return jobDetail;
    }
    public JobChainingJobListener jobChaining(JobChainingJobListener jobChainingJobListener, JobDetail jobDetail, JobDetail jobDetail2)
    {
        jobChainingJobListener.addJobChainLink(jobDetail.getKey(),jobDetail2.getKey());
        return jobChainingJobListener;
    }

    public Scheduler addJobScheduler(Scheduler scheduler,JobDetail jobDetail) throws SchedulerException {
