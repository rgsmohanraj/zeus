...
import org.vcpl.lms.portfolio.accountdetails.data.AccountSummaryCollectionData;
...
import org.vcpl.lms.portfolio.loanaccount.guarantor.data.ObligeeData;
...
public class ClientsApiResource {
    ...
    public String retrieveAll(final UriInfo uriInfo, final String sqlSearch, final Long officeId, final String externalId,
        ...
        final List<ClientData> clients = this.clientReadPlatformService.retrieveAllForLookup();
        final Page<ClientData> clientData = new Page<>(clients, clients.size());
    ...
    @Operation(summary = "Create a Client", description = "Note:\n\n"
            ...
            + "Mandatory Fields: firstname and lastname OR fullname, officeId, active=true and activationDate OR active=false, Entity,Applicant type,date of birth,age,mobileno,"
            +  "aadhaar,genderId,stateId,city,address,pincode,submittedOnDate,activationDate,beneficiaryAccountNo,ifsccode,accounttypeid\n\n"

            + "Optional Fields: middlename externalId, accountNo, staffId, clientTypeId, clientClassificationId,"
            + "beneficiaryname,micrcode,swiftcode,branch,pan,voterid,passportnumber,drivingLicense,rationcard,emailaddress\n\n")
    ...
    @PUT
    @Path("{clientId}/aadhaar")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.TEXT_PLAIN)
    @RequestBody(required = true, content = @Content(schema = @Schema(implementation = ClientsApiResourceSwagger.PostAaadhaarData.class)))
    @Operation(summary = "updating client aadhaar",description = "Updating Client Aadhaar based on their ClientId")
    public Response updateAadhaar(@PathParam("clientId") final Long clientId
                                 ,@Parameter(hidden = true) final String apiRequestBodyAsJson)  {

         this.commandsSourceWritePlatformService.logCommandSource(new CommandWrapperBuilder()
                                                                     .updateClientAadhaar(clientId)
                                                                     .withJson(apiRequestBodyAsJson)
                                                                     .build());
