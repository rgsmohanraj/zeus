...
public final class CalculateLoanScheduleQueryFromApiJsonHelper {
    ...
    final Set<String> supportedParameters = new HashSet<>(Arrays.asList(LoanApiConstants.idParameterName,
            ...
            LoanApiConstants.selfShare,LoanApiConstants.partnerShare,LoanApiConstants.foreclosureChargesParameterName,
            LoanApiConstants.overduechargeParameterName,LoanApiConstants.bounceChargeParameterName));
            ...
            if (loanTermFrequency != null && repaymentEvery != null && numberOfRepayments != null) {
                ...
                if (loanTermFrequency.intValue() < suggestsedLoanTerm) {
                    final ApiParameterError error = ApiParameterError.parameterError(
                            ...
                            "The parameter loan Term is less than the suggest loan term as indicated by numberOfRepayments and repaymentEvery.",
                            "loan Term", loanTermFrequency, numberOfRepayments, repaymentEvery);
                    dataValidationErrors.add(error);
                } else {
                    if (loanTermFrequency.intValue() > suggestsedLoanTerm) {
                        final ApiParameterError error = ApiParameterError.parameterError(
                                "validation.msg.loan.loanTermFrequency.greater.than.repayment.structure.suggests",
                                "The parameter loan Term is greater than the suggested loan term as indicated by numberOfRepayments and repaymentEvery.",
                                "loan Term", loanTermFrequency, numberOfRepayments, repaymentEvery);
                        dataValidationErrors.add(error);
                    }
