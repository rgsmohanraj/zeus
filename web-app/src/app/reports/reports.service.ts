...
export class ReportsService {
  ...
  getRunReportPdfData(reportName: string, formData: object): Observable<any> {
     let httpParams = new HttpParams();
     for (const [key, value] of Object.entries(formData)) {
             httpParams = httpParams.set(key, value);
           }
           httpParams = httpParams.set('exportPDF', true);
     return this.http.get(`/runreports/download/${reportName}`, {params: httpParams, responseType: 'blob' });
   }
  /**
  ...
  getRunReportCSVData(reportName: string, formData: object): Observable<any> {
    let httpParams = new HttpParams();
    for (const [key, value] of Object.entries(formData)) {
            httpParams = httpParams.set(key, value);
          }
          httpParams = httpParams.set('exportCSV', true);
    return this.http.get(`/runreports/${reportName}`, {
      responseType: 'arraybuffer', 
      observe: 'response', 
      params: httpParams
