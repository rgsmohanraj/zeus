 ...
 * under the License.
 */
package org.vcpl.lms.infrastructure.configuration.service;

import java.sql.ResultSet;
import java.sql.SQLException;
import java.util.Collection;
import org.vcpl.lms.infrastructure.campaigns.sms.data.MessageGatewayConfigurationData;
import org.vcpl.lms.infrastructure.configuration.data.ExternalServicesPropertiesData;
import org.vcpl.lms.infrastructure.configuration.data.S3CredentialsData;
import org.vcpl.lms.infrastructure.configuration.data.SMTPCredentialsData;
import org.vcpl.lms.infrastructure.configuration.exception.ExternalServiceConfigurationNotFoundException;
    ...
    private static final class SMTPCredentialsDataExtractor implements ResultSetExtractor<SMTPCredentialsData> {
        ...
        public SMTPCredentialsData extractData(final ResultSet rs) throws SQLException, DataAccessException {
            ...
            while (rs.next()) {
                ...
                else if (ExternalServicesConstants.SMTP_TO_EMAIL.equalsIgnoreCase(name)) {
                    toMail = value;
                }
            }
            return new SMTPCredentialsData(username, password, host, port, useTLS, fromEmail, fromName, toMail);
        }
    }
