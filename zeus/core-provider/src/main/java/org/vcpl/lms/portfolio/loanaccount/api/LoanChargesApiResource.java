...
import org.vcpl.lms.portfolio.loanaccount.data.LoanInstallmentChargeData;
import org.vcpl.lms.portfolio.loanaccount.service.LoanChargeReadPlatformService;
...
public class LoanChargesApiResource {
    ...
    public String executeLoanCharge(@PathParam("loanId") @Parameter(description = "loanId") final Long loanId,
        ...
        if (is(commandParam, "pay")) {
            ...
            final CommandWrapper commandRequest = new CommandWrapperBuilder().waiveLoanCharge(loanId, null).withJson(apiRequestBodyAsJson)
                    .build();
            result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
        } else {
