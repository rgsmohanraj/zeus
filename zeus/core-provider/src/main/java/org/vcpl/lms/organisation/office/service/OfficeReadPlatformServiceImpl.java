...
public class OfficeReadPlatformServiceImpl implements OfficeReadPlatformService {
    ...
    private static final class OfficeMapper implements RowMapper<OfficeData> {
        ...
        private final Collection<OfficeGstData> officeGsts;

        OfficeMapper(final Collection<OfficeGstData> officeGsts) {
            this.officeGsts = officeGsts;
        }
        ...
        public OfficeData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            final List<OfficeGstData> officeGstData = new ArrayList<>();
            if (this.officeGsts != null) {
                for (final OfficeGstData officeGstDatas : this.officeGsts) {
                    officeGstData.add(officeGstDatas);
                }
            }

            return new OfficeData(id, name, nameDecorated, externalId, openingDate, hierarchy, parentId, parentName, null,
                    stateOptions, officeGstData);
        }
    }
    ...
    @Override
    @Cacheable(value = "offices", key = "T(org.vcpl.lms.infrastructure.core.service.ThreadLocalContextUtil).getTenant().getTenantIdentifier().concat(#root.target.context.authenticatedUser().getOffice().getHierarchy()+'of')")
    public Collection<OfficeData> retrieveAllOffices(final boolean includeAllOffices, final SearchParameters searchParameters) {
        ...
        final OfficeMapper rm = new OfficeMapper(null);
    ...
    @Override
    @Cacheable(value = "officesForDropdown", key = "T(org.vcpl.lms.infrastructure.core.service.ThreadLocalContextUtil).getTenant().getTenantIdentifier().concat(#root.target.context.authenticatedUser().getOffice().getHierarchy()+'ofd')")
    ...
    @Override
    @Cacheable(value = "officesById", key = "T(org.vcpl.lms.infrastructure.core.service.ThreadLocalContextUtil).getTenant().getTenantIdentifier().concat(#officeId)")
    public OfficeData retrieveOffice(final Long officeId) {

        try {
            this.context.authenticatedUser();

            final Collection<OfficeGstData> officeGsts = retrieveOfficeGsts(officeId);

            final OfficeMapper rm = new OfficeMapper(officeGsts);
    ...
    public OfficeData retrieveNewOfficeTemplate() {
        ...
        return OfficeData.template(null, LocalDate.now(DateUtils.getDateTimeZoneOfTenant()), stateOptions);
    }
    ...
    @Override
    public Collection<OfficeGstData> retrieveOfficeGsts(final Long officeId) {
        final OfficeGstMapper rm = new OfficeGstMapper();
        final String sql = "select " + rm.schema() + " where g.office_id = ? ";
        return this.jdbcTemplate.query(sql, rm, new Object[] { officeId });
    }

    private static final class OfficeGstMapper implements RowMapper<OfficeGstData> {

        public String schema() {
        return "g.id as id, g.gst_number as gstNumber, g.state_cv_id as stateId, cv.code_value as stateValue, "
                + " g.cgst as cgst, g.sgst as sgst, g.igst as igst "
                + " from m_office_gst_details g LEFT JOIN m_office o on g.office_id = o.id "
                + " LEFT JOIN m_code_value cv ON g.state_cv_id = cv.id ";
        }

        @Override
        public OfficeGstData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            final BigDecimal cgst = rs.getBigDecimal("cgst");
            final BigDecimal sgst = rs.getBigDecimal("sgst");
            final BigDecimal igst = rs.getBigDecimal("igst");

            final OfficeGstData officeGstData = new OfficeGstData(id, gstNumber, state, cgst, sgst, igst);
            return officeGstData;
        }
    }
