 ...
 * under the License.
 */
package org.vcpl.lms.portfolio.self.runreport;
...
@Component
@Scope("singleton")
@Tag(name = "Self Run Report", description = "This resource allows you to run and receive output from pre-defined Zeus reports.\n"
        + "\n"
        ...
        + "\n"
        + "The Zeus reference application uses a JQuery plugin called stretchyreporting which, itself, uses this reports resource to provide a pretty flexible reporting User Interface (UI).\n"
        + "\n" + "ARGUMENTS\n"
...
public class SelfRunReportApiResource {
    ...
    public Response runReport(@PathParam("reportName") @Parameter(description = "reportName") final String reportName,
            @Context final UriInfo uriInfo) throws JRException, SQLException, IOException {
