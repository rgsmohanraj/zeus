 ...
 * under the License.
 */
package org.vcpl.lms.infrastructure.cache.service;
...
public class CacheWritePlatformServiceJpaRepositoryImpl implements CacheWritePlatformService {
    ...
    public CacheWritePlatformServiceJpaRepositoryImpl(final ConfigurationDomainService configurationDomainService,
                                                      @Qualifier("runtimeDelegatingCacheManager") final RuntimeDelegatingCacheManager cacheService, ApplicationContext applicationContext) {
    ...
    public void clearBeanCache() {
        BulkLoanUploadRequestMapper beanByType = applicationContext.getBean(BulkLoanUploadRequestMapper.class);
        Optional.of(beanByType).ifPresent(BulkLoanUploadRequestMapper::resetTheCatchToDefaultValue);
    }
}
