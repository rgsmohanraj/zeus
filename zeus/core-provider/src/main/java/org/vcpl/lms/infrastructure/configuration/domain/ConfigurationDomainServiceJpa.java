...
import org.vcpl.lms.infrastructure.cache.domain.PlatformCache;
import org.vcpl.lms.infrastructure.cache.domain.PlatformCacheRepository;
...
public class ConfigurationDomainServiceJpa implements ConfigurationDomainService {
     ...
     * (non-Javadoc)
     *
     * @see org.vcpl.lms.infrastructure.configuration.domain. ConfigurationDomainService#isHolidaysEnabled()
     */
    @Override
    ...
    public int getRoundingMode() {
        ...
        int defaultValue = 2; // 2 Stands for HALF-UP
        final GlobalConfigurationPropertyData property = getGlobalConfigurationPropertyData(propertyName);
        if (property.isEnabled()) {
    ...
    @Cacheable(value = "configByName", key = "T(org.vcpl.lms.infrastructure.core.service.ThreadLocalContextUtil).getTenant().getTenantIdentifier().concat(#propertyName)")
