...
public class CodeValueReadPlatformServiceImpl implements CodeValueReadPlatformService {
    ...
    @Override
    @Cacheable(value = "code_values", key = "T(org.vcpl.lms.infrastructure.core.service.ThreadLocalContextUtil).getTenant().getTenantIdentifier().concat(#codeId+'cv')")
    ...
    @Override
    public String getCodeValueById(Long codeValueId) {
        String getCodeValue = "select code_value from m_code_value where id = ? ";
        return this.jdbcTemplate.queryForObject(getCodeValue, String.class, new Object[] {codeValueId});
    }
}
