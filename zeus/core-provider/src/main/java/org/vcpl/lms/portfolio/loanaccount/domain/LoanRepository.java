...
public interface LoanRepository extends JpaRepository<Loan, Long>, JpaSpecificationExecutor<Loan> {
    ...
    String FIND_BY_EXTERNAL_ID = "SELECT count(l) FROM Loan l WHERE l.externalId = :externalId ";
    ...
    Integer findLoanAccountByExternalId(@Param("externalId") String externalId);
    Optional<Loan> getByIdAndExternalId(Long loanId, String externalId);

    Optional<Loan> getByExternalIdAndPartnerId(String externalId, Long partnerId);

    Optional<Loan> getByAccountNumber(String accountNumber);


    @Query(value = "select * from m_loan l where l.loan_status_id = 300 " +
            " and id in(SELECT distinct loan_id FROM m_loan_repayment_schedule " +
            " where duedate < current_date() and completed_derived = false)",nativeQuery = true)
    List<Loan> findLoanByLoanStatus();
