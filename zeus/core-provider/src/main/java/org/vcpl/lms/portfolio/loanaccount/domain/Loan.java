...
import org.vcpl.lms.infrastructure.core.serialization.JsonParserHelper;
import org.vcpl.lms.infrastructure.core.service.DateUtils;
import org.vcpl.lms.infrastructure.security.service.RandomPasswordGenerator;
import org.vcpl.lms.organisation.holiday.domain.Holiday;
import org.vcpl.lms.organisation.holiday.service.HolidayUtil;
...
import org.vcpl.lms.organisation.workingdays.service.WorkingDaysUtil;
...
import org.vcpl.lms.portfolio.calendar.domain.CalendarHistory;
import org.vcpl.lms.portfolio.calendar.domain.CalendarInstance;
import org.vcpl.lms.portfolio.calendar.domain.CalendarWeekDaysType;
...
import org.vcpl.lms.portfolio.collection.service.CollectionService;
...
import org.vcpl.lms.portfolio.loanaccount.data.LoanCollateralManagementData;
import org.vcpl.lms.portfolio.loanaccount.data.LoanTermVariationsData;
...
import org.vcpl.lms.portfolio.loanaccount.exception.InvalidRefundDateException;
import org.vcpl.lms.portfolio.loanaccount.exception.LoanApplicationDateException;
import org.vcpl.lms.portfolio.loanaccount.exception.LoanDisbursalException;
import org.vcpl.lms.portfolio.loanaccount.exception.LoanForeclosureException;
import org.vcpl.lms.portfolio.loanaccount.exception.LoanOfficerAssignmentDateException;
import org.vcpl.lms.portfolio.loanaccount.exception.LoanOfficerAssignmentException;
import org.vcpl.lms.portfolio.loanaccount.exception.LoanOfficerUnassignmentDateException;
...
import org.vcpl.lms.portfolio.loanaccount.exception.UndoLastTrancheDisbursementException;
...
import org.vcpl.lms.portfolio.loanaccount.loanschedule.domain.LoanScheduleGenerator;
...
public class Loan extends AbstractPersistableCustom {
    ...
    @Column(name = "account_no", length = 25, unique = true, nullable = false)
    ...
    @Column(name = "broken_Strategy", length = 20, unique = true, nullable = false)
    private String brokenStrategy;

    @Column(name = "broken_interest_repaid", scale = 6, precision = 19)
    private BigDecimal brokenInterestRepaid;

    @Column(name = "self_principal_amount", scale = 6, precision = 19, nullable = false)
    private BigDecimal selfPrincipaAmount;

    @Column(name = "broken_interest_derived", scale = 6, precision = 19)
    private BigDecimal brokenInterestDerived;
    ...
    @Column(name = "xirr_value", scale = 2, precision = 19, nullable = false)
    private BigDecimal xirrValue;

    @Column(name = "self_annual_nominal_interest_rate", scale = 6, precision = 19, nullable = false)
    private BigDecimal selfAnnualNominalInterestRate;

    @Column(name = "partner_annual_nominal_interest_rate", scale = 6, precision = 19, nullable = false)
    private BigDecimal partnerAnnualNominalInterestRate;

    @OneToMany(cascade = CascadeType.ALL, mappedBy = "loan", orphanRemoval = true, fetch = FetchType.LAZY)
    private Set<LoanPenalForeclosureCharges> loanPenalForeclosueCharges = new HashSet<>();
    ...
    @OneToMany(cascade = CascadeType.ALL, mappedBy = "loan", orphanRemoval = true, fetch = FetchType.LAZY)
    private List<XirrHistoryDetails> xirrHistoryDetails = new ArrayList<>();
    ...
    public BigDecimal getVclHurdleRate() {
        return vclHurdleRate;
    }
    ...
    public void setBrokenInterestRepaid(BigDecimal brokenInterestRepaid) {
        this.brokenInterestRepaid = brokenInterestRepaid;
    }

    public void setBrokenInterestDerived(BigDecimal brokenInterestDerived) {
        this.brokenInterestDerived = brokenInterestDerived;
    }
    ...
    public static Loan newIndividualLoanApplication(final String accountNo, final Client client, final Integer loanType,
                                                    ...
                                                    final LoanProductRelatedDetail loanRepaymentScheduleDetail, final List<LoanCharge> loanCharges,
            ...
            , final Long partnerId, final BigDecimal selfPrincipalAmount, final BigDecimal partnerPrincipalAmount, final String brokenStrategyType) {
        ...
        return new Loan(accountNo, client, group, loanType, fund, officer, loanPurpose, transactionProcessingStrategy, loanProduct,
                ...
                interestRateDifferential, rates, fixedPrincipalPercentagePerInstallment, partnerId, selfPrincipalAmount, partnerPrincipalAmount, brokenStrategyType);
    }
    ...
    public static Loan newGroupLoanApplication(final String accountNo, final Group group, final Integer loanType,
                                               ...
                                               final List<LoanDisbursementDetails> disbursementDetails, final BigDecimal maxOutstandingLoanBalance,
                                               final Boolean createStandingInstructionAtDisbursement, final Boolean isFloatingInterestRate,
                                               final BigDecimal interestRateDifferential, final List<Rate> rates, final BigDecimal fixedPrincipalPercentagePerInstallment, final Long partnerId, final BigDecimal selfPrincipalAmount, final BigDecimal partnerPrincipalAmount, final String brokenStrategyType) {
        ...
        return new Loan(accountNo, client, group, loanType, fund, officer, loanPurpose, transactionProcessingStrategy, loanProduct,
                loanRepaymentScheduleDetail, status, loanCharges, collateral, syncDisbursementWithMeeting, fixedEmiAmount,
                disbursementDetails, maxOutstandingLoanBalance, createStandingInstructionAtDisbursement, isFloatingInterestRate,
                interestRateDifferential, rates, fixedPrincipalPercentagePerInstallment, partnerId,
    ...
    public static Loan newIndividualLoanApplicationFromGroup(final String accountNo, final Client client, final Group group,
                                                             ...
                                                             final List<LoanDisbursementDetails> disbursementDetails, final BigDecimal maxOutstandingLoanBalance,
                                                             final Boolean createStandingInstructionAtDisbursement, final Boolean isFloatingInterestRate,
                                                             final BigDecimal interestRateDifferential, final List<Rate> rates, final BigDecimal fixedPrincipalPercentagePerInstallment, final Long partnerId, final BigDecimal selfPrincipalAmount,
                                                             final BigDecimal partnerPrincipalAmount, final String brokenStrategyType) {
    ...
    private Loan(final String accountNo, final Client client, final Group group, final Integer loanType, final Fund fund,
                 ...
                 final List<LoanCharge> loanCharges, final Set<LoanCollateralManagement> collateral, final Boolean syncDisbursementWithMeeting,
                 ...
                 final BigDecimal fixedPrincipalPercentagePerInstallment, final Long partnerId, final BigDecimal selfPrincipalAmount,
                 final BigDecimal partnerPrincipalAmount, final String brokenStrategyType) {

        this.selfPrincipaAmount = selfPrincipalAmount;
        this.partnerPrincipalAmount = partnerPrincipalAmount;

        this.brokenStrategy = brokenStrategyType;
        this.partnerId = partnerId;
        ...
        if (loanCharges != null && !loanCharges.isEmpty()) {
            ...
            this.summary = updateSummaryWithTotalFeeChargesDueAtDisbursement(deriveSumTotalOfChargesDueAtDisbursement(), deriveSumTotalOfSelfChargesDueAtDisbursement(),
                    deriveSumTotalOfPartnerChargesDueAtDisbursement());
        } else {
            this.charges = null;
        ...
        this.roundingMode = loanProduct.getRoundingMode().toString();

        this.createdDate = DateUtils.getDateOfTenant();

        // Add net get net disbursal amount from charges and principal
//        this.netDisbursalAmount = this.approvedPrincipal.subtract(deriveSumTotalOfChargesDueAtDisbursement()).subtract(deriveCgstAmount()).subtract(deriveIgstAmount()).subtract(derivesgstAmount());
        this.netDisbursalAmount = this.approvedPrincipal.subtract((deriveSumTotalOfChargesDueAtDisbursement())).subtract(deriveTotalGstAtDisbursement(loanCharges));
        if ((this.netDisbursalAmount.doubleValue() < 0) || (this.netDisbursalAmount.doubleValue() == 0)) {
            throw new InvalidLoanStateTransitionException("submittal", "netDisbursalAmount.cannot.be.negative",
                    "Total Charge Amount should not be greater than the Loan Amount");
        }
        this.netSelfDisbursalAmount = this.selfPrincipaAmount.subtract(deriveSumOfSelfTotalOfChargesDueAtDisbursement()).subtract(deriveTotalSelfGstAtDisbursement(loanCharges));
        if (this.loanProduct.getSelfPrincipalShare().doubleValue() == 100) {
            this.netPartnerDisbursalAmount = BigDecimal.ZERO;
        } else {
            this.netPartnerDisbursalAmount = this.partnerPrincipalAmount
                    .subtract(deriveSumOfPartnerTotalOfChargesDueAtDisbursement())
                    .subtract(deriveTotalPartnerGstAtDisbursement(loanCharges));
        }

        BigDecimal totalGstAtDisbursement = deriveTotalGstAtDisbursement(loanCharges);
        BigDecimal totalSelfGstAtDisbursement = deriveTotalSelfGstAtDisbursement(loanCharges);
        BigDecimal totalPartnerGstAtDisbursement = deriveTotalPartnerGstAtDisbursement(loanCharges);
        if (totalGstAtDisbursement != null) {
            this.summary.updateTotalGstDueAtDisbursement(totalGstAtDisbursement, totalSelfGstAtDisbursement, totalPartnerGstAtDisbursement);
        }
    }

    private BigDecimal deriveTotalGstAtDisbursement(List<LoanCharge> loanCharges) {
    ...
    private BigDecimal deriveTotalSelfGstAtDisbursement(List<LoanCharge> loanCharges) {
        BigDecimal total = BigDecimal.ZERO;

        for (LoanCharge loanCharge : loanCharges) {
            if (loanCharge.isDisbursementCharge()) {
                total = total.add(loanCharge.getTotalSelfGst());
    ...
    private BigDecimal deriveTotalPartnerGstAtDisbursement(List<LoanCharge> loanCharges) {
        BigDecimal total = BigDecimal.ZERO;

        for (LoanCharge loanCharge : loanCharges) {
            if (loanCharge.isDisbursementCharge()) {
                total = total.add(loanCharge.getTotalPartnerGst());
    ...
    private BigDecimal deriveSumTotalOfPartnerChargesDueAtDisbursement() {
...
//        Money chargesDue = Money.of(getCurrency(), BigDecimal.ZERO);
        ...
        for (final LoanCharge charge : charges()) {
            if (charge.isDueAtDisbursement()) {
                ...
                final BigDecimal gst = charge.getCgstAmount().add(charge.getIgstAmount().add(charge.getSgstAmount()));
//                chargesDue = chargesDue.plus(charge.partnerAmount());

                chargesDue = chargesDue.add(charge.partnerAmount());
    ...
    private BigDecimal deriveSumTotalOfSelfChargesDueAtDisbursement() {
...
//        Money chargesDue = Money.of(getCurrency(), BigDecimal.ZERO);
        ...
        for (final LoanCharge charge : charges()) {
            if (charge.isDueAtDisbursement()) {
                ...
                final BigDecimal gst = charge.getCgstAmount().add(charge.getIgstAmount().add(charge.getSgstAmount()));
//                chargesDue = chargesDue.plus(charge.selfAmount());
                chargesDue = chargesDue.add(charge.selfAmount());
    ...
    private BigDecimal deriveTotlGstAmount() {
        Money chargesDue = Money.of(getCurrency(), BigDecimal.ZERO);

        for (final LoanCharge charge : charges()) {
            if (charge.isDueAtDisbursement()) {
                chargesDue = chargesDue.plus(charge.getTotalGst());
            }
    ...
    private BigDecimal derivesgstAmount() {
        Money chargesDue = Money.of(getCurrency(), BigDecimal.ZERO);

        for (final LoanCharge charge : charges()) {
            if (charge.isDueAtDisbursement()) {
                chargesDue = chargesDue.plus(charge.sgstAmount());
            }
        }
    ...
    private BigDecimal deriveIgstAmount() {
        Money chargesDue = Money.of(getCurrency(), BigDecimal.ZERO);

        for (final LoanCharge charge : charges()) {
            if (charge.isDueAtDisbursement()) {
                chargesDue = chargesDue.plus(charge.igstAmount());
            }
        }
    ...
    private BigDecimal deriveCgstAmount() {
        Money chargesDue = Money.of(getCurrency(), BigDecimal.ZERO);

        for (final LoanCharge charge : charges()) {
            if (charge.isDueAtDisbursement()) {
                chargesDue = chargesDue.plus(charge.cgstAmount());
            }
        }
    ...
    private BigDecimal deriveSumOfPartnerTotalOfChargesDueAtDisbursement() {

//        Money chargesDue = Money.of(getCurrency(), BigDecimal.ZERO);

        BigDecimal chargesDue = BigDecimal.ZERO;

        for (final LoanCharge charge : partnerCharges()) {
            if (charge.isDueAtDisbursement()) chargesDue = chargesDue.add(charge.partnerAmount());
        }

        return chargesDue;
    }

    private Set<LoanCharge> partnerCharges() {
    ...
    private BigDecimal deriveSumOfSelfTotalOfChargesDueAtDisbursement() {

        //  Money chargesDue = Money.of(getCurrency(), BigDecimal.ZERO);
        BigDecimal chargesDue = BigDecimal.ZERO;

        for (final LoanCharge charge : selfCharges()) {
            if (charge.isDueAtDisbursement())
//                chargesDue = chargesDue.plus(charge.selfAmount());
    ...
    private Set<LoanCharge> selfCharges() {
    ...
    public BigDecimal getSelfPrincipaAmount() {
        return selfPrincipaAmount;
    }

    public BigDecimal getPartnerPrincipalAmount() {
        return partnerPrincipalAmount;
    }
    ...
    private LoanSummary updateSummaryWithTotalFeeChargesDueAtDisbursement(final BigDecimal feeChargesDueAtDisbursement, final BigDecimal feeChargesSelfDueAtDisbursement, final BigDecimal feeChargesPartnerDueAtDisbursement) {
        if (this.summary == null) {
            this.summary = LoanSummary.create(feeChargesDueAtDisbursement, feeChargesSelfDueAtDisbursement, feeChargesPartnerDueAtDisbursement);
        } else {
            this.summary.updateTotalFeeChargesDueAtDisbursement(feeChargesDueAtDisbursement, feeChargesSelfDueAtDisbursement, feeChargesPartnerDueAtDisbursement);
        }
        return this.summary;
    }
...
//                chargesDue = chargesDue.plus(charge.amount());
                chargesDue = chargesDue.add(charge.getAmount());
    ...
    private List<LoanCharge> associateChargesWithThisLoan(final List<LoanCharge> loanCharges) {
    ...
    public void addLoanCharge(final LoanCharge loanCharge, final Charge chargeDefinition) {
        ...
        if (loanCharge.getChargeCalculation().isPercentageBased()) {
            ...
            if (loanCharge.isInstalmentFee()) {
                ...
                final DaysInYearType daysInYearsType = DaysInYearType.fromInt(chargeDefinition.getPenaltyInterestDaysInYear());
                final Integer penalityInterestDaysInYears = daysInYearsType.equals(DaysInYearType.ACTUAL)
                        ? LocalDate.now().lengthOfYear()
                        : daysInYearsType.getValue();
                //  totalChargeAmt = loanCharge.amountOutstanding().divide(BigDecimal.valueOf(penalityInterestDaysInYears), RoundingMode.HALF_EVEN);
                totalChargeAmt = BigDecimal.valueOf(loanCharge.amountOutstanding().doubleValue() / (BigDecimal.valueOf(penalityInterestDaysInYears).doubleValue()));
                loanCharge.updateSplitShareAmount(totalChargeAmt);
            }
        } else {
            chargeAmt = loanCharge.amountOrPercentage();
        }
        loanCharge.update(chargeAmt, loanCharge.getDueLocalDate(), amount, fetchNumberOfInstallmensAfterExceptions(), totalChargeAmt, loanCharge.getTotalGst());
        ...
        this.summary = updateSummaryWithTotalFeeChargesDueAtDisbursement(deriveSumTotalOfChargesDueAtDisbursement(), deriveSumOfSelfTotalOfChargesDueAtDisbursement(), deriveSumOfPartnerTotalOfChargesDueAtDisbursement());

        // store Id's of existing loan transactions and existing reversed loan
        // transactions
        /**
         * for adhoc charge we kept the condition
         */
        if (loanCharge.isAdhocChargeCharge()) {
            updateLoanSummaryDerivedFields();
            this.summary.updateFeesPaidAndDerivedForAdhocCharge(this.charges);
    ...
    public ChangedTransactionDetail reprocessTransactions() {
        ...
        changedTransactionDetail = loanRepaymentScheduleTransactionProcessor.handleTransaction(getDisbursementDate(),
                allNonContraTransactionsPostDisbursement, getCurrency(), getRepaymentScheduleInstallments(), charges(), null);
    ...
    public LoanTransaction handleChargeAppliedTransaction(final LoanCharge loanCharge, final LocalDate suppliedTransactionDate,
        ...
        BigDecimal selfFeeCharges = loanCharge.selfAmount();
        BigDecimal partnerFeeCharges = loanCharge.partnerAmount();
        ...
        final LoanTransaction applyLoanChargeTransaction = LoanTransaction.accrueLoanCharge(this, getOffice(), chargeAmount,
                transactionDate, feeCharges, penaltyCharges, DateUtils.getLocalDateTimeOfTenant(), currentUser, selfFeeCharges, partnerFeeCharges);
    ...
    private void handleChargePaidTransaction(final LoanCharge charge, final LoanTransaction chargesPayment,
        ...
        final ArrayList<LoanCharge> loanCharges = new ArrayList<>(1);
        loanCharges.add(charge);
        loanRepaymentScheduleTransactionProcessor.handleTransaction(chargesPayment, getCurrency(), chargePaymentInstallments, loanCharges, null);
    ...
    public void removeLoanCharge(final LoanCharge loanCharge) {
        ...
        if (removed) {
            ...
            final LoanRepaymentScheduleProcessingWrapper wrapper = new LoanRepaymentScheduleProcessingWrapper();
            wrapper.reprocess(getCurrency(), getDisbursementDate(), getRepaymentScheduleInstallments(), charges());
            updateSummaryWithTotalFeeChargesDueAtDisbursement(deriveSumTotalOfChargesDueAtDisbursement(), deriveSumOfSelfTotalOfChargesDueAtDisbursement(), deriveSumOfPartnerTotalOfChargesDueAtDisbursement());
        }
        ...
        if (!loanCharge.isDueAtDisbursement() && loanCharge.isPaidOrPartiallyPaid(loanCurrency())) {
            ...
            loanRepaymentScheduleTransactionProcessor.handleTransaction(getDisbursementDate(), allNonContraTransactionsPostDisbursement,
                    getCurrency(), getRepaymentScheduleInstallments(), charges(), null);
        }
        ...
        if (loanCharge.isDueAtDisbursement()) {
            ...
            for (final LoanTransaction transaction : transactions) {
                if (transaction.isRepaymentAtDisbursement()
                    ...
                    final Money selfChargeAmount = Money.of(currency, loanCharge.selfAmount());
                    final Money partnerChargeAmount = Money.of(currency, loanCharge.partnerAmount());
                    if (transaction.isGreaterThan(chargeAmount)) {
                        final Money principalPortion = Money.zero(currency);
                        final Money selfPrincipal = Money.zero(currency);
                        final Money partnerPrincipal = Money.zero(currency);

                        final Money interestPortion = Money.zero(currency);
                        final Money selfInterestCharged = Money.zero(currency);
                        final Money partnerInterestCharged = Money.zero(currency);

                        final Money penaltychargesPortion = Money.zero(currency);
                        final Money selfSharePenalAmount = Money.zero(currency);
                        final Money partnerSharePenalAmount = Money.zero(currency);
                        transaction.updateComponentsAndTotal(principalPortion, interestPortion, chargeAmount, penaltychargesPortion,
                                selfPrincipal, partnerPrincipal, selfInterestCharged, partnerInterestCharged, selfSharePenalAmount,
                                partnerSharePenalAmount, selfChargeAmount.getAmount(), partnerChargeAmount.getAmount());
    ...
    public BigDecimal getTotalInterest() {
        return this.loanSummaryWrapper.calculateTotalInterestCharged(getRepaymentScheduleInstallments());
    }
    ...
    public void updateLoanCharges(final List<LoanCharge> loanCharges) {
        ...
        for (final LoanCharge loanCharge : loanCharges) {
            ...
            if (charge != null) {
                charge.update(chargeAmt, loanCharge.getDueLocalDate(), amount, fetchNumberOfInstallmensAfterExceptions(), totalChargeAmt, loanCharge.getTotalGst());
            }
    ...
    public void updateLoanSchedule(final LoanScheduleModel modifiedLoanSchedule, final AppUser currentUser) {
        this.repaymentScheduleInstallments.clear();
        BigDecimal outstanding = this.getLoanProductRelatedDetail().getPrincipal().getAmount();
        BigDecimal selfPrincipalOutstanding = this.selfPrincipaAmount;
        BigDecimal partnerPrincipalOutstanding = this.partnerPrincipalAmount;
        for (final LoanScheduleModelPeriod scheduledLoanInstallment : modifiedLoanSchedule.getPeriods()) {
            if (scheduledLoanInstallment.isRepaymentPeriod()) {
                /**
                 * Adding Last Installment Validation For Handling Negative Self and Partner Share
                 */
                boolean isLastInstallment = this.getNumberOfRepayments().equals(scheduledLoanInstallment.periodNumber());
                BigDecimal selfPrincipalDue = isLastInstallment ? selfPrincipalOutstanding : scheduledLoanInstallment.selfPrincipal();
                BigDecimal partnerPrincipalDue = isLastInstallment ? partnerPrincipalOutstanding : scheduledLoanInstallment.partnerPrincipal();
                ...
                final LoanRepaymentScheduleInstallment installment = new LoanRepaymentScheduleInstallment(this,
                        ...
                        scheduledLoanInstallment.getLoanCompoundingDetails(), selfPrincipalDue,
                        partnerPrincipalDue, scheduledLoanInstallment.selfInterestCharged(),
                        scheduledLoanInstallment.partnerInterestCharged(), scheduledLoanInstallment.selfDue(),
                        scheduledLoanInstallment.partnerDue(), null);

                outstanding = outstanding.subtract(scheduledLoanInstallment.principalDue());
                installment.setPrincipalOutstanding(outstanding);
                selfPrincipalOutstanding = selfPrincipalOutstanding.subtract(selfPrincipalDue);
                installment.setSelfPrincipalOutstanding(selfPrincipalOutstanding);
                partnerPrincipalOutstanding = partnerPrincipalOutstanding.subtract(partnerPrincipalDue);
                installment.setPartnerPrincipalOutstanding(partnerPrincipalOutstanding);
                installment.setDue(scheduledLoanInstallment.principalDue().add(scheduledLoanInstallment.interestDue()));
    ...
    private void updateAccrualsForNonPeriodicAccruals(final Collection<LoanTransaction> accruals, final AppUser currentUser) {
        ...
        BigDecimal selfDue = summary.getTotalSelfInterestCharged();
        BigDecimal partnerDue = summary.getTotalPartnerInterestCharged();
        for (LoanTransaction loanTransaction : accruals) {
            if (loanTransaction.getInterestPortion(getCurrency()).isGreaterThanZero()) {
                if (loanTransaction.getInterestPortion(getCurrency()).isNotEqualTo(interestApplied)) {
                    ...
                    final LoanTransaction interestAppliedTransaction = LoanTransaction.accrueInterest(getOffice(), this, interestApplied,
                            getDisbursementDate(), currentDateTime, currentUser, selfDue, partnerDue);
...
//            this.totalOverpaid = overpaidBy.getAmountDefaultedToNullIfZero();
            this.totalOverpaid = this.retrieveAdvanceAmount();
            ...
            final BigDecimal selfPrincipalAmount = this.getSelfPrincipaAmount();
            final BigDecimal partnerPrincipalAmount = this.getPartnerPrincipalAmount();

            this.summary.updateSummary(loanCurrency(), principal, getRepaymentScheduleInstallments(), Objects.isNull(this.loanSummaryWrapper) ?  new LoanSummaryWrapper() :this.loanSummaryWrapper ,
                    isDisbursed(), this.charges, selfPrincipalAmount, partnerPrincipalAmount);
    ...
    public Map<String, Object> loanApplicationModification(final JsonCommand command, final List<LoanCharge> possiblyModifedLoanCharges,
    ...
    public void recalculateAllCharges() {
        List<LoanCharge> charges = this.charges();
        ...
        if (disbursementID != null) {
            ...
            for (LoanTrancheCharge trancheCharge : trancheCharges) {
                ...
                final LoanCharge loanCharge = LoanCharge.createNewWithoutLoan(chargeDefinition, principal, null, null, null,
                        LocalDate.ofInstant(expectedDisbursementDate.toInstant(), DateUtils.getDateTimeZoneOfTenant()), null, null, null, null, null);
                loanCharge.update(this);
                LoanTrancheDisbursementCharge loanTrancheDisbursementCharge = new LoanTrancheDisbursementCharge(loanCharge,
                        disbursementDetails);
                loanCharge.updateLoanTrancheDisbursementCharge(loanTrancheDisbursementCharge);
                addLoanCharge(loanCharge, chargeDefinition);
            }
    ...
    private LoanChargeCommand[] getLoanCharges(final List<LoanCharge> setOfLoanCharges) {
    ...
    public void loanApplicationSubmittal(final AppUser currentUser, final LoanScheduleModel loanSchedule,
                                         ...
                                         final WorkingDays workingDays, final boolean allowTransactionsOnNonWorkingDay, final Loan loanApplication) {


        updateLoanSchedule(loanSchedule, currentUser);

        BigDecimal brokenDisbursement = loanApplicationTerms.getBrokenDisbursement();


        final String brokenStrategy = String.valueOf(loanApplicationTerms.getBrokenStrategy());

        final String broken = "DISBURSEMENT";
        ...
        if (brokenStrategy.equals(broken)) {
            loanApplication.setBrokenInterestDerived(brokenDisbursement);
            loanApplication.setBrokenInterestRepaid(brokenDisbursement);
            if (brokenDisbursement == null) {
                brokenDisbursement = BigDecimal.valueOf(0);

                this.netDisbursalAmount = BigDecimal.valueOf(netDisbursalAmount.doubleValue() - brokenDisbursement.doubleValue()).setScale(2, RoundingMode.HALF_UP);
            }
        }
    ...
    public Map<String, Object> loanApplicationApproval(final AppUser currentUser, final JsonCommand command,
        ...
        if (!newStatusEnum.hasStateOf(LoanStatus.fromInt(this.loanStatus))) {
            ...
            LocalDate expecteddisbursementDate = command.localDateValueOfParameterNamed("approvedOnDate");
    ...
    public ChangedTransactionDetail disburse(final AppUser currentUser, final JsonCommand command, final Map<String, Object> actualChanges,
        ...
        this.disbursedBy = currentUser.isSystemUser() ? approvedBy : currentUser;
        ...
        handleDisbursementTransaction(actualDisbursementDate, createdDate, disbursedBy, paymentDetail);
        updateLoanSummaryDerivedFields();
        final Money interestApplied = Money.of(getCurrency(), this.summary.getTotalInterestCharged());
        final BigDecimal selfDue = summary.getTotalSelfInterestCharged();
        final BigDecimal partnerDue = summary.getTotalPartnerInterestCharged();
         ...
         *
         * @author akashs464 - Removed Upfront Interest accrual
         **/
        ...
        /*if (((isMultiDisburmentLoan() && getDisbursedLoanDisbursementDetails().size() == 1) || !isMultiDisburmentLoan())
            ...
            final LoanTransaction interestAppliedTransaction = LoanTransaction.accrueInterest(getOffice(), this, interestApplied,
                    actualDisbursementDate, createdDate, currentUser,selfDue,partnerDue);
            addLoanTransaction(interestAppliedTransaction);
        }*/
    ...
    public void regenerateRepaymentSchedule(final ScheduleGeneratorDTO scheduleGeneratorDTO, final AppUser currentUser) {
        ...
        updateLoanSchedule(loanSchedule, currentUser);
        final List<LoanCharge> charges = this.charges();
        for (final LoanCharge loanCharge : charges) {
            if (!loanCharge.isWaived()) {
    ...
    public LoanScheduleModel regenerateScheduleModel(final ScheduleGeneratorDTO scheduleGeneratorDTO) {
        ...
        if (loanApplicationTerms.isEqualAmortization()) {
            if (loanApplicationTerms.getInterestMethod().isDecliningBalnce()) {
                ...
                List<LoanCharge> loanCharges = charges();
    ...
    private void handleDisbursementTransaction(final LocalDate disbursedOn, final LocalDateTime createdDate, final AppUser currentUser,
        ...
        final BigDecimal totalFeeChargesDueAtDisbursement = this.summary.getTotalFeeChargesDueAtDisbursement();
        /**
        ...
        BigDecimal selfDisburseMoney = BigDecimal.ZERO;
        BigDecimal partnerDisburseMoney = BigDecimal.ZERO;
        BigDecimal selfDue = selfPrincipaAmount;
        BigDecimal partnerDue = partnerPrincipalAmount;
        final LoanTransaction chargesPayment = LoanTransaction.repaymentAtDisbursement(getOffice(), disbursentMoney, paymentDetail,
                disbursedOn, null, createdDate, currentUser, selfDue, partnerDue);
        chargesPayment.setValueDate(Date.from(disbursedOn.atStartOfDay(ZoneId.systemDefault()).toInstant()));
        ...
        for (final LoanCharge charge : charges()) {
            ...
            if ((charge.getCharge().getChargeTimeType().equals(ChargeTimeType.DISBURSEMENT.getValue())
                    ...
                    && (actualDisbursementDate != null) && !charge.isWaived() && !charge.isFullyPaid())) {
                /** update for loan summary screen from Money to bigDecimal in totalFeeChargesDueAtDisbursement
                 */
                if (totalFeeChargesDueAtDisbursement.doubleValue() > 0 && !charge.getChargePaymentMode().isPaymentModeAccountTransfer()) {
                    ...
                    final LoanChargePaidBy loanChargePaidBy = new LoanChargePaidBy(chargesPayment, charge, charge.amount(),
                            installmentNumber);
                    chargesPayment.getLoanChargesPaid().add(loanChargePaidBy);
                    disbursentMoney = disbursentMoney.plus(charge.amount()).add(charge.getTotalGst());
                    selfDisburseMoney = selfDisburseMoney.add(charge.selfAmount()).add(charge.getSelfGst());
                    partnerDisburseMoney = partnerDisburseMoney.add(charge.partnerAmount()).add(charge.getPartnerGst());
        ...
        if (disbursentMoney.isGreaterThanZero()) {
            ...
            chargesPayment.updateComponentsAndTotal(zero, zero, disbursentMoney, zero, zero, zero, zero, zero, zero, zero, selfDisburseMoney, partnerDisburseMoney);
            chargesPayment.setSelfDue(selfDisburseMoney);
            chargesPayment.setPartnerDue(partnerDisburseMoney);

            chargesPayment.setAmount(selfDisburseMoney.add(partnerDisburseMoney));
            chargesPayment.setFeeChargesPortion(selfDisburseMoney.add(partnerDisburseMoney));
            chargesPayment.setEvent(LoanTransactionType.REPAYMENT_AT_DISBURSEMENT.toString());
            chargesPayment.updateLoan(this);
            addLoanTransaction(chargesPayment);
            updateLoanOutstandingBalaces();
        }
    ...
    public LoanTransaction handlePayDisbursementTransaction(final Long chargeId, final LoanTransaction chargesPayment,
        ...
        chargesPayment.getLoanChargesPaid().add(loanChargePaidBy);
        final Money zero = Money.zero(getCurrency());
        chargesPayment.updateComponents(zero, zero, charge.getAmount(getCurrency()), zero, zero, zero, zero, zero, zero.getAmount(), zero.getAmount(), zero.getAmount(), zero.getAmount());
        chargesPayment.updateLoan(this);
        addLoanTransaction(chargesPayment);
        updateLoanOutstandingBalaces();
        charge.markAsFullyPaid();
    ...
    public ChangedTransactionDetail waiveInterest(final LoanTransaction waiveInterestTransaction,
        ...
        final ChangedTransactionDetail changedTransactionDetail = handleRepaymentOrRecoveryOrWaiverTransaction(waiveInterestTransaction,
                loanLifecycleStateMachine, null, scheduleGeneratorDTO, currentUser, null);
    ...
    public ChangedTransactionDetail makeRepayment(final LoanTransaction repaymentTransaction,
                                                  ...
                                                  final AppUser currentUser, Boolean isHolidayValidationDone, LoanHistoryRepo loanHistoryRepo) {
        ...
        final ChangedTransactionDetail changedTransactionDetail = handleRepaymentOrRecoveryOrWaiverTransaction(repaymentTransaction,
                loanLifecycleStateMachine, null, scheduleGeneratorDTO, currentUser, loanHistoryRepo);
    ...
    private ChangedTransactionDetail handleRepaymentOrRecoveryOrWaiverTransaction(final LoanTransaction loanTransaction,
                                                                                  ...
                                                                                  final ScheduleGeneratorDTO scheduleGeneratorDTO, final AppUser currentUser, LoanHistoryRepo loanHistoryRepo) {
        ChangedTransactionDetail changedTransactionDetail = null;
        ...
        statusEnum = loanTransaction.isRecoveryRepayment() ? loanLifecycleStateMachine.transition(LoanEvent.LOAN_RECOVERY_PAYMENT, LoanStatus.fromInt(this.loanStatus))
                : loanLifecycleStateMachine.transition(LoanEvent.LOAN_REPAYMENT_OR_WAIVER, LoanStatus.fromInt(this.loanStatus));
...
//                .determineProcessor(this.transactionProcessingStrategy);
        ...
        if (reprocess) {
            ...
            final List<LoanTransaction> pendingTransaction = Collectionutills.getAdvanceLoanTransaction(this, loanTransaction);
            pendingTransaction.add(loanTransaction);

            // deprecated so passing null
            CollectionService collectionService = new CollectionService(null);
            collectionService.appropriateInstallment(this, loanTransactionDate, pendingTransaction, loanHistoryRepo);

           /* changedTransactionDetail = loanRepaymentScheduleTransactionProcessor.handleTransaction(getDisbursementDate(),
                    pendingTransaction, getCurrency(), getRepaymentScheduleInstallments(), charges(),loanHistoryRepo);*/
        ...
        if (loanTransaction.getAppUser().isSystemUser()) {
            ...
            /*this.loanTransactions.stream()
                    .filter(transaction -> (!transaction.isRepaymentAtDisbursement() || !transaction.isDisbursement()) && Objects.nonNull(transaction.getEvent())
                            && transaction.getEvent().equals(BusinessEventNotificationConstants.BusinessEvents.PROCESSEDADVANCE.getValue())
                            && (transaction.getPrincipalPortion() == null && transaction.getInterestPortion() == null))
                    .forEach(transaction -> {
                            transaction.setAdvanceAmountprocessed(0L);
        ...
        this.updateXirrValue(XirrService.xirrCalculation(this.getRepaymentScheduleInstallments(), this.getPrincpal().getAmount(), this.getDisbursementDate(),
                this.getCurrency(), this.getLoanCharges(), allNonContraTransactionsPostDisbursement, this));
        ...
        ServicerFeeWritePlatformService.calculateServicerFee(this.loanTransactions, loanProduct, this.getCurrency(), this);
    ...
    private List<LoanTransaction> retrieveAdvanceLoanTransaction() {

        List<LoanTransaction> repaymentsOrWaivers = new ArrayList<>();
        List<LoanTransaction> trans = getLoanTransactions();

        List<LoanTransaction> advanceTransaction = trans.stream().filter(loanTransaction ->
                Objects.nonNull(loanTransaction.getEvent()) &&
                        loanTransaction.getEvent().equals(BusinessEventNotificationConstants.BusinessEvents.UNPROCESSEDADVANCE.getValue())
                        && loanTransaction.getAdvanceAmountprocessed() == 0).toList();
        repaymentsOrWaivers.addAll(advanceTransaction);
    ...
    public void reversalForBackdatedCaseWithOverDueCharge(LocalDate transactionDate, Integer installmentNumber) {
        ...
        if (!installment.isNotFullyPaidOff()) {
            installment = fetchRepaymentScheduleInstallment(installment.getInstallmentNumber() + 1);
        }
        final LoanRepaymentScheduleInstallment as = installment;
        Integer arrearAgeing = this.getLoanProductRelatedDetail().getGraceOnArrearsAgeing();
        if (arrearAgeing != 0) {
            date = installment.getDueDate().plusDays(arrearAgeing);
            if (transactionDate.isAfter(date)) {
                loanCharges = this.getLoanCharges().stream().filter(loanCharge -> loanCharge.isPenaltyCharge()).filter(loanCharge -> loanCharge.getInstallmentNumber().equals(as.getInstallmentNumber()))
                        .filter(loanCharge -> loanCharge.getDueLocalDate().isBefore(transactionDate)).collect(Collectors.toSet());
                this.charges.removeAll(loanCharges);
                // installment.updateDpdNumberForValueDate(loanCharges.size());
                getLoanChargeByInstallmentNumber(installmentNumber, this.charges);
                LOG.info("Backdated overdueCharge with grace period charges found size : {}", loanCharges.size());
            } else {
                loanCharges = this.getLoanCharges().stream().filter(loanCharge -> loanCharge.isPenaltyCharge()).filter(loanCharge -> loanCharge.getInstallmentNumber().equals(as.getInstallmentNumber())).collect(Collectors.toSet());
                this.charges.removeAll(loanCharges);
                //   installment.updateDpdNumberForValueDate(Long.valueOf(0));
                getLoanChargeByInstallmentNumber(installmentNumber, this.charges);
                LOG.info("Backdated overdueCharge with grace period and with in due date charges found size : {}", loanCharges.size());
            ...
            LOG.info("Backdated overdueCharge without grace period charges found size : {}", loanCharges.size());

            /**
             * Again adding the backdated charges to the loan for the calculating the penalty in current installment
             **/

        }
        this.charges.addAll(loanCharges);
    }
//        final LocalDate scheduleDuedate = transactionDate;
//        LoanRepaymentScheduleInstallment installment = fetchRepaymentScheduleInstallment(installmentNumber);
//        if(!installment.isNotFullyPaidOff()){
//            installment =fetchRepaymentScheduleInstallment(installment.getInstallmentNumber() + 1);
//        }
...
//        Set<LoanCharge> backDatedCharges = new HashSet<>();
//        LocalDate dueDate = installment.getDueDate();
//        while(dueDate.plusDays(arrearAgeing).isBefore(transactionDate)){
//            for (LoanCharge loanCharge : this.getLoanCharges()) {
//                if (dueDate.equals(loanCharge.getDueLocalDate()) && loanCharge.isPenaltyCharge()
//                        && loanCharge.getInstallmentNumber().equals(installment.getInstallmentNumber())) {
//                    dates.add(loanCharge.getDueLocalDate());
//                    backDatedCharges.add(loanCharge);
//                    i++;
//                }
//            }
...
//       final Set<LoanCharge> loanCharges = this.getLoanCharges().stream().filter(loanCharge -> loanCharge.isPenaltyCharge()).filter(loanCharge ->loanCharge.getInstallmentNumber().equals(as.getInstallmentNumber()))
//                             .filter(loanCharge -> loanCharge.getDueLocalDate().isBefore(scheduleDuedate)).collect(Collectors.toSet());
//

//        if (!loanCharges.isEmpty()) {
//
//            if(transactionDate.isBefore(installment.getDueDate())){
//                this.charges.removeAll(loanCharges);
//                installment.updateDpdNumberForValueDate(loanCharges.size());
//            }
//            LOG.info("Backdated overdueCharge",loanCharges.size());
//        }
//        getLoanChargeByInstallmentNumber(installment.getInstallmentNumber(),this.charges);
...
//    private LoanRepaymentScheduleInstallment fetchInstallentByDate(LocalDate transactionDate) {
//
//        LoanRepaymentScheduleInstallment currentInstallment = null;
//
//        for(LoanRepaymentScheduleInstallment loanRepaymentScheduleInstallment : this.getRepaymentScheduleInstallments()){
//
//            if((transactionDate.isAfter(loanRepaymentScheduleInstallment.getFromDate()) && transactionDate.isBefore(loanRepaymentScheduleInstallment.getDueDate()))
//                    || (transactionDate.equals(loanRepaymentScheduleInstallment.getFromDate())) || (transactionDate.equals(loanRepaymentScheduleInstallment.getDueDate()))){
//
//                currentInstallment= loanRepaymentScheduleInstallment;
//            }
//        }
//        return currentInstallment;
//    }
    public void getLoanChargeByInstallmentNumber(Integer installmentNumber, List<LoanCharge> loanCharges) {
        for (LoanCharge loanCharge : loanCharges) {
            if (loanCharge.getInstallmentNumber().equals(installmentNumber)) {
                loanCharge.updateActive();
            }
        }
    }
    ...
    public List<LoanTransaction> retreiveListOfTransactionsPostDisbursement() {
        ...
        for (final LoanTransaction transaction : trans) {
            if (transaction.isNotReversed() && !(transaction.isDisbursement() || transaction.isNonMonetaryTransaction())
                    && !transaction.getAppUser().isSystemUser() && Objects.nonNull(transaction.getEvent())
                    && (!transaction.getEvent().equals(BusinessEventNotificationConstants.BusinessEvents.UNPROCESSEDADVANCE.toString()) || !transaction.getAmount(this.getCurrency()).isZero())) {
...
//        } else
        ...
        if (this.summary.isRepaidInFull(loanCurrency())) {
      ...
      /*  for (final LoanCharge loanCharge : this.charges) {
        ...
        if (isAllChargesPaid) {
            final LoanStatus statusEnum = loanLifecycleStateMachine.transition(LoanEvent.REPAID_IN_FULL,
                    LoanStatus.fromInt(this.loanStatus));
            this.loanStatus = statusEnum.getValue();
            LoanRepaymentScheduleInstallment loanRepaymentScheduleInstallment = this.repaymentScheduleInstallments
                    .get(this.getLoanRepaymentScheduleInstallmentsSize() - 1);
            ...
            if (loanRepaymentScheduleInstallment.getObligationsMetOnDate().isBefore(lastInstallmentDueDate)) {
                this.closedOnDate = Date.from(lastInstallmentDueDate.atStartOfDay(ZoneId.systemDefault()).toInstant());
                this.actualMaturityDate = Date.from(lastInstallmentDueDate.atStartOfDay(ZoneId.systemDefault()).toInstant());
            } else {
    ...
    private void processIncomeAccrualTransactionOnLoanClosure() {
        if (this.loanInterestRecalculationDetails != null && this.loanInterestRecalculationDetails.isCompoundingToBePostedAsTransaction()
            ...
            LoanTransaction finalIncomeTransaction = LoanTransaction.incomePosting(this, this.getOffice(), closedDate, amountToPost,
                    interestToPost, feeToPost, penaltyToPost, null, null, null, null, null, null, null);
            ...
            if (isPeriodicAccrualAccountingEnabledOnLoanProduct()) {
                ...
                LoanTransaction finalAccrual = LoanTransaction.accrueTransaction(this, this.getOffice(), closedLocalDate, amountToPost,
                        interestToPost, feeToPost, penaltyToPost, null, null, null, selfDue, partnerDue);
    ...
    public LoanTransaction deriveDefaultInterestWaiverTransaction(final LocalDateTime createdDate, final AppUser currentUser) {
        ...
        return LoanTransaction.waiver(getOffice(), this, possibleInterestToWaive, transactionDate, possibleInterestToWaive,
                possibleInterestToWaive.zero(), createdDate, currentUser, null, null);
    }
    ...
    public ChangedTransactionDetail adjustExistingTransaction(final LoanTransaction newTransactionDetail,
        ...
        if (newTransactionDetail.isRepaymentType() || newTransactionDetail.isInterestWaiver()) {
            changedTransactionDetail = handleRepaymentOrRecoveryOrWaiverTransaction(newTransactionDetail, loanLifecycleStateMachine,
                    transactionForAdjustment, scheduleGeneratorDTO, currentUser, null);
        }
    ...
    private Money calculateTotalOverpayment() {
        ...
        for (final LoanRepaymentScheduleInstallment scheduledRepayment : installments) {
            ...
            cumulativeTotalPaidOnInstallments = cumulativeTotalPaidOnInstallments
                    ...
                    .plus(scheduledRepayment.getFeeChargesPaid(currency)).plus(scheduledRepayment.getPenaltyChargesPaid(currency).plus(scheduledRepayment.getAdvanceAmount(currency).plus(this.getLoanSummary().getTotalGstPaid())));
    ...
    private Money getTotalPaidInRepayments() {
        ...
        for (final LoanTransaction repayment : this.loanTransactions) {
            if (repayment.isRepaymentType() && !repayment.isReversed()) {
                if (repayment.getPrincipalPortion() != null || repayment.getInterestPortion() != null) {
    ...
    public List<LoanCharge> charges() {
    ...
    private void addUpdateIncomeAndAccrualTransaction(LoanInterestRecalcualtionAdditionalDetails compoundingDetail,
        ...
        if (existingIncomeTransaction == null) {
            LoanTransaction transaction = LoanTransaction.incomePosting(this, this.getOffice(),
                    ...
                    compoundingDetail.getAmount(), interest, fee, penalties, currentUser, selfInterestCharged, partnerInterestCharged, selfDue, partnerDue, null, null);
        ...
        if (isPeriodicAccrualAccountingEnabledOnLoanProduct()) {
            if (existingAccrualTransaction == null) {
                LoanTransaction accrual = LoanTransaction.accrueTransaction(this, this.getOffice(), compoundingDetail.getEffectiveDate(),
                        compoundingDetail.getAmount(), interest, fee, penalties, currentUser, selfInterestCharged, partnerInterestCharged, selfDue, partnerDue);
    ...
    public void processPostDisbursementTransactions() {
        ...
        if (allNonContraTransactionsPostDisbursement.size() > 0) {
            ...
            loanRepaymentScheduleTransactionProcessor.handleTransaction(getDisbursementDate(), copyTransactions, getCurrency(),
                    getRepaymentScheduleInstallments(), charges(), null);
    ...
    public LoanApplicationTerms constructLoanApplicationTerms(final ScheduleGeneratorDTO scheduleGeneratorDTO) {
        ...
        final BigDecimal selfDue = null;
        final BigDecimal partnerDue = null;
        final BrokenStrategy brokenStrategy = BrokenStrategy.valueOf(this.brokenStrategy);
        final BrokenStrategyDayInYear brokenStrategyDayInYear = BrokenStrategyDayInYear.fromInt(this.getLoanProductRelatedDetail().getBrokenInterestDaysInYears());
        final BrokenStrategyDaysInMonth brokenStrategyDaysInMonth = BrokenStrategyDaysInMonth.fromInt(this.getLoanProductRelatedDetail().getBrokenInterestDaysInMonth());
        ...
        final LoanApplicationTerms loanApplicationTerms = LoanApplicationTerms.assembleFrom(scheduleGeneratorDTO.getApplicationCurrency(),
                ...
                scheduleGeneratorDTO.isPrincipalCompoundingDisabledForOverdueLoans(), selfPrincipal, partnerPrincipal, selfInterestCharged,
                partnerInterestCharged, loanProduct, selfDue, partnerDue, brokenStrategy, brokenStrategyDayInYear, brokenStrategyDaysInMonth);
    ...
    private LoanRepaymentScheduleInstallment getTotalOutstandingOnLoan() {
        ...
        Money selfPrincipal = Money.zero(loanCurrency());
        Money partnerPrincipal = Money.zero(loanCurrency());
        Money selfInterestCharged = Money.zero(loanCurrency());
        Money partnerInterestCharged = Money.zero(loanCurrency());
        Money selfDue = Money.zero(loanCurrency());
        Money partnerDue = Money.zero(loanCurrency());
        ...
        return new LoanRepaymentScheduleInstallment(null, 0, LocalDate.now(DateUtils.getDateTimeZoneOfTenant()),
                ...
                feeCharges.getAmount(), penaltyCharges.getAmount(), false, compoundingDetails, selfPrincipal.getAmount(), partnerPrincipal.getAmount(), selfInterestCharged.getAmount(), partnerInterestCharged.getAmount(), selfDue.getAmount(), partnerDue.getAmount(), null);
    }
    ...
    private void updateLoanOutstandingBalaces() {
        Money selfOutstanding = Money.zero(getCurrency());
        Money partnerOutstanding = Money.zero(getCurrency());
        Money outstanding = Money.zero(getCurrency());

        for (LoanTransaction loanTransaction : retreiveListOfTransactionsExcludeAccruals()) {
            if (loanTransaction.isDisbursement() || loanTransaction.isIncomePosting()) {
                ...
                selfOutstanding = selfOutstanding.plus(loanTransaction.getSelfDue(getCurrency()));
                partnerOutstanding = partnerOutstanding.plus(loanTransaction.getPartnerDue(getCurrency()));
                loanTransaction.updateOutstandingLoanBalance(outstanding.getAmount(), selfOutstanding.getAmount(), partnerOutstanding.getAmount());
            } else {
                if (this.loanInterestRecalculationDetails != null
                    ...
                    selfOutstanding = selfOutstanding.minus(loanTransaction.getSelfDue(getCurrency()));
                    partnerOutstanding = partnerOutstanding.minus(loanTransaction.getPartnerDue(getCurrency()));
                } else if(loanTransaction.getPrincipalPortion(getCurrency()).isGreaterThanZero()){
                    outstanding = outstanding.minus(loanTransaction.getPrincipalPortion(getCurrency()));
                    selfOutstanding = selfOutstanding.minus(loanTransaction.getSelfPrincipalPortion(getCurrency()));
                    partnerOutstanding = partnerOutstanding.minus(loanTransaction.getPartnerPrincipalPortion(getCurrency()));
                }
                loanTransaction.updateOutstandingLoanBalance(outstanding.getAmount(), selfOutstanding.getAmount(), partnerOutstanding.getAmount());
            }
        }
    ...
    public List<DisbursementData> getDisbursmentData() {
        ...
        while (iterator.hasNext()) {
            ...
            disbursementData.add(new DisbursementData(loanDisbursementDetails.getId(), expectedDisbursementDate, actualDisbursementDate,
                    loanDisbursementDetails.principal(), this.netDisbursalAmount, null, null, waivedChargeAmount, this.selfPrincipaAmount, this.partnerPrincipalAmount, null, null));
        }
    ...
    public LoanApplicationTerms getLoanApplicationTerms(final ApplicationCurrency applicationCurrency,
                                                        ...
                                                        final HolidayDetailDTO holidayDetailDTO, final BigDecimal selfPrincipal, final BigDecimal partnerPrincipal, final BigDecimal selfInterestCharged, final BigDecimal partnerInterestCharged, final BigDecimal selfDue, final BigDecimal partnerDue) {
        ...
        final BrokenStrategyDayInYear brokenStrategyDayInYear = null;
        final BrokenStrategyDaysInMonth brokenStrategyDaysInMonth = null;
        ...
        return LoanApplicationTerms.assembleFrom(applicationCurrency, loanTermFrequency, loanTermPeriodFrequencyType, nthDayType,
                ...
                false, this.fixedPrincipalPercentagePerInstallment, false, selfPrincipal, partnerPrincipal, selfInterestCharged,
                partnerInterestCharged, loanProduct, selfDue, partnerDue, brokenStrategy, brokenStrategyDayInYear, brokenStrategyDaysInMonth);
    }
    ...
    public LoanRepaymentScheduleInstallment fetchLoanForeclosureDetail(final LocalDate closureDate) {
        ...
        Money selfPrincipal = Money.of(getCurrency(), this.getSummary().getTotalSelfPrincipalOutstanding());
        Money partnerPrincipal = Money.of(getCurrency(), this.getSummary().getTotalPartnerPrincipalOutstanding());
        Money selfInterestCharged = Money.of(getCurrency(), this.getSummary().getTotalSelfInterestCharged());
        Money partnerInterestCharged = Money.of(getCurrency(), this.getSummary().getTotalPartnerInterestCharged());
        Money selfDue = Money.of(getCurrency(), this.getSummary().getTotalSelfOutstanding());
        Money partnerDue = Money.of(getCurrency(), this.getSummary().getTotalPartnerOutstanding());
        totalPrincipal = totalPrincipal.minus(receivables[4]);
        final Set<LoanInterestRecalcualtionAdditionalDetails> compoundingDetails = null;
        final LocalDate currentDate = DateUtils.getLocalDateOfTenant();
        BigDecimal advanceAmount = retrieveAdvanceAmount();
        return new LoanRepaymentScheduleInstallment(null, 0, currentDate, currentDate, totalPrincipal.getAmount(),
                ...
                selfPrincipal.getAmount(), partnerPrincipal.getAmount(), selfInterestCharged.getAmount(), partnerInterestCharged.getAmount(), selfDue.getAmount(),
                partnerDue.getAmount(), null, null, advanceAmount);
    }

    public BigDecimal retrieveAdvanceAmount() {
        return this.getRepaymentScheduleInstallments()
                .stream()
    ...
    public Money[] retriveIncomeOutstandingTillDate(final LocalDate paymentDate) {
        ...
        Money paidInstallment = Money.zero(currency);
        Money interestForCurrentPeriod = Money.zero(currency);
        for (final LoanRepaymentScheduleInstallment installment : this.repaymentScheduleInstallments) {
            if (!installment.getDueDate().isAfter(paymentDate)) {
                ...
                if (interest.getAmount().doubleValue() == 0) {
                    balances[0] = interest.add(installment.getInterestUptodate(currency));
                }
                ...
                if (installment.isPartlyPaid()) {
                    final BigDecimal interestPaid = installment.getInterestPaid();
                    final BigDecimal principalPaid = installment.getPrincipalCompleted();
                    final BigDecimal totalPaidForTheInstallment = interestPaid.add(principalPaid);
                    // balances [4] paid for future installment for get Api added
                    paidInstallment = Money.of(currency, totalPaidForTheInstallment);
                    ...
                    interestForCurrentPeriod = Money.of(getCurrency(), BigDecimal
                            .valueOf(calculateForeclosureInterestForDays(this.getLoanProductRelatedDetail().getDaysInYearType(), this.getLoanSummary().getTotalPrincipalOutstanding().subtract(totalPaidForTheInstallment), this.getLoanProductRelatedDetail().getAnnualNominalInterestRate(), tillDays, paymentDate)));
                }
                // balances [4] paid for future installment for get Api added
                balances[4] = paidInstallment;
                // balances [5] interest  for get Api added
                balances[5] = interestForCurrentPeriod;
                ...
                // balances [4] paid for future installment for get Api added
                balances[4] = balancesForCurrentPeroid[7];
                // balances [5] interest  for get Api added
                balances[5] = balancesForCurrentPeroid[0];
                if (balancesForCurrentPeroid[0].isGreaterThan(balancesForCurrentPeroid[7])) {
                    interest = interest.plus(balancesForCurrentPeroid[0]).minus(balancesForCurrentPeroid[5]);
                } else {
                    paidFromFutureInstallments = paidFromFutureInstallments.plus(balancesForCurrentPeroid[7])
                ...
                if (balancesForCurrentPeroid[1].isGreaterThan(balancesForCurrentPeroid[3])) {
                    ...
                    paidFromFutureInstallments = paidFromFutureInstallments
                            .plus(balancesForCurrentPeroid[7].minus(balancesForCurrentPeroid[1]));
                }
                if (balancesForCurrentPeroid[2].isGreaterThan(balancesForCurrentPeroid[6])) {
                    penalty = penalty.plus(balancesForCurrentPeroid[2].minus(balancesForCurrentPeroid[6]));
                } else {
                    paidFromFutureInstallments = paidFromFutureInstallments.plus(balancesForCurrentPeroid[6])
    ...
    private Money[] fetchInterestFeeAndPenaltyTillDate(final LocalDate paymentDate, final MonetaryCurrency currency,
        ...
        Money selfInterestForCurrentPeriod = Money.zero(getCurrency());
        Money partnerInterestForCurrentPeriod = Money.zero(getCurrency());
        //  int totalPeriodDays = Math.toIntExact(ChronoUnit.DAYS.between(installment.getFromDate(), installment.getDueDate()));
        int tillDays = Math.toIntExact(ChronoUnit.DAYS.between(installment.getFromDate(), paymentDate));
        final BigDecimal selfSplitShare = BigDecimal.valueOf(this.getLoanProduct().getSelfPrincipalShare());
        final BigDecimal selfShare = this.getLoanProduct().getSelfInterestRate();
        final BigDecimal partneShare = this.getLoanProduct().getPartnerInterestRate();
        final BigDecimal interestRate = this.getLoanProductRelatedDetail().getAnnualNominalInterestRate();
        final BigDecimal selfInterestRate = interestRate.divide(BigDecimal.valueOf(100)).multiply(selfShare);
        final BigDecimal partnerInterestRate = interestRate.subtract(selfInterestRate);
        final BigDecimal annualNominalInterestRate = this.getLoanProductRelatedDetail().getAnnualNominalInterestRate();
        final BigDecimal interestPaid = installment.getInterestPaid();
        final BigDecimal advanceAmount = installment.getAdvanceAmount();
        BigDecimal principalOutstanding = LocalDate.now().equals(paymentDate) ? this.getLoanSummary().getTotalPrincipalOutstanding().subtract(interestPaid).subtract(advanceAmount) : this.getLoanSummary().getTotalPrincipalOutstanding().subtract(advanceAmount);
        if (!installment.isObligationsMet()) {
            principalOutstanding = principalOutstanding.subtract(calculationOfPrincipalOutstandingForBackdateForeclosureEvent(installment.getInstallmentNumber() - 1));
        }
        BigDecimal beforeRoundInterest = BigDecimal.valueOf(calculateForeclosureInterestForDays(this.getLoanProductRelatedDetail().getDaysInYearType(), principalOutstanding, this.getLoanProductRelatedDetail().getAnnualNominalInterestRate(), tillDays, paymentDate));
        interestForCurrentPeriod = Money.emiRound(this.getCurrency(), beforeRoundInterest, this.getLoanProduct().getInterestDecimal(), this.getLoanProduct().getInterestRoundingMode(), null);
        //interestForCurrentPeriod = Money.roundingOff(getCurrency(),RoundingMode.valueOf(this.roundingMode));
        double selfInterest = interestForCurrentPeriod.getAmount().doubleValue() * (selfInterestRate.doubleValue() / annualNominalInterestRate.doubleValue()) * (selfSplitShare.doubleValue() / 100);
        selfInterestForCurrentPeriod = Money.roundingOff(getCurrency(), BigDecimal.valueOf(selfInterest), RoundingMode.valueOf(this.roundingMode));
        partnerInterestForCurrentPeriod = Money.roundingOff(getCurrency(), BigDecimal.valueOf(interestForCurrentPeriod.getAmount().doubleValue() - selfInterestForCurrentPeriod.getAmount().doubleValue()), RoundingMode.valueOf(this.roundingMode));
        ...
        balances[3] = selfInterestForCurrentPeriod;
        balances[4] = partnerInterestForCurrentPeriod;
        balances[5] = feeAccountedForCurrentPeriod;
        balances[6] = penaltyAccoutedForCurrentPeriod;
        balances[7] = interestAccountedForCurrentPeriod;

        return balances;
    }

    private BigDecimal calculationOfPrincipalOutstandingForBackdateForeclosureEvent(Integer installment) {
        if (installment > 0) {
            LoanRepaymentScheduleInstallment loanRepaymentScheduleInstallment = fetchRepaymentScheduleInstallment(installment);
            if (!loanRepaymentScheduleInstallment.isObligationsMet() && loanRepaymentScheduleInstallment.getInstallmentNumber() >= 1) {
                return loanRepaymentScheduleInstallment.getPrincipal().add(calculationOfPrincipalOutstandingForBackdateForeclosureEvent(loanRepaymentScheduleInstallment.getInstallmentNumber() - 1));
            }
        }
    ...
    public Money[] retriveIncomeForOverlappingPeriod(final LocalDate paymentDate) {
        Money[] balances = new Money[6];
        final MonetaryCurrency currency = getCurrency();
        balances[0] = balances[1] = balances[2] = balances[3] = balances[4] = Money.zero(currency);
        for (final LoanRepaymentScheduleInstallment installment : this.repaymentScheduleInstallments) {
            if (installment.getDueDate().isEqual(paymentDate) && !installment.isObligationsMet()) {
                ...
                Money selfInterestCharged = installment.getSelfInterestCharged(currency);
                Money partnerInterestCharged = installment.getPartnerInterestCharged(currency);
                balances[0] = interest;
                balances[1] = fee;
                balances[2] = penalty;
                balances[3] = selfInterestCharged;
                balances[4] = partnerInterestCharged;
                break;
    ...
    public double calculateForeclosureInterestForDays(int daysInYear, BigDecimal totalPrincipalOutstanding, BigDecimal annualNominalInterestRate, int days, LocalDate foreclosureDate) {
        if (totalPrincipalOutstanding.doubleValue() == 0) {
            return 0;
        }
        ...
        if (daysInYearType.isActual()) {
            return totalPrincipalOutstanding.doubleValue() * (annualNominalInterestRate.doubleValue() / 100) * days / foreclosureDate.lengthOfYear();
        }

        return totalPrincipalOutstanding.doubleValue() * (annualNominalInterestRate.doubleValue() / 100) * days / daysInYearValue;
    }
    ...
    public ChangedTransactionDetail handleForeClosureTransactions(final LoanTransaction repaymentTransaction,
                                                                  ...
                                                                  final AppUser appUser, LoanHistoryRepo loanHistoryRepo) {
    ...
    public void validateForForeclosure(final LocalDate transactionDate) {
        ProductCollectionConfig productCollectionConfig = this.getLoanProduct().getProductCollectionConfig();
        if(isForeclosureUnderCoolingPeriod(transactionDate,this.getDisbursementDate(),productCollectionConfig)){
            final String defaultUserMessage = "Foreclosure is not allowed, because client loan is under cooling period";
    ...
    public void updateInstallmentsPostDate(LocalDate transactionDate, LoanRepaymentScheduleInstallment foreclosureInstallment,
                                           ForeclosureEnum foreclosureEnum, LoanAccountDomainServiceJpa loanAccountDomainServiceJpa, LoanRepaymentScheduleInstallment currentInstallment,ProductCollectionConfig productCollectionConfig) {

        /* Added to subtract for remaining installment */
      ...
      /*  Money overDueInterest = ForeclosureUtils.retrieveOverDueInterest(this.repaymentScheduleInstallments,transactionDate,currency);
        Money overDueSelfInterest = ForeclosureUtils.retrieveOverDueSelfInterest(this.repaymentScheduleInstallments,transactionDate,currency);
        Money overDuePartnerInterest = ForeclosureUtils.retrieveOverDuePartnerInterest(this.repaymentScheduleInstallments,transactionDate,currency);*/


        for (final LoanRepaymentScheduleInstallment installment : this.repaymentScheduleInstallments) {
            if ((!installment.getDueDate().isBefore(transactionDate) || ForeclosureUtils.isPrincipalOutstandingInterestOverdue(productCollectionConfig.getForeclosureMethodType())) && !installment.isObligationsMet()) {
                totalPrincipal = totalPrincipal.plus(installment.getPrincipalOutstanding(currency));
                totalSelfPrincipal = totalSelfPrincipal.plus(installment.getSelfPrincipalOutstanding(currency));
                totalPartnerPrincipal = totalPartnerPrincipal.plus(installment.getPartnerPrincipalOutstanding(currency));
                LoanChargeActions.reverseChargesOnForeclosure(installment);
        ...
        Money interestOutstanding = ForeclosureUtils.retrieveInterestOutstanding(newInstallments, currency);

        Money selfInterestOutstanding = ForeclosureUtils.retrieveSelfInterestOutstanding(newInstallments, currency);

        Money partnerInterestOutstanding = ForeclosureUtils.retrievePartnerInterestOutstanding(newInstallments, currency);

        loanAccountDomainServiceJpa.updateForeclosureInstallment(foreclosureInstallment, foreclosureEnum, transactionDate, interestOutstanding,
                selfInterestOutstanding, partnerInterestOutstanding, totalPrincipal, this.getCurrency(), newInstallments, totalSelfPrincipal.getAmount(), totalPartnerPrincipal, this, currentInstallment,productCollectionConfig);

        // Handling For OverDue Scenario Interest

         /* foreclosureInstallment.setInterestCharged(foreclosureEnum.isOverDueForeclosure(foreclosureEnum)
                 ? foreclosureInstallment.getInterestCharged().subtract(overDueInterest.getAmount()):
                 foreclosureInstallment.getInterestCharged());

        foreclosureInstallment.setSelfInterestCharged(foreclosureEnum.isOverDueForeclosure(foreclosureEnum)
                ? foreclosureInstallment.getSelfInterestCharged().subtract(overDueSelfInterest.getAmount()):
                foreclosureInstallment.getSelfInterestCharged());

        foreclosureInstallment.setPartnerInterestCharged(foreclosureEnum.isOverDueForeclosure(foreclosureEnum)
                ? foreclosureInstallment.getPartnerInterestCharged().subtract(overDuePartnerInterest.getAmount()):
                foreclosureInstallment.getPartnerInterestCharged());*/
        if (!foreclosureEnum.isBackdateEmiFullPaidBeforeDueDate(foreclosureEnum)) {
            newInstallments.add(foreclosureInstallment);
        }
        updateLoanScheduleOnForeclosure(newInstallments,foreclosureEnum);
    ...
    public void updateLoanScheduleOnForeclosure(final List<LoanRepaymentScheduleInstallment> installments,ForeclosureEnum foreclosureEnum) {

        /* Added to Avoid the foreign key constraint error while foreclosing the loan with advance Amount */
        Foreclosure.modifyTransactionMappingForAdvanceTransaction(installments, this.repaymentScheduleInstallments, this.loanTransactions, this.getCurrency(),foreclosureEnum);
        this.repaymentScheduleInstallments.clear();
        for (final LoanRepaymentScheduleInstallment installment : installments) {
            addLoanRepaymentScheduleInstallment(installment);
        }
    ...
    public void adjustNetDisbursalAmount(BigDecimal adjustedAmount) {
        //this.netDisbursalAmount = adjustedAmount.subtract(this.deriveSumTotalOfChargesDueAtDisbursement()).subtract(deriveSumTotalOfChargesDueAtDisbursement());
        BigDecimal brokenDisbursement = this.brokenInterestDerived;
        if (brokenDisbursement == null) {
            brokenDisbursement = BigDecimal.valueOf(0);
        } else {
            brokenDisbursement = this.brokenInterestDerived;

        }
        this.netDisbursalAmount = adjustedAmount.subtract(this.deriveSumTotalOfChargesDueAtDisbursement()).subtract(brokenDisbursement).subtract(this.summary.getTotalGstDueAtDisbursement());
    }

    public void updateXirrValue(BigDecimal xirr) {
        this.xirrValue = xirr;
    }

    public LoanRepaymentScheduleInstallment fetchInstallmentByDate(LocalDate localDate) {
        LoanRepaymentScheduleInstallment currentInstallment = null;

        for (LoanRepaymentScheduleInstallment loanRepaymentScheduleInstallment : this.getRepaymentScheduleInstallments()) {

            if ((localDate.isAfter(loanRepaymentScheduleInstallment.getFromDate()) && localDate.isBefore(loanRepaymentScheduleInstallment.getDueDate()))
                    || (localDate.equals(loanRepaymentScheduleInstallment.getFromDate())) || (localDate.equals(loanRepaymentScheduleInstallment.getDueDate()))) {
    ...
    public void updateGstForSelfAndPartnerInLoan(List<LoanCharge> loanCharges,AppUser currentUser,Boolean isDisbursement) {
        BigDecimal totalGst = BigDecimal.ZERO;
        BigDecimal selfGst = BigDecimal.ZERO;
        BigDecimal partnerGst = BigDecimal.ZERO;
        Iterator loancharge = loanCharges.iterator();
        while (loancharge.hasNext()) {
            LoanCharge loanCharge = (LoanCharge) loancharge.next();
            totalGst = totalGst.add(loanCharge.getTotalGst());
            selfGst = selfGst.add(loanCharge.getSelfGst());
            partnerGst = partnerGst.add(loanCharge.getPartnerGst());
            loanCharge.setCreatedDate(DateUtils.getDateOfTenant());
            loanCharge.setCreatedUser(currentUser);
         }
         this.summary = updateTotalSelfAndPartnerGstAmount(totalGst,selfGst,partnerGst,isDisbursement);

        }



    public void updateGstForSelfAndPartnerInLoanForBounce(BigDecimal totalGst,BigDecimal selfGst,BigDecimal partnerGst,Boolean isDisbursement) {
        this.summary = updateTotalSelfAndPartnerGstAmount(totalGst,selfGst,partnerGst,isDisbursement);

    }

    private LoanSummary updateTotalSelfAndPartnerGstAmount(BigDecimal totalGst, BigDecimal selfGst, BigDecimal partnerGst,Boolean isDisbursement) {


        if(this.summary!=null) {

            this.summary.updateGstAmout(totalGst, selfGst, partnerGst, isDisbursement);
        }

        return this.summary;
    }

    public Set<LoanPenalForeclosureCharges> getLoanPenalForeclosueCharges() {
        return loanPenalForeclosueCharges;
    }

    public void setLoanPenalForeclosueCharges(Set<LoanPenalForeclosureCharges> loanPenalForeclosureCharges) {
        this.loanPenalForeclosueCharges = loanPenalForeclosureCharges;
    }

    public void updatePenalForclosureCharge(Set<LoanPenalForeclosureCharges> loanPenalForeclosureCharges) {
        this.loanPenalForeclosueCharges = loanPenalForeclosureCharges;
    }


    public void addForclosureCharge(LoanCharge foreClosureloanCharge) {
        this.charges.add(foreClosureloanCharge);
    }

    public void validateInstallmentNumber(Integer installmentNumber) {

        final LoanRepaymentScheduleInstallment nextInstallment = this.fetchRepaymentScheduleInstallment(
                installmentNumber - 1);
    }

    public void updateNetdisbursalAmount(List<LoanCharge> possiblyModifedLoanCharges) {
        this.netDisbursalAmount = this.approvedPrincipal.subtract((deriveSumTotalOfChargesDueAtDisbursement())).subtract(deriveTotalGstAtDisbursement(possiblyModifedLoanCharges));

        this.netSelfDisbursalAmount = this.selfPrincipaAmount.subtract(deriveSumOfSelfTotalOfChargesDueAtDisbursement()).subtract(deriveTotalSelfGstAtDisbursement(possiblyModifedLoanCharges));
        if (this.loanProduct.getSelfPrincipalShare().doubleValue() == 100) {
            this.netPartnerDisbursalAmount = BigDecimal.ZERO;
        } else {
            this.netPartnerDisbursalAmount = this.partnerPrincipalAmount
                    .subtract(deriveSumOfPartnerTotalOfChargesDueAtDisbursement())
                    .subtract(deriveTotalPartnerGstAtDisbursement(possiblyModifedLoanCharges));
        }
    }

    public void updateDpdForBackDatedCase(LocalDate transactionDate, Integer installmentNumber) {

        LoanRepaymentScheduleInstallment installment = fetchRepaymentScheduleInstallment(installmentNumber);
        if (transactionDate.isAfter(installment.getDueDate())) {
            LocalDate date = installment.getDueDate();
            //int diff =  transactionDate.getDayOfMonth() - date.getDayOfMonth() ;
            long daysDiff = ChronoUnit.DAYS.between(date, transactionDate);
            installment.updateDpdNumberForValueDate(daysDiff);

        } else {
            installment.updateDpdNumberForValueDate(Integer.valueOf(0));
        }
        //int days = Days.daysBetween(date, transactionDate).getDays();
    }

    public LoanRepaymentScheduleInstallment getLatestNotPaidInstallment() {
        LoanRepaymentScheduleInstallment notPaidInstallment = null;
        for (LoanRepaymentScheduleInstallment loanRepaymentScheduleInstallment : this.repaymentScheduleInstallments) {
            if (loanRepaymentScheduleInstallment.isPartlyPaid() || loanRepaymentScheduleInstallment.isNotFullyPaidOff()) {
                notPaidInstallment = loanRepaymentScheduleInstallment;
                break;
            }
        }
        return notPaidInstallment;
    }

    public LoanRepaymentScheduleInstallment fetchLoanForeclosureDetailForTransactionScreen(LocalDate closureDate) {
        ...
        final LocalDate currentDate = DateUtils.getLocalDateOfTenant();
        BigDecimal advanceAmount = retrieveAdvanceAmount();
        return new LoanRepaymentScheduleInstallment(null, 0, currentDate, currentDate, totalPrincipal.getAmount().subtract(advanceAmount),
                receivables[0].getAmount(), receivables[1].getAmount(), receivables[2].getAmount(), false, compoundingDetails, selfPrincipal.getAmount(), partnerPrincipal.getAmount(), selfInterestCharged.getAmount(), partnerInterestCharged.getAmount(), selfDue.getAmount(), partnerDue.getAmount(), null, null, advanceAmount);
    }

    public void interestRateSplitCalculation(LoanProduct loanProduct, LoanApplicationTerms loanApplicationTerms) {

        this.selfAnnualNominalInterestRate = BigDecimal.valueOf(loanApplicationTerms.getAnnualNominalInterestRate().doubleValue() * (loanProduct.getSelfInterestRate().doubleValue() / 100));
        this.partnerAnnualNominalInterestRate = BigDecimal.valueOf(loanApplicationTerms.getAnnualNominalInterestRate().doubleValue() - selfAnnualNominalInterestRate.doubleValue());
    }
    ...
    public List<XirrHistoryDetails> getXirrHistoryDetails() {
        return xirrHistoryDetails;
    }

    public void setXirrHistoryDetails(List<XirrHistoryDetails> xirrHistoryDetails) {
        this.xirrHistoryDetails = xirrHistoryDetails;
    }

    public boolean isNewTransaction(LoanTransaction loanTransaction) {
        boolean val = false;
        for (LoanTransaction transaction : loanTransaction.getLoan().getLoanTransactions()) {
            if (transaction.getInterestPortion(loanTransaction.getLoan().getCurrency()).getAmount().doubleValue() == (loanTransaction.getInterestPortion(loanTransaction.getLoan().getCurrency())).getAmount().doubleValue() &&
                    transaction.getPrincipalPortion(loanTransaction.getLoan().getCurrency()).getAmount().doubleValue() == (loanTransaction.getPrincipalPortion(loanTransaction.getLoan().getCurrency())).getAmount().doubleValue()
                    && loanTransaction.getValueDate().equals(transaction.getValueDate()) && transaction.getAmount().doubleValue() == loanTransaction.getAmount().doubleValue()) {
                val = true;
                break;
            }
        }
    ...
    public void updateLoanTransaction(LoanTransaction loanTransaction) {
        this.loanTransactions.remove(loanTransaction);
    }

    public void LoanSummaryDerivedFields() {
        updateLoanSummaryDerivedFields();
    }

    public void doPostLoanTransactionChecksAfterForeclosed(LocalDate transactionDate, LoanLifecycleStateMachine loanLifecycleStateMachine) {
        // Written a Wrapper Function Over A doPostLoanTransactionChecks
        doPostLoanTransactionChecks(transactionDate, loanLifecycleStateMachine);
    }

    public boolean isForeclosureUnderCoolingPeriod(LocalDate transactionDate,LocalDate disbursementDate,ProductCollectionConfig productCollectionConfig){
        long difference = ChronoUnit.DAYS.between(disbursementDate,transactionDate);
        Integer coolingOffDays = productCollectionConfig.getCoolingOffThresholdDays();
        return !Objects.isNull(coolingOffDays) && difference >= 0 && difference <= coolingOffDays;
    }
	 public void updateCoolingOffInstallment(LoanRepaymentScheduleInstallment loanRepaymentScheduleInstallment) {
        this.repaymentScheduleInstallments.forEach(LoanChargeActions::reverseChargesOnForeclosure);
        this.repaymentScheduleInstallments.clear();
        addLoanRepaymentScheduleInstallment(loanRepaymentScheduleInstallment);
    }

    public boolean isForeclosed() {
        return status().isForeclosed();
    }

    public boolean isLoanCoolingOff() {
        return status().isLoanCoolingOff();
    }
}
