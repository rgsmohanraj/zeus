...
public final class OfficeCommandFromApiJsonDeserializer {
    ...
    private final Set<String> supportedParameters = new HashSet<>(
            Arrays.asList("name", "parentId", "openingDate", "externalId", "locale", "dateFormat","limit","expiryDate"));
    ...
    public void validateForCreate(final String json) {
        ...
        if (this.fromApiJsonHelper.parameterExists("limit", element)) {
            final BigDecimal limit = this.fromApiJsonHelper.extractBigDecimalWithLocaleNamed("limit", element);
            baseDataValidator.reset().parameter("limit").value(limit).notNull().positiveAmount();
        }

        final LocalDate expiryDate = this.fromApiJsonHelper.extractLocalDateNamed("expiryDate", element);
        baseDataValidator.reset().parameter("expiryDate").value(expiryDate).notNull();
