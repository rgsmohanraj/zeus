...
import org.vcpl.lms.organisation.workingdays.data.AdjustedDateDetailsDTO;
import org.vcpl.lms.organisation.workingdays.domain.RepaymentRescheduleType;
...
import org.vcpl.lms.portfolio.loanaccount.loanschedule.data.LoanScheduleParams;
import org.vcpl.lms.portfolio.loanaccount.loanschedule.exception.MultiDisbursementEmiAmountException;
import org.vcpl.lms.portfolio.loanaccount.loanschedule.exception.MultiDisbursementOutstandingAmoutException;
import org.vcpl.lms.portfolio.loanaccount.loanschedule.exception.ScheduleDateException;
...
public abstract class AbstractLoanScheduleGenerator implements LoanScheduleGenerator {
    ...
    public LoanScheduleModel generate(final MathContext mc, final LoanApplicationTerms loanApplicationTerms,
                                      final List<LoanCharge> loanCharges, final HolidayDetailDTO holidayDetailDTO) {
    ...
    private LoanScheduleModel generate(final MathContext mc, final LoanApplicationTerms loanApplicationTerms,
        ...
        final LoanProduct loanProduct = loanApplicationTerms.getLoanProduct();
        ...
        final BigDecimal selfChargesDueAtTimeOfDisbursement = deriveTotalSelfChargesDueAtTimeOfDisbursement(loanCharges);
        final BigDecimal partnerChargesDueAtTimeOfDisbursement = deriveTotalPartnerChargesDueAtTimeOfDisbursement(loanCharges);
        ...
        if (!scheduleParams.isPartialUpdate()) {
            periods = createNewLoanScheduleListWithDisbursementDetails(numberOfRepayments, loanApplicationTerms,
                    chargesDueAtTimeOfDisbursement,selfPrincipalAmount,partnerPrincipalAmount,selfChargesDueAtTimeOfDisbursement,partnerChargesDueAtTimeOfDisbursement);
        }
        ...
        //to get the orginal principal amount

        final Money principaltobeScheduled =scheduleParams.getPrincipalToBeScheduled();

      //  final BigDecimal netDisbursalAmount= loanApplication.getNetDisbursalAmount();
        ...
        loanApplicationTerms.setFirstDisbursementDate(firstRepaymentdate);
        ...
        if (!scheduleParams.isPartialUpdate()) {
            ...
            if (loanApplicationTerms.isMultiDisburseLoan()) {
                ...
                BigDecimal disburseAmt = getDisbursementAmount(loanApplicationTerms, scheduleParams.getPeriodStartDate(), periods,
                        ...
                        selfPrincipalAmount,partnerPrincipalAmount,selfChargesDueAtTimeOfDisbursement,partnerChargesDueAtTimeOfDisbursement);
        ...
        final List<LoanCharge> nonCompoundingCharges = seperateTotalCompoundingPercentageCharges(loanCharges);
        ...
        while (!scheduleParams.getOutstandingBalance().isZero() || !scheduleParams.getDisburseDetailMap().isEmpty()) {
            ...
            if (loanApplicationTerms.isMultiDisburseLoan()) {
                updateBalanceBasedOnDisbursement(loanApplicationTerms, chargesDueAtTimeOfDisbursement, scheduleParams, periods,
                        scheduledDueDate,selfPrincipalAmount,partnerPrincipalAmount,selfChargesDueAtTimeOfDisbursement,partnerChargesDueAtTimeOfDisbursement);
            }
            ...
            handleRecalculationForNonDueDateTransactions(mc, loanApplicationTerms, loanCharges, holidayDetailDTO, scheduleParams, periods,
                    ...
                    periodStartDateApplicableForInterest, applicableTransactions, currentPeriodParams, numberOfRepayments, principaltobeScheduled);
            ...
            PrincipalInterest principalInterestForThisPeriod = calculatePrincipalInterestComponentsForPeriod(
                    ...
                    scheduleParams.getCompoundingMap(), periodStartDateApplicableForInterest, scheduledDueDate, interestRates, numberOfRepayments, principaltobeScheduled);
...
//            }
            ...
            if (!loanProduct.enableColendingLoan()){
                ...
                LoanScheduleModelPeriod installment = LoanScheduleModelRepaymentPeriod.repayment(scheduleParams.getInstalmentNumber(),
                        ...
                        totalInstallmentDue, !isCompletePeriod,scheduleParams.getTotalCumulativeSelfPrincipal(),scheduleParams.getTotalCumulativePartnerPrincipal(),scheduleParams.getTotalCumulativeSelfInterestCharged(),scheduleParams.getTotalCumulativePartnerInterestCharged(),scheduleParams.getSelfDue(),scheduleParams.getPartnerDue());
                ...
                installment = handleRecalculationForTransactions(mc, loanApplicationTerms, holidayDetailDTO, currency, scheduleParams,
                        ...
                        lastTotalOutstandingInterestPaymentDueToGrace, installment, loanCharges, numberOfRepayments, principaltobeScheduled);
            ...
            } else if(loanProduct.enableColendingLoan())
            {
                ...
                SplitData repaymentSplit = addLoanSplitRepaymentScheduleInstallment(principalInterestForThisPeriod, loanProduct, null, null,mc);

                final Money partnerPrincipal = repaymentSplit.getPartnerPrincipal();
                final Money partnerInterestCharged = repaymentSplit.getPartnerInterestCharged();
                final Money selfPrincipal = repaymentSplit.getSelfPrincipal();
                final Money selfInterestCharged = repaymentSplit.getSelfInterestCharged();
                final Money selfDue = repaymentSplit.getSelfDue();
                final Money partnerDue = repaymentSplit.getPartnerDue();
                ...
                LoanScheduleModelPeriod installment = LoanScheduleModelRepaymentPeriod.repayment(scheduleParams.getInstalmentNumber(),
                        ...
                        scheduleParams.getOutstandingBalance(), currentPeriodParams.getInterestForThisPeriod(),
                        currentPeriodParams.getFeeChargesForInstallment(), currentPeriodParams.getPenaltyChargesForInstallment(),
                        totalInstallmentDue, !isCompletePeriod, selfPrincipal, partnerPrincipal, selfInterestCharged, partnerInterestCharged, selfDue, partnerDue);


                if (principalInterestForThisPeriod.getRescheduleInterestPortion() != null)
                    installment.setRescheduleInterestPortion(principalInterestForThisPeriod.getRescheduleInterestPortion().getAmount());

                addLoanRepaymentScheduleInstallment(scheduleParams.getInstallments(), installment, loanProduct);
        ...
        if (scheduleParams.applyInterestRecalculation() && scheduleParams.getLatePaymentMap().size() > 0
            ...
            Money totalInterest = addInterestOnlyRepaymentScheduleForCurrentdate(mc, loanApplicationTerms, holidayDetailDTO, currency,
                    periods, currentDate, loanRepaymentScheduleTransactionProcessor, transactions, loanCharges, scheduleParams, numberOfRepayments, principaltobeScheduled);
...
//        final BigDecimal totalSelfPrincipal = BigDecimal.ZERO;
//        final BigDecimal totalPartnerPrincipal = BigDecimal.ZERO;
//        final BigDecimal totalSelfInterestCharged = BigDecimal.ZERO;
//        final BigDecimal totalPartnerInterestCharged = BigDecimal.ZERO;
        ...
        return LoanScheduleModel.from(periods, applicationCurrency, scheduleParams.getLoanTermInDays(),
                ...
                totalOutstanding,loanProduct,scheduleParams.getTotalSelfRepaymentExpected().getAmount(),
                scheduleParams.getTotalPartnerRepaymentExpected().getAmount(),null,
                null,null,null,
                null,null,null,null,null,null);
    }

    private BigDecimal deriveTotalPartnerChargesDueAtTimeOfDisbursement(List<LoanCharge> loanCharges) {

        BigDecimal chargesDueAtTimeOfDisbursement = BigDecimal.ZERO;
        for (final LoanCharge loanCharge : loanCharges) {
            if (loanCharge.isDueAtDisbursement()) {
                chargesDueAtTimeOfDisbursement = chargesDueAtTimeOfDisbursement.add(loanCharge.partnerAmount());
            }
        }
        return chargesDueAtTimeOfDisbursement;
    }

    private BigDecimal deriveTotalSelfChargesDueAtTimeOfDisbursement(List<LoanCharge> loanCharges) {

        BigDecimal chargesDueAtTimeOfDisbursement = BigDecimal.ZERO;
        for (final LoanCharge loanCharge : loanCharges) {
            if (loanCharge.isDueAtDisbursement()) {
                chargesDueAtTimeOfDisbursement = chargesDueAtTimeOfDisbursement.add(loanCharge.selfAmount());
            }
        }
        return chargesDueAtTimeOfDisbursement;
    }

    private SplitData addLoanSplitRepaymentScheduleInstallment(PrincipalInterest principalInterestForThisPeriod, LoanProduct loanProduct,List<LoanRepaymentScheduleInstallment> repaymentScheduleInstallments,MonetaryCurrency currency,final MathContext mc) {
        ...
        final BigDecimal selfInterestRate = loanProduct.getSelfInterestRate();
        final BigDecimal partnerInterestRate = loanProduct.getPartnerInterestRate();
        final BigDecimal interestRate = loanProduct.getInterestRate();

        final Money selfPrincipal = calculatePrincipal(principalInterestForThisPeriod.principal(), selfPrincipalShare);
        final Money partnerPrincipal = calculatePrincipal(principalInterestForThisPeriod.principal(),partnerPrincipalShare);
        // As of now assigning the interest with self interest Need to impl for  split at source
       // final Money selfInterestCharged = calculateSelfInterest(principalInterestForThisPeriod.interest(),selfPrincipalShare,selfInterestRate,interestRate);
        final Money selfInterestCharged = principalInterestForThisPeriod.interest();
        final Money partnerInterestCharged = calculatePartnerInterest(principalInterestForThisPeriod.interest(),selfInterestCharged);
        final Money selfDue = calculateDue(selfPrincipal,selfInterestCharged);
        final Money partnerDue = calculateDue(partnerPrincipal,partnerInterestCharged);


        SplitData SplitData = new SplitData(selfPrincipal,partnerPrincipal, selfInterestCharged,  partnerInterestCharged,selfDue,partnerDue);

        return SplitData;

    }

//    private SplitData addLoanSplitRepaymentScheduleInstallment(PrincipalInterest principalInterestForThisPeriod LoanProduct loanProduct) {
//
//        final Integer selfPrincipalShare = loanProduct.getSelfPrincipalShare();
//        final Integer partnerPrincipalShare = loanProduct.getPartnerPrincipalShare();
//
//        final Integer selfInterestRate = loanProduct.getSelfInterestRate();
//        final Integer partnerInterestRate = loanProduct.getPartnerInterestRate();
//        final Integer interestRate = loanProduct.getInterestRate();
//        scheduledLoanInstallment.selfInterestCharged();
//
//        final Money selfPrincipal = calculatePrincipal(scheduledLoanInstallment.principalDue(), selfPrincipalShare);
//        final Money selfInterestCharged = calculateInterest(scheduledLoanInstallment.interestDue(),selfInterestRate,selfPrincipalShare,interestRate);
//        final Money partnerPrincipal = calculatePrincipal(scheduledLoanInstallment.principalDue(),partnerPrincipalShare);
//        final Money partnerInterestCharged = calculateInterest(scheduledLoanInstallment.interestDue(),partnerInterestRate,partnerPrincipalShare,interestRate);
//
//        SplitData SplitData = new SplitData(selfPrincipal, selfInterestCharged, partnerPrincipal, partnerInterestCharged);
//
//
//
//
...
//    }
    ...
    public Money calculateDue(Money selfPrincipal, Money selfInterest){

        return selfPrincipal.add(selfInterest) ; }

//    public Money calculateTotalDue(final List<LoanRepaymentScheduleInstallment> repaymentScheduleInstallments,
//                                                final MonetaryCurrency currency) {
//        Money totalDue = Money.zero(currency);
//        for (final LoanRepaymentScheduleInstallment installment : repaymentScheduleInstallments) {
//            totalDue = totalDue.plus(installment.getSelfDue(currency));
//        }
//        return totalDue;
//    }
    ...
    public Money calculatePrincipal(Money principal, Integer principalShare){

        final BigDecimal divisor = BigDecimal.valueOf(Double.parseDouble("100.0"));

     //   return principal.multiplyRetainScale(principalShare,mc.getRoundingMode()).dividedBy(divisor,mc.getRoundingMode());}

       return principal.multipliedBy(principalShare).dividedBy(divisor,RoundingMode.HALF_EVEN); }


    public Money calculateSelfInterest(Money interestCharged, Integer principalShare,BigDecimal interestShare,BigDecimal interestRate){
        final BigDecimal divisor = BigDecimal.valueOf(Double.parseDouble("100.0"));

        // return (interestCharged.multipliedBy(principalShare).dividedBy(divisor,RoundingMode.HALF_EVEN)).multipliedBy(interestShare).dividedBy(divisor,RoundingMode.HALF_EVEN).dividedBy(interestRate).dividedBy(divisor,RoundingMode.HALF_EVEN));

        final BigDecimal selfps = BigDecimal.valueOf(principalShare).divide(divisor);

        final BigDecimal selfIR = (interestShare).divide(divisor);
        final BigDecimal overallIR = (interestRate).divide(divisor);

        final BigDecimal interestShared = selfIR.divide(overallIR, RoundingMode.HALF_EVEN);
//
//        return interestCharged.multipliedBy(BigDecimal.valueOf(principalShare).divide(divisor,mc.getRoundingMode()).multiply(interestShare)
//                .divide(divisor,mc.getRoundingMode()).divide(interestRate,mc.getRoundingMode()).divide(divisor,mc.getRoundingMode()));



        return interestCharged.multipliedBy(BigDecimal.valueOf(principalShare).divide(divisor))
                .multipliedBy((interestShare).divide(divisor))
                .dividedBy((interestRate).divide(divisor), RoundingMode.HALF_EVEN);



//        return ((interestCharged.multipliedBy(principalShare).dividedBy(divisor,RoundingMode.HALF_EVEN)).multipliedBy(interestShare).dividedBy(divisor,RoundingMode.HALF_EVEN).dividedBy(interestRate).dividedBy(divisor,RoundingMode.HALF_EVEN));

//return (((interestCharged.multipliedBy(principalShare).multipliedBy(interestShare)).dividedBy(divisor,RoundingMode.CEILING)).dividedBy(interestRate).dividedBy(divisor,RoundingMode.CEILING));
//        return interestCharged.multipliedBy(principalShare).multipliedBy(interestShare).dividedBy(interestRate);
    }

    public Money calculatePartnerInterest(Money interestCharged, Money selfInterestCharged){
        final BigDecimal divisor = BigDecimal.valueOf(Double.parseDouble("100.0"));

        // return (interestCharged.multipliedBy(principalShare).dividedBy(divisor,RoundingMode.HALF_EVEN)).multipliedBy(interestShare).dividedBy(divisor,RoundingMode.HALF_EVEN).dividedBy(interestRate).dividedBy(divisor,RoundingMode.HALF_EVEN));

        final Money partnerInterest = interestCharged.minus(selfInterestCharged);

//        return interestCharged.multipliedBy(selfps).multipliedBy(interestShared);

        return partnerInterest;

//        return interestCharged.multipliedBy(BigDecimal.valueOf(principalShare).divide(divisor))
//                .multipliedBy(BigDecimal.valueOf(interestShare).divide(divisor))
//                .dividedBy(BigDecimal.valueOf(interestRate).divide(divisor), RoundingMode.HALF_EVEN);
//
...
//        return interestCharged.multipliedBy(principalShare).multipliedBy(interestShare).dividedBy(interestRate);
    ...
    private void applyChargesForCurrentPeriod(final List<LoanCharge> loanCharges, final MonetaryCurrency currency,
    ...
    private void updatePeriodsWithCharges(final MonetaryCurrency currency, LoanScheduleParams scheduleParams,
                                          final Collection<LoanScheduleModelPeriod> periods, final List<LoanCharge> nonCompoundingCharges) {
        for (LoanScheduleModelPeriod loanScheduleModelPeriod : periods) {
    ...
    private LoanScheduleModelPeriod handleRecalculationForTransactions(final MathContext mc,
                                                                       ...
                                                                       final LoanScheduleModelPeriod installment, List<LoanCharge> loanCharges,final int numberOfRepayments,final Money principaltobeScheduled) {
            ...
            for (RecalculationDetail detail : applicableTransactions) {
                if (!detail.isProcessed()) {
                    ...
                    Money unprocessed = loanRepaymentScheduleTransactionProcessor.handleRepaymentSchedule(currentTransactions, currency,
                            scheduleParams.getInstallments(),null);
                    ...
                    if (unprocessed.isGreaterThanZero()) {
                        ...
                        modifiedInstallment = handlePrepaymentOfLoan(mc, loanApplicationTerms, holidayDetailDTO, scheduleParams,
                                ...
                                lastTotalOutstandingInterestPaymentDueToGrace, transactionDate, modifiedInstallment, loanCharges, numberOfRepayments, principaltobeScheduled);
    ...
    private LoanScheduleModelPeriod handlePrepaymentOfLoan(final MathContext mc, final LoanApplicationTerms loanApplicationTerms,
                                                           ...
                                                           final ScheduleCurrentPeriodParams currentPeriodParams, final Money lastTotalOutstandingInterestPaymentDueToGrace,
                                                           final LocalDate transactionDate, final LoanScheduleModelPeriod installment, List<LoanCharge> loanCharges,final int numberOfRepayments,final Money principaltobeScheduled) {
        LoanScheduleModelPeriod modifiedInstallment = installment;
        ...
        if (!oustanding.isGreaterThan(currentPeriodParams.getInterestForThisPeriod()) && !scheduledDueDate.equals(transactionDate)) {
            ...
            PrincipalInterest interestTillDate = calculatePrincipalInterestComponentsForPeriod(this.paymentPeriodsInOneYearCalculator,
                    ...
                    periodStartDateApplicableForInterest, calculateTill, interestRates, numberOfRepayments,principaltobeScheduled);
            ...
            if (scheduleParams.getOutstandingBalance().minus(diff).isGreaterThanZero()) {
                ...
                modifiedInstallment = LoanScheduleModelRepaymentPeriod.repayment(scheduleParams.getInstalmentNumber(),
                        ...
                        scheduleParams.getOutstandingBalance(), currentPeriodParams.getInterestForThisPeriod(),
                        currentPeriodParams.getFeeChargesForInstallment(), currentPeriodParams.getPenaltyChargesForInstallment(),
                        currentPeriodParams.fetchTotalAmountForPeriod(), false,scheduleParams.getTotalCumulativeSelfPrincipal(),scheduleParams.getTotalCumulativePartnerPrincipal(),scheduleParams.getTotalCumulativeSelfInterestCharged(),scheduleParams.getTotalCumulativePartnerInterestCharged(),scheduleParams.getSelfDue(),scheduleParams.getPartnerDue());
    ...
    private void handleRecalculationForNonDueDateTransactions(final MathContext mc, final LoanApplicationTerms loanApplicationTerms,
                                                              final List<LoanCharge> loanCharges, final HolidayDetailDTO holidayDetailDTO, LoanScheduleParams scheduleParams,
                                                              ...
                                                              final Collection<RecalculationDetail> applicableTransactions, final ScheduleCurrentPeriodParams currentPeriodParams,final int numberOfRepayments,final Money principaltobeScheduled) {
                ...
                if (transactionDate.isBefore(scheduledDueDate)) {
                    if (scheduleParams.getLoanRepaymentScheduleTransactionProcessor() != null && scheduleParams
                        ...
                        if (!transactionDate.isEqual(scheduleParams.getPeriodStartDate()) || scheduleParams.getInstalmentNumber() == 1) {
                            ...
                            if (daysInPeriodApplicable > 0) {
                                ...
                                PrincipalInterest principalInterestForThisPeriod = calculatePrincipalInterestComponentsForPeriod(
                                        ...
                                        interestRates, numberOfRepayments, principaltobeScheduled);
                            ...
                            installment = LoanScheduleModelRepaymentPeriod.repayment(scheduleParams.getInstalmentNumber(),
                                    ...
                                    penaltyChargesForInstallment, totalInstallmentDue, true,scheduleParams.getTotalCumulativeSelfPrincipal(),scheduleParams.getTotalCumulativePartnerPrincipal(),scheduleParams.getTotalCumulativeSelfInterestCharged(),scheduleParams.getTotalCumulativePartnerInterestCharged(),scheduleParams.getSelfDue(),scheduleParams.getPartnerDue());
                            periods.add(installment);
                            addLoanRepaymentScheduleInstallment(scheduleParams.getInstallments(), installment, loanProduct);
                            updateCompoundingMap(loanApplicationTerms, holidayDetailDTO, scheduleParams, lastRestDate, scheduledDueDate);
                        ...
                        Money unprocessed = scheduleParams.getLoanRepaymentScheduleTransactionProcessor()
                                .handleRepaymentSchedule(currentTransactions, currency, scheduleParams.getInstallments(),null);
                        if (unprocessed.isGreaterThanZero()) {
                        ...
                        if (applicableDate.isBefore(scheduledDueDate)) {
                            ...
                            if (unprocessed.isGreaterThanZero()) {
                                ...
                                if (!scheduleParams.getOutstandingBalance().isGreaterThan(unprocessed) && !loanApplicationTerms
                                    ...
                                    if (!principalInterestForThisPeriod.interest()
                                        ...
                                        LoanTransaction loanTransaction = LoanTransaction.repayment(null, unprocessed, null,
                                                transactionDate, null, DateUtils.getLocalDateTimeOfTenant(), null,null,null,null,null,null,null,null);
                                ...
                                LoanTransaction loanTransaction = LoanTransaction.repayment(null, unprocessed, null, scheduledDueDate, null,
                                        DateUtils.getLocalDateTimeOfTenant(), null,null,null,null,null,null,null,null);
    ...
    private void updateBalanceBasedOnDisbursement(final LoanApplicationTerms loanApplicationTerms,
                                                  ...
                                                  final BigDecimal selfDisbursedAmount,final BigDecimal partnerDisbursedAmount,final BigDecimal selfChargesDueAtTimeOfDisbursement,final BigDecimal partnerChargesDueAtTimeOfDisbursement) {
        for (Map.Entry<LocalDate, Money> disburseDetail : scheduleParams.getDisburseDetailMap().entrySet()) {
            if (disburseDetail.getKey().isAfter(scheduleParams.getPeriodStartDate())
                ...
                final LoanScheduleModelDisbursementPeriod disbursementPeriod = LoanScheduleModelDisbursementPeriod
                        .disbursement(disburseDetail.getKey(), disburseDetail.getValue(), chargesDueAtTimeOfDisbursement,selfDisbursedAmount,partnerDisbursedAmount,selfChargesDueAtTimeOfDisbursement,partnerChargesDueAtTimeOfDisbursement);
    ...
    private Money addInterestOnlyRepaymentScheduleForCurrentdate(final MathContext mc, final LoanApplicationTerms loanApplicationTerms,
                                                                 ...
                                                                 final Collection<RecalculationDetail> transactions, final List<LoanCharge> loanCharges, final LoanScheduleParams params,final int numberOfRepayments,final Money principaltobeScheduled) {
        ...
        do {
            ...
            for (RecalculationDetail detail : applicableTransactions) {
                ...
                if (!params.getPeriodStartDate().isEqual(transactionDate)) {
                    PrincipalInterest principalInterestForThisPeriod = calculatePrincipalInterestComponentsForPeriod(
                            ...
                            transactionDate, applicableVariations, numberOfRepayments, principaltobeScheduled);
                    ...
                    LoanScheduleModelRepaymentPeriod installment = LoanScheduleModelRepaymentPeriod.repayment(params.getInstalmentNumber(),
                            ...
                            totalInterest.zero(), totalInterest, true,totalSelfPrincipal,totalPartnerPrincipal,totalSelfInterestCharged,totalPartnerInterestCharged,selfDue,partnerDue);
                    ...
                    addLoanRepaymentScheduleInstallment(params.getInstallments(), installment, loanProduct);
                ...
                loanRepaymentScheduleTransactionProcessor.handleRepaymentSchedule(currentTransactions, currency, params.getInstallments(),null);
                updateLatePaidAmountsToPrincipalMap(detail.getTransaction(), loanApplicationTerms, currency, holidayDetailDTO, lastRestDate,
                        params);
            ...
            if (!outstanding.isZero()) {
                PrincipalInterest principalInterestForThisPeriod = calculatePrincipalInterestComponentsForPeriod(
                        ...
                        totalInterest.zero(), totalInterest.zero(), totalInterest.zero(), outstanding, loanApplicationTerms,
                        periodNumberTemp, mc, mergeVariationsToMap(params), params.getCompoundingMap(), params.getPeriodStartDate(),
                        params.getActualRepaymentDate(), applicableVariations, numberOfRepayments,  principaltobeScheduled);
                Money interest = principalInterestForThisPeriod.interest();
                totalInterest = totalInterest.plus(interest);
        ...
        if (totalInterest.isGreaterThanZero()) {
            LoanScheduleModelRepaymentPeriod installment = LoanScheduleModelRepaymentPeriod.repayment(params.getInstalmentNumber(),
                    ...
                    totalInterest.zero(), totalInterest.zero(), totalInterest, true,totalSelfPrincipal,totalPartnerPrincipal,totalSelfInterestCharged,totalPartnerInterestCharged,selfDue,partnerDue);
            params.incrementInstalmentNumber();
            periods.add(installment);
    ...
    private Money getIncomeForCompounding(final LoanApplicationTerms loanApplicationTerms, final MonetaryCurrency currency,
        ...
        switch (loanApplicationTerms.getInterestRecalculationCompoundingMethod()) {
            case INTEREST:
                ...
                selfInterestCharged = selfInterestCharged.plus(loanRepaymentScheduleInstallment.getSelfInterestCharged(currency));
                partnerInterestCharged = partnerInterestCharged.plus(loanRepaymentScheduleInstallment.getPartnerInterestCharged(currency));
                break;
            case FEE:
        ...
        amountCharged = interestCharged.plus(feeCharged).plus(penaltyCharged).plus(selfInterestCharged).plus(partnerInterestCharged);
    ...
    private void populateCompoundingDatesInPeriod(final LocalDate startDate, final LocalDate endDate,
                                                  ...
                                                  final LoanScheduleParams scheduleParams, final List<LoanCharge> charges, MonetaryCurrency currency) {
        if (loanApplicationTerms.getInterestRecalculationCompoundingMethod().isCompoundingEnabled()) {
    ...
    public abstract PrincipalInterest calculatePrincipalInterestComponentsForPeriod(PaymentPeriodsInOneYearCalculator calculator,
                                                                                    ...
                                                                                    Collection<LoanTermVariationsData> termVariations,final int numberOfRepayments,final Money principaltobeScheduled);
    ...
    private BigDecimal deriveTotalChargesDueAtTimeOfDisbursement(final List<LoanCharge> loanCharges) {
        BigDecimal chargesDueAtTimeOfDisbursement = BigDecimal.ZERO;
        for (final LoanCharge loanCharge : loanCharges) {
            if (loanCharge.isDueAtDisbursement()) {
                chargesDueAtTimeOfDisbursement = chargesDueAtTimeOfDisbursement.add(loanCharge.amount()).add(loanCharge.cgstAmount().add(loanCharge.sgstAmount()).add(loanCharge.igstAmount()));
            }
        }
        return chargesDueAtTimeOfDisbursement;
    }
        ...
        for (DisbursementData disbursementData : loanApplicationTerms.getDisbursementDatas()) {
            if (disbursementData.disbursementDate().equals(disbursementDate)) {
                final LoanScheduleModelDisbursementPeriod disbursementPeriod = LoanScheduleModelDisbursementPeriod.disbursement(
                        disbursementData.disbursementDate(), Money.of(currency, disbursementData.amount()), chargesDueAtTimeOfDisbursement,selfDisbursedAmount,partnerDisbursedAmount,selfChargesDueAtTimeOfDisbursement,partnerChargesDueAtTimeOfDisbursement);
                periods.add(disbursementPeriod);
    ...
    private Collection<LoanScheduleModelPeriod> createNewLoanScheduleListWithDisbursementDetails(final int numberOfRepayments,
        ...
        if (loanApplicationTerms.isMultiDisburseLoan()) {
            ...
            final LoanScheduleModelDisbursementPeriod disbursementPeriod = LoanScheduleModelDisbursementPeriod
                    .disbursement(loanApplicationTerms, chargesDueAtTimeOfDisbursement,selfDisbursedAmount,partnerDisbursedAmount,selfChargesDueAtTimeOfDisbursement,partnerChargesDueAtTimeOfDisbursement);
            periods.add(disbursementPeriod);
        }
    ...
    private List<LoanCharge> seperateTotalCompoundingPercentageCharges(final List<LoanCharge> loanCharges) {
        List<LoanCharge> interestCharges = new ArrayList<>();
        for (final LoanCharge loanCharge : loanCharges) {
    ...
    private Money cumulativePenaltyChargesDueWithin(final LocalDate periodStart, final LocalDate periodEnd,
                                                    final List<LoanCharge> loanCharges, final MonetaryCurrency monetaryCurrency,
    ...
    private LoanScheduleDTO rescheduleNextInstallments(final MathContext mc, final LoanApplicationTerms loanApplicationTerms, Loan loan,
       ...
       final BigDecimal selfDisbursedAmount= loan.getSelfPrincipaAmount();
        final BigDecimal partnerDisbursedAmount =loan.getPartnerPrincipalAmount();
        ...
        if (rescheduleFrom != null) {
            ...
            BigDecimal outstandingselfPrincipal=loan.getSelfPrincipaAmount();
            BigDecimal outstandingpartnerPrincipal=loan.getPartnerPrincipalAmount();
            ...
            final BigDecimal selfChargesDueAtTimeOfDisbursement = deriveTotalSelfChargesDueAtTimeOfDisbursement(loan.charges());
            final BigDecimal partnerChargesDueAtTimeOfDisbursement = deriveTotalPartnerChargesDueAtTimeOfDisbursement(loan.charges());
            periods = createNewLoanScheduleListWithDisbursementDetails(loanApplicationTerms.fetchNumberOfRepaymentsAfterExceptions(),
                    loanApplicationTerms, chargesDueAtTimeOfDisbursement,outstandingselfPrincipal,outstandingpartnerPrincipal,selfChargesDueAtTimeOfDisbursement,partnerChargesDueAtTimeOfDisbursement);
            ...
            Money totalCumulativeSelfPrincipal = principalToBeScheduled.zero();
            Money totalCumulativePartnerPrincipal = principalToBeScheduled.zero();

            Money totalCumulativeInterest = principalToBeScheduled.zero();
            Money totalCumulativeSelfInterestCharged = principalToBeScheduled.zero();
            Money totalCumulativePartnerInterestCharged = principalToBeScheduled.zero();
            ...
            Money totalSelfRepaymentExpected = principalToBeScheduled.zero();
            Money totalPartnerRepaymentExpected = principalToBeScheduled.zero();

            //Sample checking of split data
            ...
            if (loanApplicationTerms.isMultiDisburseLoan()) {
                ...
                BigDecimal disburseAmt = getDisbursementAmount(loanApplicationTerms, loanApplicationTerms.getExpectedDisbursementDate(),
                        periods, chargesDueAtTimeOfDisbursement, disburseDetailMap, true,null,null,selfChargesDueAtTimeOfDisbursement,partnerChargesDueAtTimeOfDisbursement);
            ...
            for (LoanRepaymentScheduleInstallment installment : processInstallmentsInstallments) {
                ...
                totalCumulativeSelfPrincipal = totalCumulativeSelfPrincipal.plus(installment.getSelfPrincipal(currency));
                totalCumulativePartnerPrincipal = totalCumulativePartnerPrincipal.plus(installment.getPartnerPrincipal(currency));
                totalCumulativeInterest = totalCumulativeInterest.plus(installment.getInterestCharged(currency));
                totalCumulativeSelfInterestCharged = totalCumulativeSelfInterestCharged.plus(installment.getSelfInterestCharged(currency));
                totalCumulativePartnerInterestCharged = totalCumulativePartnerInterestCharged.plus(installment.getPartnerInterestCharged(currency));
            ...
            totalSelfRepaymentExpected = totalCumulativeSelfPrincipal.plus(totalCumulativeSelfInterestCharged).plus(totalFeeChargesCharged)
                    .plus(totalPenaltyChargesCharged);
            totalPartnerRepaymentExpected = totalCumulativePartnerPrincipal.plus(totalCumulativePartnerInterestCharged).plus(totalFeeChargesCharged)
                    .plus(totalPenaltyChargesCharged);
            ...
            if (!newRepaymentScheduleInstallments.isEmpty() && totalCumulativeInterest.isGreaterThanZero()) {
                ...
                loanScheduleParams = LoanScheduleParams.createLoanScheduleParamsForPartialUpdate(periodNumber, instalmentNumber,
                        ...
                        currency, applyInterestRecalculation,totalSelfRepaymentExpected,totalPartnerRepaymentExpected);
        ...
        for (LoanScheduleModelPeriod loanScheduleModelPeriod : loanScheduleModel.getPeriods()) {
            if (loanScheduleModelPeriod.isRepaymentPeriod()) {
                ...
                addLoanRepaymentScheduleInstallment(retainedInstallments, loanScheduleModelPeriod, loanProduct);
            }
        }
        ...
        for (RecalculationDetail detail : applicableTransactions) {
            if (!detail.isProcessed()) {
                ...
                loanRepaymentScheduleTransactionProcessor.handleRepaymentSchedule(currentTransactions, currency,
                        newRepaymentScheduleInstallments,null);
    ...
    private LoanRepaymentScheduleInstallment addLoanRepaymentScheduleInstallment(final List<LoanRepaymentScheduleInstallment> installments,
                                                                                 final LoanScheduleModelPeriod scheduledLoanInstallment, LoanProduct loanProduct) {


        LoanRepaymentScheduleInstallment installment = null;
        if (scheduledLoanInstallment.isRepaymentPeriod() && !loanProduct.enableColendingLoan()) {
            installment = new LoanRepaymentScheduleInstallment(null, scheduledLoanInstallment.periodNumber(),
                    ...
                    scheduledLoanInstallment.rescheduleInterestPortion(),null,null,null,null,null,null);
            installments.add(installment);
        }
        else if (scheduledLoanInstallment.isRepaymentPeriod() && loanProduct.enableColendingLoan()) {
            final Integer selfPrincipalShare = loanProduct.getSelfPrincipalShare();
            final Integer partnerPrincipalShare = loanProduct.getPartnerPrincipalShare();
...
//            final BigDecimal selfPrincipal = calculatePrincipal(scheduledLoanInstallment.principalDue(), selfPrincipalShare);
//            final BigDecimal selfInterestCharged = calculateInterest(scheduledLoanInstallment.interestDue(),selfInterestRate,selfPrincipalShare,interestRate);
//            final BigDecimal partnerPrincipal = calculatePrincipal(scheduledLoanInstallment.principalDue(),partnerPrincipalShare);
//            final BigDecimal partnerInterestCharged = calculateInterest(scheduledLoanInstallment.interestDue(),partnerInterestRate,partnerPrincipalShare,interestRate);
...
//                    scheduledLoanInstallment.feeChargesDue(), scheduledLoanInstallment.penaltyChargesDue(),
//                    scheduledLoanInstallment.isRecalculatedInterestComponent(), scheduledLoanInstallment.getLoanCompoundingDetails(),
//                    scheduledLoanInstallment.rescheduleInterestPortion(),selfPrincipal,partnerPrincipal,selfInterestCharged,partnerInterestCharged);
//            installments.add(installment);
//        }
            return installment;
        }
/*
        ...
        if(loanProduct.enableColendingLoan()){
            final BigDecimal principal = this.fromApiJsonHelper
                    .extractBigDecimalWithLocaleNamed("principal", element);
            final Integer selfPrincipalShare = loanProduct.getSelfPrincipalShare();
            final Integer partnerPrincipalShare = loanProduct.getPartnerPrincipalShare();
            ...
            final BigDecimal interestCharged = this.fromApiJsonHelper.extractBigDecimalWithLocaleNamed("interestCharged",element);
            JsonObject jsonObj = deepCopy(element.getAsJsonObject(), JsonObject.class);

            BigDecimal selfPrincipal = calculateSelfPrincipal(principal, selfPrincipalShare);
            jsonObj.addProperty("principal", selfPrincipal);

            BigDecimal partnerPrincipal = calculatePartnerPrincipal(principal, partnerPrincipalShare);
            jsonObj.addProperty("principal", partnerPrincipal);

            BigDecimal selfInterestCharged = calculateSelfInterest(interestCharged, selfPrincipalShare,selfInterestRate,interestRate);
            jsonObj.addProperty("interestCharged", selfInterestCharged);

            BigDecimal partnerInterestCharged = calculatePartnerInterest(interestCharged, partnerPrincipalShare,partnerInterestRate,interestRate);
            jsonObj.addProperty("interestCharged", partnerInterestCharged);
        }
        if (scheduledLoanInstallment.isRepaymentPeriod()) {
            installment = new LoanRepaymentScheduleInstallment(null, scheduledLoanInstallment.periodNumber(),
                    ...
                    scheduledLoanInstallment.feeChargesDue(), scheduledLoanInstallment.penaltyChargesDue(),
                    scheduledLoanInstallment.isRecalculatedInterestComponent(), scheduledLoanInstallment.getLoanCompoundingDetails(),
                    scheduledLoanInstallment.rescheduleInterestPortion(),scheduledLoanInstallment.selfPrincipal(),scheduledLoanInstallment.partnerPrincipal(),scheduledLoanInstallment.selfInterestCharged(),scheduledLoanInstallment.partnerInterestCharged());
            installments.add(installment);
        }
*/
        return null;
    ...
    private LoanScheduleModelPeriod createLoanScheduleModelPeriod(final LoanRepaymentScheduleInstallment installment,
        ...
        LoanScheduleModelPeriod scheduledLoanInstallment = LoanScheduleModelRepaymentPeriod.repayment(installment.getInstallmentNumber(),
                ...
                installment.isRecalculatedInterestComponent(),installment.getSelfPrincipal(currency),installment.getPartnerPrincipal(currency),installment.getSelfInterestCharged(currency),installment.getPartnerInterestCharged(currency),installment.getSelfDue(currency),installment.getPartnerDue(currency));
    ...
    public LoanRepaymentScheduleInstallment calculatePrepaymentAmount(final MonetaryCurrency currency, final LocalDate onDate,
        ...
        loanRepaymentScheduleTransactionProcessor.handleTransaction(loanApplicationTerms.getExpectedDisbursementDate(), loanTransactions,
                currency, loanScheduleDTO.getInstallments(), loan.charges(),null);
