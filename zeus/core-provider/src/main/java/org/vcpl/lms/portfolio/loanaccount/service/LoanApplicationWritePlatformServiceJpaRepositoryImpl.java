...
import org.vcpl.lms.infrastructure.accountnumberformat.domain.AccountNumberFormatRepositoryWrapper;
import org.vcpl.lms.infrastructure.accountnumberformat.domain.EntityAccountType;
...
import org.vcpl.lms.infrastructure.configuration.domain.GlobalConfigurationProperty;
import org.vcpl.lms.infrastructure.configuration.domain.GlobalConfigurationRepositoryWrapper;
...
import org.vcpl.lms.infrastructure.entityaccess.FineractEntityAccessConstants;
import org.vcpl.lms.infrastructure.entityaccess.domain.FineractEntityAccessType;
import org.vcpl.lms.infrastructure.entityaccess.domain.FineractEntityRelation;
import org.vcpl.lms.infrastructure.entityaccess.domain.FineractEntityRelationRepository;
import org.vcpl.lms.infrastructure.entityaccess.domain.FineractEntityToEntityMapping;
import org.vcpl.lms.infrastructure.entityaccess.domain.FineractEntityToEntityMappingRepository;
import org.vcpl.lms.infrastructure.entityaccess.exception.NotOfficeSpecificProductException;
...
import org.vcpl.lms.portfolio.calendar.domain.CalendarFrequencyType;
...
import org.vcpl.lms.portfolio.calendar.exception.CalendarNotFoundException;
...
import org.vcpl.lms.portfolio.client.domain.AccountNumberGenerator;
...
import org.vcpl.lms.portfolio.collateralmanagement.domain.ClientCollateralManagementRepository;
...
import org.vcpl.lms.portfolio.group.exception.GroupMemberNotFoundInGSIMException;
...
import org.vcpl.lms.portfolio.loanaccount.exception.LoanApplicationNotInSubmittedAndPendingApprovalStateCannotBeDeleted;
import org.vcpl.lms.portfolio.loanaccount.exception.LoanApplicationNotInSubmittedAndPendingApprovalStateCannotBeModified;
...
import org.vcpl.lms.portfolio.loanaccount.serialization.LoanApplicationTransitionApiJsonValidator;
...
import org.vcpl.lms.portfolio.savings.data.GroupSavingsIndividualMonitoringAccountData;
...
import org.vcpl.lms.portfolio.savings.service.GSIMReadPlatformService;
...
public class LoanApplicationWritePlatformServiceJpaRepositoryImpl implements LoanApplicationWritePlatformService {
    ...
    public LoanApplicationWritePlatformServiceJpaRepositoryImpl(final PlatformSecurityContext context, final FromJsonHelper fromJsonHelper,
                                                                ...
                                                                final OfficeRepository officeRepository, final ClientRepositoryWrapper clientRepositoryWrapper,
                                                                final AddressReadPlatformService addressReadPlatformService, final FromJsonHelper fromApiJsonHelper,
                                                                final ChargeReadPlatformService chargeReadPlatformService,
                                                                final PartnerRepository partnerRepository, final OfficeReadPlatformServiceImpl officeReadPlatformServiceImpl, ServicerFeeRepository servicerFeeRepository, GstService gstService, RefreshRepaymentSchedule refreshRepaymentSchedule) {
    ...
    public CommandProcessingResult submitApplication(final JsonCommand command) {
        ...
        try {
           final AppUser currentUser = getAppUserIfPresent();
            ...
            final Partner currentPartner=loanProduct.getPartner();

            final  Long partnerId= currentPartner.getId();
            ...
            if(loanRepositoryWrapper.isExternalExistForPartner(externalId, partnerId)) {
                List<ApiParameterError> errors = new ArrayList<>();
                errors.add(ApiParameterError.generalError("validation.msg.externalId.already.exist.for.partner",
                        "External Id already exist for this Partner"));
            ...
            if (loanProduct.useBorrowerCycle()) {
                ...
                this.loanProductCommandFromApiJsonDeserializer.validateMinMaxConstraints(command.parsedJson(), baseDataValidator,
                        loanProduct, cycleNumber,partner);
            } else {
                this.loanProductCommandFromApiJsonDeserializer.validateMinMaxConstraints(command.parsedJson(), baseDataValidator,
                        loanProduct,partner);
            }
            ...
            final Loan newLoanApplication = this.loanAssembler.assembleFrom(command, currentUser,partnerId);
            ...
            if(Objects.nonNull(newLoanApplication)){
                newLoanApplication.setVclHurdleRate(newLoanApplication.getLoanProduct().isServicerFeeInterestConfigEnabled()? newLoanApplication.getLoanProduct().getServicerFeeConfig().getVclHurdleRate():BigDecimal.ZERO);
            }
            ...
            // Generation of account number based on partner

       //     newLoanApplication.updateAccountNo(this.accountNumberGenerator.generateAccountNumberByPartner(newLoanApplication));
            ...
            if (newLoanApplication.isAccountNumberRequiresAutoGeneration()) {
               ...
               //     newLoanApplication.updateAccountNo(this.accountNumberGenerator.generate(newLoanApplication, accountNumberFormat));
                     ...
                     */
                    newLoanApplication.setValueDate(Date.from(newLoanApplication.getSubmittedOnDate().atStartOfDay(ZoneId.systemDefault()).toInstant()));
                    this.loanRepositoryWrapper.saveAndFlush(newLoanApplication);
                }
            }
            ...
            // Save linked account informationN
    ...
    public CommandProcessingResult modifyApplication(final Long loanId, final JsonCommand command) {
        ...
        try {
            ...
            final Long partnerId=existingLoanApplication.getPartnerId();
            ...
            final List<LoanCharge> existingCharges = existingLoanApplication.charges();
            ...
            final Long clientIds = this.fromApiJsonHelper.extractLongNamed("clientId", command.parsedJson().getAsJsonObject());

            final List<Charge> charge =this.loanChargeAssembler.retrieveCharge(command.parsedJson().getAsJsonObject());

            final Client clientDetail=this.clientRepositoryWrapper.findOneWithNotFoundDetection(clientIds);
            Office office =clientDetail.getOffice();
            final Long officeId =office.getId();
			 // Added For calculation Of gst at The Time of Modifing The Loan Appliacation
            final List<GstData> gstData = this.gstService.calculationOfGst(clientIds,charge,principal,loanProductForValidations,element,null);
            /**
             * Stores all charges which are passed in during modify loan application
             **/

            final List<LoanCharge> possiblyModifedLoanCharges = this.loanChargeAssembler.fromParsedJson(command.parsedJson(),disbursementDetails,gstData);
            possiblyModifedLoanCharges.stream().filter(Objects::nonNull).forEach(loanCharge -> {
                loanCharge.setModifiedDate(DateUtils.getDateOfTenant());
                loanCharge.setModifiedUser(currentUser);
            });
            ...
            for (Charge retrievecharge : newTrancheChages) {
                existingLoanApplication.addTrancheLoanCharge(retrievecharge);
            ...
            final Set<LoanPenalForeclosureCharges> loanPenalForeclosureCharges =this.loanChargeAssembler.retrievePenaltyAndForeclosureCharge(existingLoanApplication,loanProductForValidations,element);
            ...
            if (newLoanProduct != null) {
                ...
                if (newLoanProduct.useBorrowerCycle()) {
                    ...
                    this.loanProductCommandFromApiJsonDeserializer.validateMinMaxConstraints(command.parsedJson(), baseDataValidator,
                            newLoanProduct, cycleNumber,partner);
                } else {
                    this.loanProductCommandFromApiJsonDeserializer.validateMinMaxConstraints(command.parsedJson(), baseDataValidator,
                            newLoanProduct,partner);
                }
            ...
            if(!loanPenalForeclosureCharges.isEmpty()){
                existingLoanApplication.updatePenalForclosureCharge(loanPenalForeclosureCharges);
            }
            ...
            if (changes.containsKey(chargesParamName)) {
                ...
                existingLoanApplication.updateNetdisbursalAmount(possiblyModifedLoanCharges);
            }
            LOG.info("NetDIsbursalAmount is Modified" + existingLoanApplication.getNetDisbursalAmount());
            if((existingLoanApplication.getNetDisbursalAmount().doubleValue() < 0) ||
                    (existingLoanApplication.getNetDisbursalAmount().doubleValue() == 0)) {
            ...
            if (changes.containsKey("recalculateLoanSchedule")) {
                ...
                final LoanScheduleModel loanSchedule = this.calculationPlatformService.calculateLoanSchedule(query, false,gstData);
            ...
            if (!isLinkedAccPresent) {
                final List<LoanCharge> charges = existingLoanApplication.charges();
    ...
    private OfficeGstData retrieveGstData(Collection<OfficeGstData> officeGstData, CodeValue stateCode) {
    ...
    public CommandProcessingResult approveApplication(final Long loanId, final JsonCommand command) {
        ...
        expectedDisbursementDate = command.localDateValueOfParameterNamed("approvedOnDate");
        ...
        refreshRepaymentSchedule.updateLoan(loan,expectedDisbursementDate);
        ...
        if (!changes.isEmpty()) {
             ...
             * For value date added
             */
            loan.setValueDate( Date.from(loan.getApprovedOnDate().atStartOfDay(ZoneId.systemDefault()).toInstant()));
    ...
    public CommandProcessingResult rejectApplication(final Long loanId, final JsonCommand command) {
        ...
        loan.setExternalId(loan.getExternalId() + REJECTED);
