...
import org.vcpl.lms.portfolio.loanaccount.exception.InvalidAmountOfCollateralQuantity;
...
public final class LoanApplicationCommandFromApiJsonHelper {
    ...
    final Set<String> supportedParameters = new HashSet<>(Arrays.asList(LoanApiConstants.dateFormatParameterName,
            ...
            LoanApiConstants.daysInYearTypeParameterName, LoanApiConstants.fixedPrincipalPercentagePerInstallmentParamName,LoanApiConstants.selfShare,
            LoanApiConstants.partnerShare,LoanApiConstants.foreclosureChargesParameterName,LoanApiConstants.overduechargeParameterName,LoanApiConstants.partnerName,
            LoanApiConstants.bounceChargeParameterName));
    ...
    public LoanApplicationCommandFromApiJsonHelper(final FromJsonHelper fromApiJsonHelper,
            ...
            final ClientCollateralManagementRepositoryWrapper clientCollateralManagementRepositoryWrapper, ChargeRepositoryWrapper chargeRepositoryWrapper) {
    ...
    public void validateForCreate(final String json, final boolean isMeetingMandatoryForJLGLoans, final LoanProduct loanProduct) {
        ...
        if (this.fromApiJsonHelper.parameterExists(externalIdParameterName, element)) {
            ...
            baseDataValidator.reset().parameter(externalIdParameterName).value(externalId)
                    .notNull()
        ...
        validateDisbursementDate(expectedDisbursementDate);
        ...
        if (this.fromApiJsonHelper.parameterExists(interestChargedFromDateParameterName, element)) {
            final LocalDate interestChargedFromDate = this.fromApiJsonHelper.extractLocalDateNamed(interestChargedFromDateParameterName,
                    element);
            baseDataValidator.reset().parameter(interestChargedFromDateParameterName).value(interestChargedFromDate).ignoreIfNull().isBefore(expectedDisbursementDate,baseDataValidator,interestChargedFromDate);
        }
        ...
        if (this.fromApiJsonHelper.parameterExists(createSiAtDisbursementParameterName, element)) {
            ...
            if(Objects.nonNull(createStandingInstructionAtDisbursement)){
        ...
        // charges
        List<Charge> duplicateCharge = new ArrayList<>();
        final String chargesParameterName = "charges,overdueCharges,foreclosureCharges,bounceCharge";
        chargeValidation(element,chargesParameterName,baseDataValidator,duplicateCharge);
        if(!duplicateCharge.isEmpty()){
            validateDuplicateCharge(baseDataValidator,duplicateCharge);
        }
    ...
    private void validateDisbursementDate(LocalDate expectedDisbursementDate) {
        if (Objects.nonNull(expectedDisbursementDate) && expectedDisbursementDate.isAfter(LocalDate.now())) {
            final String errorMessage = "The date on which a loan is disbursement cannot be future date.";
            throw new InvalidLoanStateTransitionException("disbursal", "cannot.be.future.date", errorMessage,expectedDisbursementDate);
        }
    }

    private void validateDuplicateCharge(DataValidatorBuilder baseDataValidator, List<Charge> duplicateCharge) {

        Set<Charge> seen = new HashSet<>();
        Set<Charge> duplicates = duplicateCharge.stream().filter(id -> !seen.add(id)).collect(Collectors.toSet());
        ...
        if (element.isJsonObject() && this.fromApiJsonHelper.parameterExists(chargesParameterName, element)) {
            ...
            if (topLevelJsonElement.get(chargesParameterName).isJsonArray()) {
                ...
                final Set<String> supportedParameters = new HashSet<>(
                        Arrays.asList("id", "chargeId", "amount", "chargeTimeType", "chargeCalculationType", "dueDate","selfShare","partnerShare"));
                ...
                for (int i = 1; i <= array.size(); i++) {
                    ...
                    final BigDecimal minChargeAmount=charge.getMinChargeAmount();
                    final BigDecimal maxChargeAmount=charge.getMaxChargeAmount();

                    if (maxChargeAmount != null && maxChargeAmount.compareTo(BigDecimal.ZERO) >= 0) {

                        if (minChargeAmount != null && minChargeAmount.compareTo(BigDecimal.ZERO) >= 0) {
                            if (minChargeAmount.compareTo(maxChargeAmount) <= 0 && amount != null) {
                                baseDataValidator.reset().parameter(charge.getName()).value(amount).inMinAndMaxAmountRange(minChargeAmount,
                                        maxChargeAmount);
                            }}
//                        } else if (amount != null) {
//                            baseDataValidator.reset().parameter("ChargeAmount").value(amount).notGreaterThanMax(maxChargeAmount);
//                        }
                    } else if (minChargeAmount != null && minChargeAmount.compareTo(BigDecimal.ZERO) >= 0 && amount != null) {

                        baseDataValidator.reset().parameter(charge.getName()).value(amount).notLessThanMin(minChargeAmount);
                    }
...
//    public void validateMinMaxConstraintPartnerValues(final JsonElement element, final Office office )
//    {
//        final List<ApiParameterError> dataValidationErrors = new ArrayList<>();
//        final DataValidatorBuilder baseDataValidator = new DataValidatorBuilder(dataValidationErrors).resource("loan");
//
//        final BigDecimal aviabileLimit = office.getLimit();
//        final String principalParameterName = "principal";
//
//        if (this.fromApiJsonHelper.parameterExists(principalParameterName, element)) {
//            final BigDecimal principal = this.fromApiJsonHelper.extractBigDecimalWithLocaleNamed(principalParameterName, element);
//            baseDataValidator.reset().parameter(principalParameterName).value(principal).notNull().positiveAmount().lessThanAmount(principal,aviabileLimit);
//        }
//
//        if (!dataValidationErrors.isEmpty()) {
//            throw new PlatformApiDataValidationException("validation.msg.validation.errors.exist", "Validation errors exist.",
...
//    }
    ...
    public void validateLoanCharges(final List<LoanCharge> charges, final List<ApiParameterError> dataValidationErrors) {
        if (charges == null) {
            return;
        }
        final DataValidatorBuilder baseDataValidator = new DataValidatorBuilder(dataValidationErrors).resource("loan");
...
private void chargeValidation(JsonElement element, String chargesParameterList, DataValidatorBuilder baseDataValidator, List<Charge> duplicateCharge) {
    for (String chargesParameterName : chargesParameterList.split(",")) {
    if (element.isJsonObject() && this.fromApiJsonHelper.parameterExists(chargesParameterName, element)) {
        final JsonObject topLevelJsonElement = element.getAsJsonObject();
        final String dateFormat = this.fromApiJsonHelper.extractDateFormatParameter(topLevelJsonElement);
        final Locale locale = this.fromApiJsonHelper.extractLocaleParameter(topLevelJsonElement);
        ...
        if (topLevelJsonElement.get(chargesParameterName).isJsonArray()) {
            ...
            final JsonArray array = topLevelJsonElement.get(chargesParameterName).getAsJsonArray();
            for (int i = 1; i <= array.size(); i++) {
                ...
                final Long chargeId = this.fromApiJsonHelper.extractLongNamed("chargeId", loanChargeElement);
                baseDataValidator.reset().parameter(chargesParameterName).parameterAtIndexArray("chargeId", i).value(chargeId).notNull()
                        .integerGreaterThanZero();
                ...
                final BigDecimal amount = this.fromApiJsonHelper.extractBigDecimalNamed("amount", loanChargeElement, locale);
                baseDataValidator.reset().parameter(chargesParameterName).parameterAtIndexArray("amount", i).value(amount).notNull()
                        .positiveAmount();
                ...
                if (maxChargeAmount != null && maxChargeAmount.compareTo(BigDecimal.ZERO) >= 0) {
                    ...
                    if (minChargeAmount != null && minChargeAmount.compareTo(BigDecimal.ZERO) >= 0) {
                        ...
                        baseDataValidator.reset().parameter(charge.getName()).value(amount).notGreaterThanMax(maxChargeAmount);
                    }
                } else if (minChargeAmount != null && minChargeAmount.compareTo(BigDecimal.ZERO) >= 0 && amount != null) {
                    baseDataValidator.reset().parameter(charge.getName()).value(amount).notLessThanMin(minChargeAmount);
                }
