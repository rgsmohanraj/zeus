...
import org.vcpl.lms.infrastructure.dataqueries.service.ReadWriteNonCoreDataService;
...
@Component
@Scope("singleton")
@Tag(name = "Data Tables", description = "The datatables API allows you to plug-in your own tables (MySql) that have a relationship to a Zeus core table. For example, you might want to add some extra client fields and record information about each of the clients' family members. Via the API you can create, read, update and delete entries for each 'plugged-in' table. The API checks for permission and for 'data scoping' (only data within the users' office hierarchy can be managed by the user).\n"
        + "\n"
        + "The Zeus Reference App uses a JQuery plug-in called stretchydatatables (which in turn uses this datatables resource) to provide a pretty flexible CRUD (Create, Read, Update, Delete) User Interface.")
public class DatatablesApiResource {
    ...
    @Operation(summary = "List Data Tables", description = "Lists registered data tables and the Zeus Core application table they are registered \n"
            + "\n" + "ARGUMENTS\n" + "\n" + "apptable  - optional" + "\n" + "The Zeus core application table.\n" + "\n"
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    @Operation(summary = "Create Data Table", description = "Create a new data table and registers it with the Zeus Core application table.\n"
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    @Operation(summary = "Delete Data Table", description = "Deletes a data table and deregisters it from the Zeus Core application table.")
    @ApiResponses({
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    @Operation(summary = "Retrieve Data Table Details", description = "Lists a registered data table details and the Zeus Core application table they are registered to.")
    @ApiResponses({
