 ...
 * under the License.
 */
package org.vcpl.lms.template.api;
...
import org.vcpl.lms.template.data.TemplateData;
import org.vcpl.lms.template.domain.Template;
import org.vcpl.lms.template.domain.TemplateEntity;
import org.vcpl.lms.template.domain.TemplateType;
import org.vcpl.lms.template.service.TemplateDomainService;
import org.vcpl.lms.template.service.TemplateMergeService;
...
@Tag(name = "User Generated Documents", description = "User Generated Documents(alternatively, Templates) are used for end-user features such as custom user defined document generation (AKA UGD). They are based on {{ moustache }} templates. Think of them as a sort of built-in \"mail merge\" functionality.\n"
        + "\n"
        + "User Generated Documents (and other types of templates) can aggregate data from several Zeus back-end API calls via mappers. Mappers can even access non-Zeus REST services from other servers. UGDs can render such data in tables, show images, etc. TBD: Please have a look at some of the Example UGDs included in Zeus (or the Wiki page, for now.).\n"
        + "\n"
